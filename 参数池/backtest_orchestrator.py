...
def _run_backtest_init(params: Dict[str, float], bar_data: pd.DataFrame,
    ...
    _trial_isolation_flag = uuid.uuid4().hex[:12]
    try:
...
def _run_backtest_finalize(bt, bar_data: pd.DataFrame, params: Dict[str, float],
    ...
    if len(bt.positions) > 0:
        try:
            from ali2026v3_trading.param_pool.backtest_checks import _force_close_all_position
            ...
            try:
                from ali2026v3_trading.参数池.backtest_checks import _force_close_all_position
                _last_bar = bar_data.iloc[-1] if bar_data is not None and len(bar_data) > 0 else None
                if _last_bar is not None:
                    _force_close_all_position(bt, _last_bar, params, reason="backtest_end_force_close")
            except ImportError:
