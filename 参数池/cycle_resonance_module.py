...
class CycleResonanceModule:
    ...
    def get_signal_priority(self, phase: Phase) -> List[str]:
        """策略优先级：资金冲突时的分配顺序"""
        if phase == Phase.RELEASE:
            return ["resonance", "spring", "high_freq", "box"]
        elif phase == Phase.CHARGE:
            return ["spring", "high_freq", "resonance", "box"]
        elif phase == Phase.EXHAUST:
            return ["high_freq", "box", "spring", "resonance"]
        else:
            return ["high_freq", "resonance", "box", "spring"]
...
_CRM_GLOBAL_KEY = '_CYCLE_RESONANCE_MODULE_INSTANCE'
_crm_lock = _crm_threading.Lock()
_crm_instance = None
...
def get_cycle_resonance_module(**kwargs) -> CycleResonanceModule:
    ...
    with _crm_lock:
        if _crm_instance is None:
            _crm_instance = CycleResonanceModule(**kwargs)
