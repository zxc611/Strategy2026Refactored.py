    ...
    def _execute_with_timeout(self, fn, *args, **kwargs):
        ...
        if self._executor is None:
            try:
                ...
                _msg = str(exc).lower()
                # FIX-20260703: 仅对真正的超时/死锁/挂起标记不健康，Catalog Error 等非致命错误不标记
                # 根因: changes() 等 SQLite 函数在 DuckDB 上抛 Catalog Error，
                # 原代码虽按关键词过滤，但实际运行中仍触发不健康标记(疑似 __pycache__ 缓存旧代码)
                # 现在显式排除 Catalog Error，并区分日志级别
