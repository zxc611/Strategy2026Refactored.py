...
统一序列化工厂- 修复SER-01/SER-P1-01/SER-P1-03
...
审计报告：第二十轮独立审计报告警全系统全链路核查找20260523.md
...
# FIX-20260702: 本地化 get_logger，切断 serialization_utils <-> _helpers 循环导入。
# _helpers 需要本模块的 json_dumps/json_loads，因此本模块不能再从 _helpers 导入。
...
# [P2-06] 权威序列化工具，所有json/pickle/yaml操作应委托此模块
...
def json_default_serializer(obj: Any) -> Any:
    ...
    - datetime.datetime/datetime.date _ISO格式字符串（可逆）'
    - pandas.Timestamp _ISO格式字符串（可逆）

    - numpy.integer _Python int

    - numpy.floating _Python float（含NaN/Infinity特殊处理器

    - decimal.Decimal _str（精度保留）
    ...
    - NaN _{"__special__": "NaN"}

    - Infinity _{"__special__": "Infinity"}
    ...
    if isinstance(obj, datetime.datetime):
        ...
        except (ValueError, TypeError, AttributeError) as _err:

            import logging as _logging

            _logging.debug("[serialization_utils] 数据解析降级: %s", _err)
...
def json_object_hook(dct: Dict[str, Any]) -> Any:  # [R22-P2-TS26]
    ...
    JSON反序列化化- 修复SER-01/SER-P1-01/SER-P1-03
    ...
    - {"__datetime__": "..."} _datetime.datetime

    - {"__date__": "..."} _datetime.date

    - {"__timestamp__": "..."} _pandas.Timestamp

    - {"__special__": "NaN/Infinity/-Infinity"} _numpy_

    - {"__decimal__": "..."} _decimal.Decimal

    - {"__series__": [...]} _pandas.Series

    - {"__dataframe__": [...]} _pandas.DataFrame
...
def json_dumps(obj: Any, indent: Optional[int] = None, sort_keys: bool = True, ensure_ascii: bool = False, **kwargs) -> str:
    ...
    统一JSON序列化接收- 替换json.dumps(..., default=str)
    ...
    >>> json_str = json_dumps(data, indent=2) # 缩进输出，适用于配置文档
    ...
    - indent=None（紧凑输出，适用于JSONL日志；配置文件可传indent=2_
    ...
    R21-MEM-P1-14修复: 对相同对象缓存序列化结果，避免高频调用重复序列化化
    ...
    # R21-MEM-P1-14修复: LRU缓存 _对可哈希的简单对象缓存json.dumps结果
...
def json_loads(s: str, **kwargs) -> Any:
    ...
    try:
        result = json.loads(s, object_hook=json_object_hook, **kwargs)
        return result
    except (json.JSONDecodeError, ValueError, TypeError) as e:
        _json_loads_fail_count['count'] += 1
        if _json_loads_fail_count['count'] % 100 == 0:
            logging.warning("[Observability] json_loads反序列化失败累计: %d次, 最近错误: %s", _json_loads_fail_count['count'], e)
        raise


_json_loads_fail_count: Dict[str, int] = {'count': 0}
...
def safe_json_dumps(obj: Any, fallback: str = "null", **kwargs) -> str:
    ...
    解决SER-P1-02（事件持久化截断可能损坏JSON结构建



    仅用于日志非关键路径。关键路径应使用json_dumps（失败时抛TypeError）_
...
def validate_json_roundtrip(obj: Any) -> tuple[bool, str]:
    ...
    - (False, error_msg) - roundtrip失败，返回错误信号
...
def yaml_safe_load(stream):
    ...
    SER-P1-13修复: YAML布尔值守卫的预处理yes/no/on/off为显式布尔值，

    防止YAML将布尔字符串静默转换为Python bool导致的配置语义错误率
...
def safe_pickle_dump(obj: Any, filepath: str) -> None:
    ...
    # SER-P1-07修复: 可序列化性预检查链序列化后立即反序列化验证，失败则抛出SerializationError
    ...
    try:
        ...
        if type(_loaded) != type(obj):
            ...
            raise SerializationError(
                ...
                f"pickle roundtrip类型不一） original={type(obj).__name__}, loaded={type(_loaded).__name__}"
        ...
        try:
            ...
            _logging.debug("[serialization_utils] I/O操作降级: %s", _err)

        raise

    except (TypeError, ValueError, EOFError, AttributeError, ImportError) as _rt_err:

        import logging as _logging

        _logging.warning(

            "SER-P1-07: pickle roundtrip验证失败(非致命): %s, 文件已写入但可能不可逆",
...
def safe_dataframe_to_parquet(
    ...
    - P2-12: Schema版本元数据（写入schema_version和created_at_
...
def safe_dataframe_from_parquet(
    ...
    SER-P1-04修复: 安全DataFrame从Parquet加载，恢复时区信号

    

    P2-12: 读取时验证schema_version元数据，版本不匹配时发出WARNING_
        ...
        if _metadata and isinstance(_metadata, dict):
            ...
            if file_version and file_version != PARQUET_SCHEMA_VERSION:
                ...
                _logging.warning(
                    ...
                    "P2-12: Parquet schema版本不匹配file=%s current=%s: %s",

                    filepath, PARQUET_SCHEMA_VERSION, file_version,

                )

                # SER-P1-05修复: Schema evolution _自动添加缺失败填充NaN)并删除多余列

                try:

                    from ali2026v3_trading.infra.serialization_utils import safe_dataframe_to_parquet as _safe_to_parquet

                    # 读取当前schema期望的列集（通过写入一个空DataFrame获取消

                    _expected_cols = set(pd.DataFrame().columns)  # fallback空集

                    # 使用当前版本写入一个临时文件来获取期望的列表
                    ...
                    if _missing_cols:
                        ...
                        _logging.info(
                            ...
                            "SER-P1-05: Schema evolution添加缺失败 %s",
                    ...
                    if _extra_cols and _expected_cols:  # 仅当有期望列集时才删除
                        ...
                        _logging.info(
                            ...
                            "SER-P1-05: Schema evolution删除多余） %s",

                            sorted(_extra_cols),

                        )

                except (ValueError, KeyError, TypeError, AttributeError, OSError) as _evo_err:
    ...
    except (OSError, IOError, ValueError, TypeError, KeyError) as _err:
        ...
        for col in list(df.columns):
            ...
            if f'{col}_timezone' in df.columns:
                ...
                if pd.api.types.is_datetime64_any_dtype(df[col]) and tz_str:
                    ...
                    except (ValueError, TypeError, KeyError) as _err:
...
def safe_jsonl_append_line(f, record: Any) -> None:
    ...
    解决：单条记录序列化后可能超长（如含大字段），导致JSONL文件损坏或读取OOM_
    ...
    2. 若行长度超过MAX_JSONL_LINE_LENGTH(1MB)，截断并添加权权truncated标记
...
def validate_duckdb_parquet_chain(
    ...
    SER-P1-11修复: DuckDB类型一致性检查链验证DataFrame→Parquet→DuckDB往返类型一致。'
    ...
    try:
        ...
        from ali2026v3_trading.data.db_adapter import connect_in_memory, fetchdf, close
    ...
    for col in original_dtypes:
        ...
        if col not in result_dtypes:
            ...
            mismatches.append(f"{col}: 列缺陷原{original_dtypes[col]})")
    ...
    for col in result_dtypes:
        ...
        if col not in original_dtypes:
            ...
            mismatches.append(f"{col}: 额外观类型{result_dtypes[col]})")



    if mismatches:

        return False, f"SER-P1-11: 类型不一） {'; '.join(mismatches)}"
    ...
    if roundtrip_check:
        ...
        for col in df.columns:
            ...
            if len(orig_vals) != len(res_vals):
                ...
                mismatches.append(f"{col}: 行数不一。{len(orig_vals)}→{len(res_vals)})")
            ...
            if not orig_vals.equals(res_vals):
                ...
                mismatches.append(f"{col}: 值不一。{diff_count}处差）")



    if mismatches:

        return False, f"SER-P1-11: roundtrip不一） {'; '.join(mismatches)}"
...
def normalize_parquet_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """SER-P1-15修复: 统一Parquet列名为小写，消除跨环境大小写不一。'
    DuckDB/Spark等引擎读取Parquet时可能将列名转为小写入

    此函数统一为小写，确保round-trip一致性能
...
if __name__ == '__main__':
    ...
    print(f"\nRoundtrip验证：{'_通过' if is_valid else '_失败'}")
...
def rotate_jsonl_if_needed(filepath: str, max_bytes: int = 50 * 1024 * 1024,

                           backup_count: int = 3) -> bool:

    """P2-01修复: JSONL文件轮转公共函数 _统一3个调用点的重复实现



    当文件大小超过max_bytes时执行编号轮。'
      - 删除最老备份.{backup_count}

      - 依次重命令.{backup_count-1} _.{backup_count}, ..., .1 _.2

      - 当前文件重命名为 .1

    使用os.replace()替代os.rename()确保原子子跨平台安全

    对ENOSPC(磁盘满记录CRITICAL日志而非静默吞没
    ...
    Args:
        ...
        max_bytes: 最大文件大的字节),默认50MB

        backup_count: 保留备份份默认3
        ...
        if os.path.exists(src):
            ...
            try:
                ...
                if _e.errno == errno.ENOSPC:
                    ...
                    logging.critical("[rotate_jsonl] 磁盘空间不足,无法轮转 %s _%s", src, dst)

                else:

                    logging.debug("[rotate_jsonl] 重命名失败%s _%s: %s", src, dst, _e)
    ...
    try:
        ...
        logging.info("[rotate_jsonl] 已轮）%s (超过%dMB)", filepath, max_bytes // (1024 * 1024))
        ...
        if _e.errno == errno.ENOSPC:
            ...
            logging.critical("[rotate_jsonl] 磁盘空间不足,无法轮转 %s", filepath)

        else:

            logging.error("[rotate_jsonl] 轮转失败 %s: %s", filepath, _e)
