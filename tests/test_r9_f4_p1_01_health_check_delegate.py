...
验证运行时行为：
1) HealthCheckAPI有_delegate_to_aggregator方法
2) 委托失败时回退到原有逻辑
...
def test_has_delegate_method():
    """HealthCheckAPI应有_delegate_to_aggregator方法"""
    from ali2026v3_trading.infra.health_monitor import HealthCheckAPI
    api = HealthCheckAPI()
    assert hasattr(api, '_delegate_to_aggregator'), \
        "HealthCheckAPI缺少_delegate_to_aggregator方法"
    print("  OK: HealthCheckAPI有_delegate_to_aggregator方法")
