...
class OrderService(BaseService):
    ...
    def send_order(
        ...
        # P1-12修复: 根据操作类型选择差异化超时
        _op_type = 'close' if action in ('CLOSE', 'close', 'SELL', 'sell') else 'open'
        _timeout = self._operation_timeouts.get(_op_type, self._operation_timeouts['default'])
        ...
        # EX-11修复: tick_size对齐 — 在订单提交路径强制对齐价格到最小变动价位整数倍
        price = self._correct_price(price, instrument_id)
        # SIG-02修复: 订单提交延迟打点
        ...
        if action != TradeAction.CLOSE and not is_chase:
        ...
        if action == TradeAction.OPEN:
            _opposite_dir = TradeDirection.SELL if direction == TradeDirection.BUY else TradeDirection.BUY
        ...
        try:
            ...
            if not hasattr(self, '_algo_compliance'):
                self._algo_compliance = AlgoTradingCompliance()
            try:
                self._algo_compliance.record_algo_order({
                    'instrument_id': instrument_id, 'volume': volume, 'price': price,
                    'direction': direction, 'action': action, 'signal_id': signal_id,
                })
            except Exception as _ac_err:
                logging.debug("[AlgoTradingCompliance] record_algo_order异常(不阻断): %s", _ac_err)
            if not hasattr(self, '_wash_detector'):
                self._wash_detector = WashTradeDetector()
            try:
                self._wash_detector.record_trade(instrument_id, direction, volume, price)
                _wash_result = self._wash_detector.detect_wash_trade(instrument_id)
                if _wash_result.get('suspected'):
                    logging.warning("[WashTradeDetector] 疑似洗盘检测: instrument=%s result=%s", instrument_id, _wash_result)
            except Exception as _wt_err:
                logging.debug("[WashTradeDetector] detect异常(不阻断): %s", _wt_err)
            with self._lock:
            ...
            if self._platform_insert_order and callable(self._platform_insert_order):
                try:
                    ...
                    # R27-P0-FIX: 记录下单时间戳到计数窗口
                    with self._lock:
                        self._order_count_window.append(time.time())
                    ...
                    # SIG-02修复: 记录订单提交延迟
                    _order_submit_delay_ms = (time.perf_counter() - _order_submit_start_ts) * 1000.0
                    if _order_submit_delay_ms > 10.0:
                        logging.warning('[SIG-02] 订单提交延迟: %.1fms instrument=%s', _order_submit_delay_ms, instrument_id)
    ...
    def cancel_order(self, order_id: str) -> bool:
        try:
            ...
            if self._platform_cancel_order and callable(self._platform_cancel_order):
                try:
                    # R33-P1-12: 撤单API添加超时保护（与_platform_submit对齐）
                    self._invoke_platform_cancel_with_timeout(platform_id)
                    logging.info("[OrderService] 平台撤单成功: %s", order_id)
                    # R27-P0-FIX: 记录撤单时间戳到计数窗口
                    with self._lock:
                        self._cancel_count_window.append(time.time())
                except Exception as e:
    ...
    # R27-P0-RC-04修复: cancel_all_pending方法——断路器/状态切换时撤销所有挂单
    ...
    def _get_tick_size(self, instrument_id: str) -> float:
        ...
        try:
            ...
            # ✅ P1修复：添加异常日志，便于诊断
            logging.debug("[OrderService._get_tick_size] Failed to get tick_size from ParamsService: %s", e)
    ...
    def _estimate_slippage(self, instrument_id: str, price: float, volume: int,
        ...
        """CS-01修复: 统一滑点估算 - 使用shared_utils.compute_slippage_bps"""
    ...
    def _chase_reorder(self, original_order: Dict, retry_count: int) -> None:
        ...
        if chase_ticks > max_chase_ticks:
            logging.warning("[OrderService._chase_reorder] Chase ticks %d exceeds max %d, capping", chase_ticks, max_chase_ticks)
    ...
    def _wal_write(self, order_id: str, state: str, order: Dict) -> None:
        try:
            ...
            if is_disk_full_error(e):
                ...
                # 磁盘满时尝试降级到内存队列
                if not hasattr(self, '_disk_full_mode'):
    ...
    def _append_order_state(self, order_id: str, state: str, order: Dict) -> None:
        ...
        try:
            ...
            with self._order_state_lock:
                self._rotate_jsonl_if_needed(self._order_state_file)
                with open(self._order_state_file, 'a', encoding='utf-8') as f:
                    safe_jsonl_append_line(f, record)
            # R33-P1-10: 写入成功则重置连续失败计数
            self._append_state_fail_count = 0
        except Exception as e:
            # R33-P1-10: 升级为error+连续失败计数+CRITICAL告警（与WAL路径R31-P1-10对齐）
            self._append_state_fail_count += 1
            logging.error("[R33-P1-10] 订单状态追加写失败(连续第%d次): order=%s state=%s err=%s",
                          self._append_state_fail_count, order_id, state, e)
            if is_disk_full_error(e):
                logging.critical("[R33-P1-16] 订单状态追加写失败: 磁盘满(ENOSPC)! err=%s", e)
                if not hasattr(self, '_disk_full_mode'):
                    self._disk_full_mode = True
                    logging.critical("[R33-P1-16] 进入磁盘满降级模式，后续订单仅内存暂存")
            if self._append_state_fail_count >= self._append_state_fail_critical_threshold:
                logging.critical(
                    "[R33-P1-10] CRITICAL: 订单状态追加写已连续失败%d次(阈值%d)，"
                    "WAL写入链路可能已损坏，订单状态持久化丢失风险！",
                    self._append_state_fail_count,
                    self._append_state_fail_critical_threshold,
                )
