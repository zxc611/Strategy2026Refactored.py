...
FEE_STRUCTURE_V2 = {
    "SSE": {
        "50ETF_OPTION": {"maker": 1.3, "taker": 1.3},
        "300ETF_OPTION": {"maker": 1.3, "taker": 1.3},
        "DEFAULT": {"maker": 1.5, "taker": 1.5},
    },
    "CFFEX": {
        "IO_INDEX_OPTION": {"maker": 10.0, "taker": 15.0},
        "MO_INDEX_OPTION": {"maker": 10.0, "taker": 15.0},
        "DEFAULT": {"maker": 12.0, "taker": 15.0},
    },
    "DCE": {
        "COMMODITY_OPTION": {"maker": 3.0, "taker": 5.0},
        "COMMODITY_FUTURE": {"maker": 8.0, "taker": 10.0},
        "DEFAULT": {"maker": 8.0, "taker": 10.0},
    },
    "DEFAULT": {
        "DEFAULT": {"maker": 12.0, "taker": 15.0},
    },
...
def _compute_commission(symbol: str, lots: int, open_time=None, close_time=None,
    ...
    if exchange is not None:
        ...
        if contract_fee is not None:
            rate = contract_fee.get(order_type, contract_fee.get("taker", 15.0))
            return rate * lots
...
def _check_safety(
    ...
    if bt.equity > 0 and len(bt.positions) > 0:
        try:
            ...
            _margin_occupancy = _total_margin / bt.equity
...
def _try_open(
    ...
    actual_lots = _compute_fill_quantity(lots, bar, params)
    if actual_lots <= 0:
    ...
    if params.get("execution_model", "standard") == "institutional":
        ...
        if open_delay > 0:
            ...
            bt.pending_orders.append(_PendingOrder(
                symbol=symbol,
                order_type=params.get("default_order_type", "taker"),
                volume=volume,
                lots=lots,
                reason=reason,
                signal_bar_idx=idx if 'idx' in dir() else 0,
                signal_price=price,
                tp_ratio=tp_ratio,
                sl_ratio=sl_ratio,
    ...
    if pos.volume > 0:
        ...
        if enable_gap and prev_close > 0:
            if bar_open <= sl_price:
                return (True, "GapStopLoss", bar_open)
            if bar_open >= tp_price:
                return (True, "GapStopProfit", bar_open)
        is_bullish = bar_close >= bar_open
        if is_bullish:
            if bar_low <= sl_price:
                return (True, "IntraBarStopLoss", sl_price)
            if bar_high >= tp_price:
                return (True, "IntraBarStopProfit", tp_price)
        else:
            if bar_high >= tp_price:
                return (True, "IntraBarStopProfit", tp_price)
            if bar_low <= sl_price:
...
def _update_mtm_equity(
    ...
    for sym, pos in bt.positions.items():
        if getattr(pos, 'instrument_type', 'future') == 'option_buyer':
...
def _compute_fill_quantity(
    ...
    Returns:
        int: 实际可成交手数
    """
    if not params.get("enable_partial_fill", False):
        return order_lots
    if order_lots <= 0:
        return 0
    ...
    if bar_volume <= 0 or max_part_rate >= 1.0:
        return order_lots
    max_fill = max(1, int(bar_volume * max_part_rate))
...
def _apply_fidelity_presets(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if mode == "institutional":
        ...
        params.setdefault("max_participation_rate", 0.3)
                ...
                if order_age >= required_delay:
                    if order.symbol not in bt.positions:
                        ...
                        if safe_price_check(exec_price):
                            ...
                            slip_cost = exec_price * slip_bps / 10000 * order.lots
                            commission = _compute_commission(order.symbol, order.lots, is_open=(order.order_type == "open"), exchange_id=_infer_exchange_id(order.symbol))
                            if order.order_type == "open":
                                ...
                                if direction > 0:
                                    ...
                                    exec_price -= exec_price * slip_bps / 10000
                                sp_price = exec_price * order.tp_ratio if order.volume > 0 else exec_price / order.tp_ratio
                                sl_price = exec_price * (1 - order.sl_ratio) if order.volume > 0 else exec_price * (1 + order.sl_ratio)
                                pos = _BacktestPosition(
