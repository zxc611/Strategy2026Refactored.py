...
from ali2026v3_trading.strategy_lifecycle_mixin import StrategyState, _LifecycleMixin
    ...
    def _init_core_deps(self, event_bus):
        try:
            ...
            if PhaseFeatureFlag.is_enabled('USE_BUILDER_INIT'):
                ...
                _b.apply_to(self)
                self._callback_group = self._try_import('ali2026v3_trading.callback_registry', 'get_callback_group')
                self._data_access = self._try_import('ali2026v3_trading.data_access', 'get_data_access')
                self._strategy_executor = self._create_executor(); return
        except Exception: pass
        self._state_store = self._try_import('ali2026v3_trading.state_store', 'get_state_store')
        self._callback_group = self._try_import('ali2026v3_trading.callback_registry', 'get_callback_group')
        self._data_access = self._try_import('ali2026v3_trading.data_access', 'get_data_access')
...
# re-export保持向后兼容
from ali2026v3_trading.strategy_2026 import StrategyParams, Strategy2026
