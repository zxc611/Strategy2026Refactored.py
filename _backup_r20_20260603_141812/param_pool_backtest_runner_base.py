...
from ali2026v3_trading.shared_utils import compute_slippage_bps, UNIFIED_EXPIRY_SLIPPAGE_MULTIPLIERS
try:
        ...
        if bar is not None and len(bt.positions) > 0:
            for sym in list(bt.positions.keys()):
                ...
                bt.closed_trades.append(_ClosedTrade(
                    ...
                    premium_pnl=_premium_pnl if '_premium_pnl' in dir() else 0.0, delta_pnl=_delta_pnl if '_delta_pnl' in dir() else 0.0,
...
def _try_open(
    ...
    # CS-03修复: 使用基于波动率缩放的延迟滑点公式（替代魔法数字0.1）
    if _exec_delay_ms > 0:
