...
class ModeEngine:
    ...
    def switch_mode(self, scale_str: str, reason: str = 'unknown') -> Dict[str, Any]:  # R24-P1-TR-03修复: 添加切换原因
        ...
        with self._propagation_lock:
            ...
            # R27-P0-FIX: 保存RiskService旧params用于回滚
            _old_risk_params = {}
            try:
                ...
                if hasattr(rs, 'params') and rs.params is not None:
                    _old_risk_params = {
                        'consecutive_loss_limit': rs.params.get('consecutive_loss_limit'),
                        'asymmetric_risk_enabled': rs.params.get('asymmetric_risk_enabled'),
                        'recovery_timeout_seconds': rs.params.get('recovery_timeout_seconds'),
                    }
            except Exception:
                pass
            self._old_risk_params = _old_risk_params
        ...
        for comp in reversed(propagated):
            try:
                if comp == 'RiskService':
                    ...
                    # R27-P0-FIX: 恢复RiskService params
                    if hasattr(rs, 'params') and rs.params is not None and hasattr(self, '_old_risk_params') and self._old_risk_params:
                        for _k, _v in self._old_risk_params.items():
                            if _v is not None:
                                rs.params[_k] = _v
