...
from infra.shared_utils import api_version  # P1-21: 统一从shared_utils导入
...
class PositionCommandService:
    ...
    def _resolve_strategy_group(self, open_reason: str) -> str:
        ...
        # 根因: dry_run模式在order_executor_validation.py:350和callback_injector.py:143
        #   给open_reason加'dry_run:'前缀(如'dry_run:DIVERGENCE_REVERSAL'),
        #   但_REASON_STRATEGY_MAP的key不带前缀(如'DIVERGENCE_REVERSAL'),
        ...
        # FIX-SG-MAP-FAIL-LOUD-20260725: 从根因消除"回退high_freq"隐患
        #   原代码: return _result if _result else 'high_freq' → 未注册reason静默错配high_freq风控
        #   S7资金崩盘根因正是此回退: dry_run:DIVERGENCE_REVERSAL查表失败→high_freq→churn→equity=2.20
        #   修复: 查表失败时记录CRITICAL日志 + 返回'unknown'(不使用high_freq激进风控)
        #   get_risk_surface('unknown')返回默认RiskSurfaceAdjustment(保守参数),不会引发新隐患
        #   不修改策略逻辑(映射表不变), 仅修复fallback行为
    ...
    def on_trade(self, trade: Any) -> None:
        from infra.shared_utils import require_precondition
    ...
    def on_tick(self, tick: Any) -> None:
        try:
            ...
            try:
                from infra.event_bus import get_global_event_bus, TickEvent
    ...
    def _add_position(self, exchange: str, instrument_id: str,
        ...
        with self._ps._get_instrument_lock(instrument_id):
            ...
            # V4-FIX-C1: stop_loss_price<=0时拒绝开仓(fail-closed)
            # 原则: 止损价无效=持仓无保护=不开仓，而非止损价无效=放行开仓
            if sl_price <= 0:
                logging.error("[V4-FIX-C1] stop_loss_price<=0, 拒绝开仓 (fail-closed): inst=%s price=%.4f sl_price=%.4f sl_ratio=%.4f",
                              instrument_id, price, sl_price, sl_ratio)
                return

            from position.position_service import PositionRecord

            # [R28-C023修复] 交易前捕获快照，用于错误交易回滚
            _prev_pos = self._ps.positions[instrument_id].get(pos_id)
            try:
                from strategy_judgment.causal_chain_utils import TradeRollbackManager
            ...
            try:
                from strategy_judgment.market_snapshot_collector import SnapshotTrigger
            ...
            if self._ps._structured_logger is not None:
                try:
                    ...
                    pass
            try:
                from infra.event_bus import get_global_event_bus, PositionEvent
                _eb = get_global_event_bus()
                if _eb and not getattr(_eb, '_shutdown', True):
    ...
    def _reduce_position(self, exchange: str, instrument_id: str, volume: int, is_buy: bool, price: float) -> None:
        ...
        with self._ps._get_instrument_lock(instrument_id):
            ...
            # FIX-OO3 (R37-ROOT-CIRCULAR): 反转FIX-NN1的_closing排除逻辑
            # 根因: FIX-NN1排除_closing=True记录→成交回调到达时closeable=0→_reduce_position早退
            #       →持仓永不被reduce→_closing永不清零→持仓卡死堆积→R37 WARNING 974次
            # 修复: 恢复包含_closing记录的closeable计算。_closing标志的防重复作用
            #       由_trigger_close_position L1011的closing_order_id guard保证，
            #       不应由_reduce_position的closeable计算承担防重复职责。
    ...
    def _trigger_close_position(self, record, reason: str, current_price: float = 0.0) -> None:
        ...
        try:
            from infra.market_time_service import get_market_status
        ...
        with self._ps._get_instrument_lock(record.instrument_id):
            ...
            try:
                ...
                if price <= 0:
                    ...
                    # V4-FIX-C12: 最后fallback优先使用current_price(非open_price), 避免胖手指
                    # 原FIX-R26b直接使用open_price可能导致平仓价远离市场价→胖手指防护触发→平仓失败→C-3重试耗尽
                    if base <= 0:
                        _last_price = getattr(record, 'current_price', 0.0) or 0.0
                        if _last_price > 0:
                            base = float(_last_price)
                            logging.info("[V4-FIX-C12] 使用current_price作为平仓价: %s current_price=%.4f", record.instrument_id, base)
                        else:
                ...
                if order_id:
                    ...
                    record._closing = True
                    try:
                        from order.order_service import get_order_service as _get_os
                        _osvc = _get_os()
    ...
    def _schedule_close_retry(self, record, price: float) -> None:
        ...
        if self._close_retry_executor is None:
            ...
            try:
                from lifecycle.lifecycle_resource import register_thread_pool
        ...
        def _retry_worker():
            ...
            if result is None:
                ...
                # V4-FIX-C3: 重试耗尽时区分幽灵持仓和真实持仓
                # 原则: 幽灵持仓(平台确认不存在)→删除；真实持仓(平台有但无法平仓)→标记CANNOT_CLOSE不删除
                _is_ghost_close = False
                _platform_get_pos = getattr(self._ps, '_platform_get_position', None)
                if callable(_platform_get_pos):
                    try:
                        _platform_pos = _platform_get_pos(record.instrument_id)
                        if _platform_pos is None or (hasattr(_platform_pos, 'volume') and getattr(_platform_pos, 'volume', 0) == 0):
                            _is_ghost_close = True
                    except (ValueError, KeyError, TypeError, AttributeError) as _gpe2:
                        logging.debug("[V4-FIX-C3] 平台持仓查询失败, 按真实持仓处理: %s", _gpe2)

                if _is_ghost_close:
                    logging.error("[V4-FIX-C3] 平仓重试耗尽+平台确认幽灵持仓, 删除: inst=%s pos_id=%s method=%s",
                    ...
                    self._rollback_position(record.instrument_id, getattr(record, 'position_id', ''),
                                            reason=f'ghost_cannot_close:{_exhausted_method}')
                else:
                    # V4-FIX-C3: 真实持仓无法平仓→标记CANNOT_CLOSE, 不删除, 清除_closing允许后续重试
                    # V4-FIX-C2: 清除_closing标志防止持仓卡死，允许后续重试或人工介入
                    with self._ps._get_instrument_lock(record.instrument_id):
                        record.close_method = 'CANNOT_CLOSE'
                        record._closing = False
                        record.closing_order_id = ''
                    logging.critical("[V4-FIX-C3] 平仓重试耗尽+平台有持仓, 标记CANNOT_CLOSE(不删除): inst=%s pos_id=%s method=%s",
                                     record.instrument_id, getattr(record, 'position_id', ''), _exhausted_method)
