...
def _calc_max_volume_from_capital(capital: float, price: float, leverage: float = 1.0, margin_rate: float = 0.15) -> int:
    try:
        ...
        # FIX-POS-UTIL-EXCEPT (2026-07-20, RC-4): 窄异常元组扩展为 except Exception
        # 原窄异常元组无法覆盖 ZeroDivisionError/OSError 等未列举异常
        return 0
...
def _cancel_and_resend(order_id: str, new_params: Dict[str, Any]) -> bool:
    try:
        ...
        # FIX-POS-UTIL-EXCEPT (2026-07-20, RC-4): 窄异常元组扩展为 except Exception
        # 根因: 违反 NEW-1 硬约束（实时回调路径必须用 except Exception）
        # 原窄异常元组 (ValueError/KeyError/TypeError/RuntimeError/AttributeError)
        # 无法覆盖 OSError/ZeroDivisionError 等未列举异常，导致穿透到上层
...
def _check_available_amount(position_data: Dict[str, Any], amount: int) -> bool:
    try:
        ...
        # FIX-POS-UTIL-EXCEPT (2026-07-20, RC-4): 窄异常元组扩展为 except Exception
        # 原窄异常元组无法覆盖 OSError/TypeError 子类等未列举异常
        return False
