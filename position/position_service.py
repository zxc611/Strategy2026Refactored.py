...
from risk.risk_support import RiskBridgeAdapter, PositionBridgeAdapter, BridgeRiskLevel

try:
    from infra.security_service import structured_audit_log as _structured_audit_log  # R1-4修复
except ImportError:
    _structured_audit_log = None

# CP-06修复: 以下import从函数体内延迟import提升为模块级import（共享常量模块无循环依赖风险）
from infra.commission_utils import REASON_MULTIPLIERS  # R27-CP-01-FIX
...
class PositionService(object):
    ...
    def __init__(self, risk_service: Any = None):
        ...
        try:
            from order.order_persistence import SelfTradeDetector, NetworkRetryManager, PartialFillHandler
        ...
        from config._params_canary_env import DEFAULT_LOG_DIR as _DEFAULT_LOG_DIR2
        ...
        # CC-09: 初始化子服务
        from position.position_command_service import PositionCommandService
        from position.position_check_service import PositionCheckService
        from position.position_pnl_service import PositionPnlService
        from position.position_persistence import PositionPersistenceService
    ...
    def _get_platform_attr(obj: Any, *attr_names: str, default: Any = None) -> Any:
        ...
        try:
            from infra.shared_utils import get_tick_field, TICK_FIELD_NAMES
    ...
    def _apply_crm_stop_loss_adjustment(self, sl_ratio: float, open_reason: str) -> float:
        try:
            ...
            strategy = self._map_reason_to_strategy(open_reason)  # FIX-SG-MAP-20260724: 统一走规范化映射,剥离dry_run:前缀
...
from position.position_utils import (
...
from position.position_greeks import (
