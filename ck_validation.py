...
# DEPRECATED(P0-20): validate_logic_reversal_no_future别名已删除（权威实现在param_pool/checks_individual.py）_

# 本模块仍被生产代码引用 risk/risk_service.py(check_safety_meta_layer, validate_circuit_breaker_vs_time_stop)_

# param_pool/validation_deep_orchestrator.py(validate_indicator_multiscale_consistency)。待上述引用迁移后可删除除
...
def validate_shadow_b_stability(
    ...
    if len(arr) < 2:
        ...
        return {'passed': False, 'ci_width': float('inf'), 'reason': '样本存2'}
...
def validate_tick_rounding_impact(
    ...
    max_sharpe_diff: float = 0.2,

    min_ratio: float = None,

) -> Dict[str, Any]:

    """P2-裂缝13修复: 验证取整前后Sharpe绝对差异 < max_sharpe_diff(默认0.2)_



    原实现使用相对比。min_ratio=0.7)，与裂缝要求不符。'
    修复后改用绝对差） abs(sharpe_before - sharpe_after) < max_sharpe_diff_

    保留min_ratio参数以兼容旧调用方法

    """

    sharpe_diff = abs(sharpe_before - sharpe_after)

    if min_ratio is not None:

        ratio = sharpe_after / sharpe_before if sharpe_before != 0 else 0.0

        passed = ratio >= min_ratio

    else:

        passed = sharpe_diff < max_sharpe_diff
...
def compute_cascade_slippage(
    ...
    cascade_cap_bps: float = 50.0,

) -> float:

    """计算状态切换时的级联滑点。P0-20修复: 与回测版对齐使用sqrt冲击模型



    新开仓滑点= min(原滑点× (1 + sqrt(平仓位平均成交叉) × cascade_multiplier, cap)
    ...
    result = base_slippage_bps * volume_impact * cascade_multiplier

    return min(result, cascade_cap_bps)
...
def validate_cascade_slippage_impact(
    ...
    if sharpe_before <= 0:
        ...
        return {'passed': True, 'decay_pct': 0.0, 'reason': '原夏。=0'}
...
def validate_circuit_breaker_vs_time_stop():
    ...
    """验证4种组合下can_open/can_close的正确性能"""
...
def validate_indicator_multiscale_consistency(
    ...
    """验证原生频率指标下采样vs 重采样后直接计算的相关系。> min_corr_"""
...
def validate_alpha_confidence_overlap(
    ...
    """验证验影子Sharpe置信区间重叠比例 <= max_overlap_ratio_



    若重复50%，标记alpha_unreliable，资金分配回退到基础权重复
...
def validate_logic_reversal_timestamp_causality(
    ...
    """验证逻辑反转平仓触发时刻，wrong_pct不使用未来数据库
    ...
    2. wrong_pct_bar_timestamps: wrong_pct数据来源bar的时间戳戳

       必须 <= signal_timestamp（不使用信号触发后的bar数据库
...
def validate_shadow_param_orthogonality(
    ...
    """验证影子参数分布与主参数分布的KL散度 > min_kl_



    将参数向量直方图化为离散分布后计算KL散度量
    ...
    if vmax <= vmin:
        ...
        return {'passed': False, 'kl_divergence': 0.0, 'reason': '参数无差。'}
...
def validate_attribution_residual_significance(
    ...
    """验证归因残差是否统计显著且超过相对阈值值



    触发E7条件: t检验p<alpha _mean(|residual|) > residual_pct_threshold * mean(|pnl|)
