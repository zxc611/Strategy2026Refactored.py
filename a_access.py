...
from ali2026v3_trading.infra.shared_utils import sanitize_sql_identifier, CHINA_TZ as _CHINA_TZ
...
class DefaultDataAccess:
    ...
    def save_trade(
        ...
        try:
            from ali2026v3_trading.risk.risk_service import save_state_snapshot
            save_state_snapshot(
    ...
    def write_records(
        ...
        try:
            from ali2026v3_trading.data.db_adapter import connect, execute
    ...
    def execute_query(
        ...
        try:
            from ali2026v3_trading.data.db_adapter import connect_in_memory, connect, execute, fetchall
            from ali2026v3_trading.data.ds_realtime_cache import _resolve_duckdb_file
            if in_memory:
    ...
    def register_dataframe(self, name: str, df: Any) -> None:
        try:
            from ali2026v3_trading.data.db_adapter import connect, register_df
            from ali2026v3_trading.data.ds_realtime_cache import _resolve_duckdb_file
            db_path = _resolve_duckdb_file()
            conn = connect(db_path)
            try:
    ...
    def get_storage_stats(self) -> StorageStatsResult:
        try:
            from ali2026v3_trading.data.db_adapter import connect, fetchall
            from ali2026v3_trading.data.ds_realtime_cache import _resolve_duckdb_file
            import os
            db_path = _resolve_duckdb_file()
