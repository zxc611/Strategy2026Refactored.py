...
class OrderFlowBridge:
    ...
    _MicrostructureAnalyzer 封装为策略可用的实时服务务

    1. 从tick热路径接收成交数据（on_tick_feed_

    2. 从tick热路径接收订单簿快照（on_depth_feed_

    3. 提供实时订单流方向一致性查询（get_flow_consistency_

    4. 提供综合评估查询（get_composite_assessment_
    ...
    def __init__(self, config: MicrostructureConfig = None):
        ...
        # MS-02: 深度质量统计（REAL vs INFERRED_
    ...
    def _feed_depth_from_tick(self, product: str, bid_price: float,
        ...
        # MS-02: 标记INFERRED深度（从tick推断，非真实L2数据库
