...
_VALID_ORDER_TRANSITIONS = {
    ...
    'SUBMITTED': {'PARTIAL_FILLED', 'FILLED', 'ALL_FILLED', 'CANCELLED', 'FAILED', 'TIMEOUT', '部分成交', '全部成交', '已撤销', '部成部撤', '未成交', '已报入未应答', '待报入', '部分撤单还在队列中', '部成部撤还在队列中'},

    'PARTIAL_FILLED': {'FILLED', 'ALL_FILLED', 'CANCELLED', 'FAILED', 'TIMEOUT', '全部成交', '已撤销', '部成部撤', '部分撤单还在队列中', '部成部撤还在队列中'},

    'PARTIAL': {'FILLED', 'ALL_FILLED', 'PARTIAL_FILLED', 'CANCELLED', 'FAILED', 'TIMEOUT', '部分成交', '全部成交', '已撤销', '部成部撤', '部分撤单还在队列中', '部成部撤还在队列中'},

    'TIMEOUT': {'SUBMITTED', 'CANCELLED', 'FAILED', 'FILLED', 'ALL_FILLED', 'PARTIAL_FILLED', 'PARTIAL', '部分成交', '全部成交', '已撤销', '部分撤单还在队列中', '部成部撤还在队列中'},

    'FILLED': set(), 'ALL_FILLED': set(), 'CANCELLED': {'FILLED', 'ALL_FILLED', 'PARTIAL_FILLED', '全部成交', '部分成交', '未成交', '部成部撤', '部分撤单还在队列中', '部成部撤还在队列中'},

    'FAILED': {'SUBMITTED'}, 'ORPHANED': {'CANCELLED', 'FAILED', '已撤销'},

    '全成': set(), '全部成交': set(),

    '部分成交': {'FILLED', 'ALL_FILLED', 'CANCELLED', 'FAILED', 'TIMEOUT', '全部成交', '已撤销', '部成部撤', '部分撤单还在队列中', '部成部撤还在队列中'},

    '已撤销': {'FILLED', 'ALL_FILLED', 'PARTIAL_FILLED', '全部成交', '部分成交'},

    '未成交': {'部分成交', '全部成交', '已撤销', 'FILLED', 'CANCELLED', '部成部撤', '部分撤单还在队列中', '部成部撤还在队列中'},

    '部成部撤': {'FILLED', 'ALL_FILLED', 'CANCELLED', 'FAILED', '全部成交', '已撤销'},

    '已报入未应答': {'部分成交', '全部成交', '已撤销', '未成交', '部成部撤', '部分撤单还在队列中', '部成部撤还在队列中'},

    '待报入': {'未成交', '已报入未应答', '部分成交', '全部成交', '已撤销', '部分撤单还在队列中', '部成部撤还在队列中'},

    '部分撤单还在队列中': {'部分成交', '全部成交', '已撤销', '部成部撤', '部分撤单还在队列中', '部成部撤还在队列中'},

    '部成部撤还在队列中': {'部分成交', '全部成交', '已撤销', '部成部撤', '部分撤单还在队列中', '部成部撤还在队列中'},
...
class OrderQueryService:
    ...
    _PLATFORM_ATTR_MAP = {'order_id': 'order_id', 'status': 'status', 'traded_volume': 'traded_volume'}
    ...
    def _get_platform_attr(obj, *attr_names, default=None):
        ...
        for attr in attr_names:
            ...
            # FIX-R31-DICT: 支持dict格式，getattr无法获取dict的键值
    ...
    def _normalize_platform_result(self, result):
        if isinstance(result, int):
            return {'order_id': str(result)}
        if result is None:
            return {}
        _raw = {k: self._get_platform_attr(result, k, v) for k, v in self._PLATFORM_ATTR_MAP.items()}
        if 'order_id' in _raw and _raw['order_id'] is not None:
            _raw['order_id'] = str(_raw['order_id'])
        return _raw
    ...
    def get_order_by_platform_id(self, platform_order_id):
        ...
        with self._lock:
            ...
            # FIX-CLOSE-LOOKUP: 使用str()比较避免int/string类型不匹配导致查找失败
            _pid_str = str(platform_order_id)
            for o in self._orders_by_id.values():

                if str(o.get('platform_order_id')) == _pid_str: return o
