...
class OrderExecutor:
    ...
    def _pre_send_checks(self, ctx: OrderContext) -> OrderContext:
        ...
        if _HAS_CAUSAL_CHAIN:
            ...
            # [DRY-RUN A-05] dry_run模式下跳过CyclicDependencyGuard检查
            _dry_run_cd = False
            try:
                from ali2026v3_trading.config.config_service import get_cached_params as _gcp_cd
                _dry_run_cd = bool(_gcp_cd().get('dry_run_mode', False))
            except (ValueError, KeyError, TypeError, AttributeError, ImportError) as _dr_cd_err:
                pass
            if not _dry_run_cd:
    ...
    def _execute_platform_insert(self, ctx: OrderContext) -> OrderContext:
        ...
        try:
            ...
            # [DRY-RUN A-02] dry_run拦截分支：必须位于if svc._platform_insert_order之前(第586行之前)，
            # 否则R37-TRADING-RECOVERY(order_executor.py:610-633)的CLOSE订单恢复逻辑
            # 会强制setattr(_trading_flag, 'trading', True)并实际下单重试，穿透dry_run
            _dry_run = False
            try:
                from ali2026v3_trading.config.config_service import get_cached_params as _gcp_dr
                _dry_run = bool(_gcp_dr().get('dry_run_mode', False))
            except (ValueError, KeyError, TypeError, AttributeError, ImportError) as _dr_e:
                pass
            if _dry_run:
                _signal_id = ctx.signal_id or ctx.order.get('signal_id', '')
                _reason = ctx.open_reason or ctx.order.get('open_reason', ctx.order.get('close_reason', ''))
                _action_label = '开仓' if ctx.action in ('OPEN', 'BUY_OPEN', 'SELL_OPEN') else '平仓' if ctx.action in ('CLOSE', 'BUY_CLOSE', 'SELL_CLOSE') else ctx.action
                _strategy_name = ''
                try:
                    from ali2026v3_trading.config.config_service import get_cached_params as _gcp_sn
                    _strategy_name = _gcp_sn().get('strategy_name', '') or getattr(svc, 'strategy_id', '')
                except Exception:
                    _strategy_name = getattr(svc, 'strategy_id', '')
                logging.info("[DRY-RUN] %s快照 | 策略=%s 合约=%s 方向=%s 数量=%.0f 价格=%.4f 理由=%s signal=%s order=%s",
                             _action_label, _strategy_name, ctx.instrument_id, ctx.direction, ctx.volume, ctx.price,
                             _reason, _signal_id, ctx.order_id)
                with svc._lock:
                ...
                # 触发虚拟成交回报(详见m3-06)，闭合订单生命周期至FILLED
                try:
    ...
    def _simulate_dry_run_callbacks(self, ctx: Any) -> None:
        """[DRY-RUN m3-06] dry_run下构造虚拟on_order/on_trade回报，闭合订单生命周期至FILLED。
        ...
        def _fire() -> None:
            try:
                _time.sleep(_delay)
                # 1. 构造虚拟on_order回报(全部成交)
                # 注意: traded_volume对齐_PLATFORM_ATTR_MAP(order_id/status/traded_volume)，
                # 否则on_trade_update归一化后filled=0导致持仓更新拿到0成交量
                _virtual_order = {
                    ...
                    'price': ctx.price,
                    'status': '全部成交',  # 触发_is_filled分支
                    'memo': _internal_id,  # FIX-R34-MEMO: memo携带内部ID
                }
                # 2. 构造虚拟on_trade回报
                _virtual_trade = {
                ...
                if _bl is None:
                    try:
                        ...
                        if _strategy_obj is not None:
                            ...
                            if _bl is not None:
                                logging.debug("[DRY-RUN] 通过cached_params.strategy路径获取业务层成功")
                    except (ValueError, KeyError, TypeError, AttributeError, ImportError):
                        pass
                if _bl is None:
    ...
    def _invoke_platform_cancel_with_timeout(self, platform_id: str) -> None:
        ...
        def _target() -> None:
            ...
            try:
                ...
                # [DRY-RUN A-03] dry_run模式下跳过实际撤单调用，仅记录日志
                _dry_run_cancel = False
                try:
                    from ali2026v3_trading.config.config_service import get_cached_params as _gcp_c
                    _dry_run_cancel = bool(_gcp_c().get('dry_run_mode', False))
                except (ValueError, KeyError, TypeError, AttributeError, ImportError):
                    pass
                if _dry_run_cancel:
                    logging.info("[DRY-RUN] 撤单拦截(仅记录): platform_id=%s", platform_id)
                else:
