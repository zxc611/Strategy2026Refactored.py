...
order_compliance.py - 自成交检查算法交易报备/洗盘检查
...
- 拆单自成交检查(check_self_trade_across_splits)

- 算法交易报备 (AlgoTradingCompliance)

- 洗盘检查(WashTradeDetector)
...
def check_self_trade_across_splits(
    ...
    对一组拆分子单进行原子性自成交检查，检测同合约反方向的子单播
    ...
    Returns:
        ...
        List[Dict]: 检测到的自成交叉 含{buy_idx, sell_idx, instrument_id, ban_until}
                    ...
                    if price_a > 0 and price_b > 0:
                        ...
                        if buy_price >= sell_price:
                            ...
                            logging.warning(
                                ...
                                "[R18-06] 拆单自成交检查 BUY[%d]@%.2f >= SELL[%d]@%.2f instrument=%s",
