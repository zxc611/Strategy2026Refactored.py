...
from ali2026v3_trading.strategy.box_spring_detector import SpringState
        assert SpringState is not None

    def test_box_range_importable(self):
        from ali2026v3_trading.strategy.box_spring_detector import BoxRange
        assert BoxRange is not None

    def test_spring_signal_importable(self):
        from ali2026v3_trading.strategy.box_spring_detector import SpringSignal
        assert SpringSignal is not None

    def test_spring_position_importable(self):
        from ali2026v3_trading.strategy.box_spring_detector import SpringPosition
        assert SpringPosition is not None

    def test_pending_pullback_importable(self):
        from ali2026v3_trading.strategy.box_spring_detector import PendingPullback
        assert PendingPullback is not None

    def test_get_box_spring_strategy_importable(self):
        from ali2026v3_trading.strategy.box_spring_detector import get_box_spring_strategy
        assert callable(get_box_spring_strategy)

    def test_reexport_matches_source(self):
        from ali2026v3_trading.strategy.box_spring_detector import SpringState as SS_shim
...
class TestShadowStrategyTypesReexport:
    def test_shadow_trade_record_importable(self):
        from ali2026v3_trading.strategy._shadow_types import ShadowTradeRecord
        assert ShadowTradeRecord is not None

    def test_alpha_metrics_importable(self):
        from ali2026v3_trading.strategy._shadow_types import AlphaMetrics
        assert AlphaMetrics is not None

    def test_shadow_params_snapshot_importable(self):
        from ali2026v3_trading.strategy._shadow_types import ShadowParamsSnapshot
        assert ShadowParamsSnapshot is not None

    def test_reexport_matches_source(self):
        from ali2026v3_trading.strategy._shadow_types import ShadowTradeRecord as STR_shim
...
class TestStrategyCoreServiceBuilderReexport:
    ...
    def test_reexport_matches_source(self):
        from ali2026v3_trading.strategy.strategy_core_service import StrategyCoreServiceBuilder as B_shim
