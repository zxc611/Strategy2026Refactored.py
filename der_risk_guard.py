...
Phase 2 (CC-P1-01): 从OrderService提取的风控防护职责任
...
class OrderRiskGuard:
        ...
        if orders_by_id:
            ...
            try:
                ...
                for order in orders_by_id.values():
                    ...
                    if order.get('instrument_id') == instrument_id and order.get('status') in ('FILLED', 'ALL_FILLED', '全部成交'):
    ...
    def release_margin_reservation(self, order_id: str) -> None:
        ...
        try:
            ...
            logging.debug("[R14-P0-BIZ-01] 释放保证金预留失败可忽。: order_id=%s err=%s", order_id, e)
    ...
    def is_duplicate_order(self, svc, instrument_id: str, order_key: str) -> bool:

        with svc._lock:

            if instrument_id not in svc._recent_orders_by_instrument:

                svc._recent_orders_by_instrument[instrument_id] = []

            now = time.time()

            recent = svc._recent_orders_by_instrument[instrument_id]
