...
DEFAULT_PARAM_TABLE = {  # R21-MEM-P2-05修复: 模块级大字典(~200个，无分片机制；若内存敏感可按功能域拆分为子字典懒加载
    ...
    "STRATEGY_SL_RATIOS": {
        ...
        "incorrect_reversal": 0.6,  # R27-P0-FIX: 与V7.0手册§9.2及position_service对齐
        "other": 0.5,
        "box_extreme": 0.3,
        "hft": 0.2,
    },
...
PARAMS_JSON_CACHE_TTL: float = 60.0  # R23-FR-01-FIX: JSON缓存TTL从00秒缩短至60秒  # R17-P2-CFG-02: 可配置化
...
def check_config_hot_reload() -> bool:
    ...
    try:
        ...
        if current_mtime > _config_file_watchdog_last_mtime:
            # P1-13修复: 热重载前验证format_version，防止不兼容配置生效
            try:
                ...
                if _format_ver is not None and _format_ver != '1.0':
                    logging.warning("[P1-FIX] Config hot-reload: format_version mismatch: %s (expected 1.0), keeping current config", _format_ver)
                    _config_file_watchdog_last_mtime = current_mtime
                    return False
            except (json.JSONDecodeError, IOError) as _cfg_err:
                logging.warning("[P1-FIX] Config hot-reload: failed to read config for format_version check: %s", _cfg_err)
                _config_file_watchdog_last_mtime = current_mtime
                return False
            _config_file_watchdog_last_mtime = current_mtime
            reset_param_cache()
            logging.info("[R4-D-10] 配置热更新 %s 已变更，缓存已刷新", path)
            # SER-P1-14修复: 热重载后序列化格式一致性验证
            try:
                ...
                # 检查序列化后是否能正常还原
                from ali2026v3_trading.serialization_utils import json_loads
            ...
            # R17-P2-CFG-06: 热重载后的配置变更未通知HealthCheckAPI — 需在此处添加 health_check_api.notify_config_changed() 调用
            return True
...
def update_cached_params(updates: Dict[str, Any], sync_default_table: bool = True, caller_id: str = "unknown", strategy_id: Optional[str] = None) -> Dict[str, Any]:  # [R22-P2-TS03]  # P0-R11-18修复: 支持策略级缓存隔离
    ...
    if caller_id not in _TRUSTED_CALLERS:
        ...
        from ali2026v3_trading.audit_log_utils import structured_audit_log as _sal_unauth  # R27-CP-05-FIX: 从独立模块导入
    ...
    for change in audit_changes:
        ...
        try:
            ...
            _hmac_sig = hashlib.new(_hmac_key.encode(), f'{change}:{caller_info}:{time.time()}'.encode(), hashlib.sha256).hexdigest()[:16]
        except Exception:
            pass
        logging.warning(
    ...
    for _ac_key, _ac_val in normalized_updates.items():
        try:
            from ali2026v3_trading.audit_log_utils import structured_audit_log as _sal  # R27-CP-05-FIX: 从独立模块导入
...
def _resolve_runtime_env_name() -> str:
    # R17-P1-CFG-P1-08~14: 环境解析已部分修复(含环境选择器/环境覆盖/配置优先级/热更新/参数版本追踪/schema验证/环境变量文档)
...
def resolve_config_with_priority(
    ...
    from ali2026v3_trading.config_resolver import resolve_config_with_priority as _rcwp_delegated
    return _rcwp_delegated(base_params, env_prefix, DEFAULT_PARAM_TABLE, load_default_params_from_json)
...
def _data_quality_score(tick_count: int, missing_pct: float, outlier_pct: float,
    ...
    from ali2026v3_trading.config_resolver import _data_quality_score as _dqs_delegated
    return _dqs_delegated(tick_count, missing_pct, outlier_pct, ohlc_violation_pct, staleness_sec)
...
from ali2026v3_trading.resilience_config import _auto_recovery_decision_tree
from ali2026v3_trading.security_config import check_key_lifecycle, SecurityEventResponder, get_security_responder
...
# Phase3-Sprint7: 向后兼容重导出 — security_config和resilience_config中的函数仍可从config_params导入
from ali2026v3_trading.security_config import (
    _SecureCredential as _SecureCredential,
    reveal_credential as reveal_credential,
    _sanitize_for_return as _sanitize_for_return,
    get_sensitive_credential as get_sensitive_credential,
    _validate_path_safety as _validate_path_safety,
    _check_config_file_permissions as _check_config_file_permissions,
    _filter_sensitive_keys as _filter_sensitive_keys,
    check_key_lifecycle as check_key_lifecycle,
    SecurityEventResponder as SecurityEventResponder,
    get_security_responder as get_security_responder,
)
...
from ali2026v3_trading.resilience_config import (
    _auto_recovery_decision_tree as _auto_recovery_decision_tree,
)

# Phase3-Sprint8: ParamTableProvider re-export — 通过provider访问参数表的规范入口
