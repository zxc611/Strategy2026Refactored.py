...
class OrderService(BaseService):
    def __init__(self, event_bus=None, params=None):  # R27-P0-FIX: 添加params参数
        ...
        self._state_manager = OrderStateManager()
            ...
            with self._lock:
                ...
                if order:
                    ...
                    if sts in ('FILLED', 'ALL_FILLED', '全成') or order['status'] == 'FILLED':
                        ...
                        self._state_manager.remove_pending(oid)
    ...
    def scan_order_timeouts(self) -> int:
        ...
        Returns:
            处理的超时订单数量
        """
        _before = self._state_manager.pending_count
        self.check_pending_orders()
        _after = self._state_manager.pending_count
        ...
        with self._lock:
            ...
            for order_id, order in list(self._orders_by_id.items()):
                if order['status'] not in ('SUBMITTED', 'PENDING'):
                    self._state_manager.remove_pending(order_id)
                    ...
                    continue
                if not self._state_manager.get_pending(order_id):
                    self._state_manager.add_pending(order_id, order)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                **self._stats,
                'active_orders': self._state_manager.pending_count,
...
_OPEN_REASON_CODES = frozenset({
    'BOX_SPRING', 'CORRECT_RESONANCE', 'CORRECT_DIVERGENCE',
    'SHADOW_A_REVERSAL', 'SHADOW_B_RANDOM', 'OTHER_SCALP',
    'BOX_EXTREME', 'ARBITRAGE', 'HFT_TICK_CONFIRM',
})
