...
from ali2026v3_trading.strategy_config_layer import StrategyConfigLayer
from ali2026v3_trading.strategy_business_layer import StrategyBusinessLayer
from ali2026v3_trading.strategy_monitoring_layer import StrategyMonitoringLayer
...
from ali2026v3_trading.shared_trading_constants import P0_IRON_RULES as _P0_RULES  # R27-CP-01-FIX
...
# - risk_service (get_risk_service, get_safety_meta_layer, RiskService等) — 服务懒初始化
# - position_service (get_position_service) — 服务懒初始化
# - order_service (get_order_service) — 服务懒初始化
# - strategy_ecosystem/shadow_strategy_engine/box_spring_strategy — 服务懒初始化
...
class StrategyCoreService(_InstrumentHelperMixin,
    ...
    def __init__(self, strategy_id: str = None, event_bus: Optional[Any] = None):
        ...
        try:
            ...
            if PhaseFeatureFlag.is_enabled('USE_BUILDER_INIT'):
                from ali2026v3_trading.strategy_core_service_builder import StrategyCoreServiceBuilder
                _builder = StrategyCoreServiceBuilder()
                _builder.set_config(getattr(self, '_params', {}))
                _builder.set_event_bus(event_bus)
                _builder.build_state_store().build_managers().build_thread_pools().build_platform_apis().build_monitoring()
        except Exception:
            pass
    ...
    def _init_locks(self):
        ...
        self._config_layer = StrategyConfigLayer(provider=self)
        self._business_layer = StrategyBusinessLayer(provider=self)
        self._monitoring_layer = StrategyMonitoringLayer(provider=self)
        ...
        StrategyCheckpointMixin._validate_mixin_attributes(self)
    ...
    def _init_logging(self, params: Optional[Dict[str, Any]]) -> None:
        _LifecycleMixin._init_logging(self, params)
        self._config_layer.init_logging()

    def _init_scheduler(self) -> None:
        _LifecycleMixin._init_scheduler(self)
        self._config_layer.init_scheduler()

    def _stop_scheduler(self) -> None:
        _LifecycleMixin._stop_scheduler(self)
        self._config_layer.stop_scheduler()
    ...
    # R27: Recovery methods delegated to StrategyRecoveryMixin
    def _auto_recovery_flow(self) -> bool:
        return StrategyRecoveryMixin._auto_recovery_flow(self)

    def _watchdog_restart(self, max_restarts=3, cooldown_sec=60):
        return StrategyRecoveryMixin._watchdog_restart(self, max_restarts, cooldown_sec)

    def recover_from_checkpoint(self) -> Dict[str, Any]:
        return StrategyRecoveryMixin.recover_from_checkpoint(self)

    # R27: Checkpoint methods delegated to StrategyCheckpointMixin
    def save_checkpoint(self, reason: str = 'manual') -> Dict[str, Any]:
        return StrategyCheckpointMixin.save_checkpoint(self, reason)

    def _on_system_event_checkpoint(self, event):
        return StrategyCheckpointMixin._on_system_event_checkpoint(self, event)

    def _sync_state_to_store(self) -> None:
        return StrategyCheckpointMixin._sync_state_to_store(self)
    ...
    def _ensure_order_service(self) -> None:
        ...
        with self._order_service_init_lock:
            ...
            try:
                ...
                self._order_service = None
        try:
            self._business_layer.ensure_order_service()
        except Exception:
            pass
    ...
    def _ensure_position_service(self) -> None:
        ...
        try:
            ...
            self._position_service = None
        try:
            self._business_layer.ensure_position_service()
        except Exception:
            pass
    ...
    def _ensure_hft_engine(self) -> None:
        ...
        try:
            ...
            self._hft_engine = None
        try:
            self._business_layer.ensure_hft_engine()
        except Exception:
            pass
    ...
    def _check_ecosystem_exclusion(self, targets: list) -> bool:
        try:
            ...
            # 委托业务层检查
            try:
                self._business_layer.check_ecosystem_exclusion(strategy_id)
            except Exception:
                pass
            return True
        except Exception as e:
    ...
    def _resolve_open_reason(self) -> str:
        return self._config_layer.resolve_open_reason()
    ...
    def get_health_status(self) -> Dict[str, Any]:
        return self._monitoring_layer.get_health_status()

    def confirm_daily_resume(self, caller_id: str = "unknown") -> bool:
        return self._monitoring_layer.confirm_daily_resume(caller_id=caller_id)

    # P0-R8-04修复: 检查硬停止状态
    def is_hard_stop_triggered(self) -> bool:
        return self._monitoring_layer.is_hard_stop_triggered()
