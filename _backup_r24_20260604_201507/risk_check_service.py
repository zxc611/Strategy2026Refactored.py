...
class RiskCheckService:
    ...
    def _rule_engine(self) -> RiskCheckEngine:
        """懒加载规则引擎实例 — 委托给RiskCheckEngine执行已注册规则"""
    ...
    def check_before_trade(self, signal: Dict[str, Any]):
        ...
        # --- 规则引擎委托: 优先使用RiskCheckEngine执行已注册规则 ---
        ...
        # --- 规则引擎委托结束 ---
        # LEGACY: Retained for backward compatibility, superseded by RiskCheckEngine
        RiskCheckResponse, RiskCheckResult, RiskLevel, AlertLevel = self._get_types()
    ...
    def _check_safety_meta_layer(self, signal: Dict[str, Any]):
        # LEGACY: Retained for backward compatibility, superseded by RiskCheckEngine (DailyDrawdownRule)
        RiskCheckResponse, RiskCheckResult, RiskLevel, AlertLevel = self._get_types()
        try:
    ...
    def _check_rate_limit(self, symbol: str):
        # LEGACY: Retained for backward compatibility, superseded by RiskCheckEngine (SignalCooldownRule)
        RiskCheckResponse, RiskCheckResult, RiskLevel, AlertLevel = self._get_types()
    ...
    def _check_position_limit(self, account_id: str, required_amount: float,
        ...
        # LEGACY: Retained for backward compatibility, superseded by RiskCheckEngine (PositionLimitRule)
        RiskCheckResponse, RiskCheckResult, RiskLevel, AlertLevel = self._get_types()
    ...
    def _check_capital_sufficiency_in_trade(self, signal: Dict[str, Any]):
        # LEGACY: Retained for backward compatibility, superseded by RiskCheckEngine (MarginSufficiencyRule)
        RiskCheckResponse, RiskCheckResult, RiskLevel, AlertLevel = self._get_types()
        try:
            equity = 0.0
            try:
    ...
    def _check_greeks_limits(self, signal: Dict[str, Any]):
        ...
        try:
            calc, positions_dict = self._compute_greeks_exposure(signal)
            if calc is None:
                return RiskCheckResponse.block_result(
            ...
            if not positions_dict:
                return RiskCheckResponse.pass_result()
            return self._validate_greeks_thresholds(signal, calc, positions_dict)
    ...
    def check_expiry_risk(self, instrument_id: str, days_to_expiry: int = None):
        # LEGACY: Retained for backward compatibility, superseded by RiskCheckEngine (NearExpiryRule)
        RiskCheckResponse, RiskCheckResult, RiskLevel, AlertLevel = self._get_types()
