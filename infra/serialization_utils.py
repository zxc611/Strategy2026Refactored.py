...
# MODULE_ID: M1-107
"""
统一序列化工�?- 修复SER-01/SER-P1-01/SER-P1-03

解决审计报告中的序列化问题：
- SER-01: JSON default=str导致datetime不可逆丢�?
- SER-P1-01: WAL日志双重default=str导致时间戳冗余丢�?
- SER-P1-03: NaN/Infinity在JSON序列化中无防�?
...
def json_object_hook(dct: Dict[str, Any]) -> Any:  # [R22-P2-TS26]
    """
    JSON反序列化�?- 修复SER-01/SER-P1-01/SER-P1-03
    ...
    - {"__special__": "NaN/Infinity/-Infinity"} �?numpy�?
...
def json_dumps(obj: Any, indent: Optional[int] = None, sort_keys: bool = True, ensure_ascii: bool = False, **kwargs) -> str:
    """
    统一JSON序列化接�?- 替换json.dumps(..., default=str)

    使用方式�?
    >>> json_str = json_dumps(data)           # 紧凑输出，适用于JSONL日志
    >>> json_str = json_dumps(data, indent=2) # 缩进输出，适用于配置文�?
...
def safe_json_dumps(obj: Any, fallback: str = "null", **kwargs) -> str:
    """
    安全JSON序列�?- 失败时返回fallback而非抛异�?

    解决SER-P1-02（事件持久化截断可能损坏JSON结构�?

    仅用于日�?非关键路径。关键路径应使用json_dumps（失败时抛TypeError）�?
...
def validate_json_roundtrip(obj: Any) -> tuple[bool, str]:
    ...
    返回�?
    - (True, "") - roundtrip成功
    - (False, error_msg) - roundtrip失败，返回错误信�?
    """
    try:
...
def yaml_safe_dump(data: Any, stream=None, **kwargs):
    """
    统一YAML序列�?- 修复SER-02（YAML dump/safe_load不匹配）
...
def yaml_safe_load(stream):
    ...
    SER-P1-13修复: YAML布尔值守�?�?预处理yes/no/on/off为显式布尔值，
...
def safe_pickle_load(filepath: str) -> Any:
    ...
    class _RestrictedUnpickler(pickle.Unpickler):
        ...
        def find_class(self, module, name):
            ...
            raise pickle.UnpicklingError(f"SER-P1-17: 不允许反序列�?{module}.{name}，疑似RCE攻击")
...
def safe_pickle_dump(obj: Any, filepath: str) -> None:
    ...
    # SER-P1-07修复: 可序列化性预检�?�?序列化后立即反序列化验证，失败则抛出SerializationError
    try:
        with open(filepath, 'rb') as f:
        ...
        import logging as _logging
        _logging.warning(
            "SER-P1-07: pickle roundtrip验证失败(非致�?: %s, 文件已写入但可能不可�?,
            _rt_err,
        )
...
def safe_dataframe_from_parquet(
    ...
    """
    SER-P1-04修复: 安全DataFrame从Parquet加载，恢复时区信�?
    ...
    try:
        ...
        if _metadata and isinstance(_metadata, dict):
            ...
            if file_version and file_version != PARQUET_SCHEMA_VERSION:
                import logging as _logging
                _logging.warning(
                    "P2-12: Parquet schema版本不匹�?file=%s current=%s: %s",
                    filepath, PARQUET_SCHEMA_VERSION, file_version,
                )
                # SER-P1-05修复: Schema evolution �?自动添加缺失�?填充NaN)并删除多余列
                try:
                    ...
                    if _missing_cols:
                        for _col in _missing_cols:
                            df[_col] = np.nan
                        _logging.info(
                            "SER-P1-05: Schema evolution添加缺失�? %s",
                            sorted(_missing_cols),
                        )
                    # 删除多余�?
                    _extra_cols = _actual_cols - _expected_cols
                    if _extra_cols and _expected_cols:  # 仅当有期望列集时才删�?
            ...
            import logging as _logging
            _logging.debug("P2-12: Parquet文件无schema_version元数�? %s", filepath)
...
def validate_duckdb_parquet_chain(
    ...
    """
    SER-P1-11修复: DuckDB类型一致性检�?�?验证DataFrame→Parquet→DuckDB往返类型一致�?
    ...
    try:
        ...
        return False, "SER-P1-11: duckdb未安装，无法执行类型一致性检�?
    ...
    for col in original_dtypes:
        if col not in result_dtypes:
            mismatches.append(f"{col}: 列缺�?原{original_dtypes[col]})")
    ...
    for col in result_dtypes:
        if col not in original_dtypes:
            mismatches.append(f"{col}: 额外�?类型{result_dtypes[col]})")
...
def serialization_perf_monitor(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            _elapsed = _time.perf_counter() - _start
            logging.error("P2-28: 序列化操作失�?%.2fs): %s - %s", _elapsed, func.__name__, e)
            raise
    return wrapper
...
def rotate_jsonl_if_needed(filepath: str, max_bytes: int = 50 * 1024 * 1024,
    ...
    """P2-01修复: JSONL文件轮转公共函数 �?统一3个调用点的重复实�?

    当文件大小超过max_bytes时执行编号轮�?
      - 删除最老备�?.{backup_count}
      - 依次重命�?.{backup_count-1} �?.{backup_count}, ..., .1 �?.2
      - 当前文件重命名为 .1
    使用os.replace()替代os.rename()确保原子�?跨平台安�?
    对ENOSPC(磁盘�?记录CRITICAL日志而非静默吞没

    Args:
        filepath: JSONL文件路径
        max_bytes: 最大文件大�?字节),默认50MB
        backup_count: 保留备份�?默认3
        ...
        if os.path.exists(src):
            try:
                ...
                if _e.errno == errno.ENOSPC:
                    logging.critical("[rotate_jsonl] 磁盘空间不足,无法轮转 %s �?%s", src, dst)
                else:
                    logging.debug("[rotate_jsonl] 重命名失�?%s �?%s: %s", src, dst, _e)
