...
合并自commission_utils.py + cross_system.py + deviation_classifier.py (2026-06-12)
"""
...
class UnifiedPositionSizer:
    max_risk_ratio: float = 0.2  # R26-P2对齐标注: 评判系统保守定位(0.2)，生产默�?.8，属有意差异——评判系统需更严格风�?
...
_EXECUTION_MODEL_DIFFERENCES = {
    "backtest_execution": "bar_close_price",       # 回测: bar收盘�?
    "live_execution": "counterparty_price",        # 实盘: 对手�?
    "backtest_slippage": "fixed_bps_cascade",      # 回测: 固定bps+级联
    "live_slippage": "market_impact_model",        # 实盘: 市场冲击模型
    "backtest_latency": "zero",                    # 回测: 零延�?
...
# with语句保证GeneratorExit异常触发__exit__，自动调用生成器.close()。若不使用with而直接调用perf_monitor()，需手动调用gen.close()确保资源释放�?
...
def get_spawn_context():
    ...
    统一替代 __import__('multiprocessing').get_context('spawn') 的重复代码，l2_optimizer.py / _preprocess.py 均通过此函数获取spawn context�?避免DRY违反并确保context配置一致性�?
    Returns:
...
def classify_deviation(
    ...
    *,
    ...
    Args:
        baseline: 重构前（BEFORE）信�?
        actual: 重构后（AFTER）信�?
        noise_tol: 浮点噪声绝对容差
        acceptable_diff: 可接受价格差异阈�?
    Returns:
    ...
    if baseline is not None and actual is not None:
        ...
        if (baseline.direction == actual.direction
            ...
            return DeviationResult('MATCH', '信号完全一�?, baseline, actual)
        ...
        if price_diff < noise_tol:
            ...
            return DeviationResult(
                'NUMERICAL_NOISE',
                f'微小价格差异(可接�? price_diff={price_diff:.4f}',
                baseline, actual,
            )
        else:
            return DeviationResult(
    ...
    if baseline is not None and actual is None:
        return DeviationResult(
            'CORRECT_DEVIATION',
            f'未来函数消除导致的信号消�?方向={baseline.direction})',
            baseline, actual,
        )

    # baseline无信号，actual为新增信�?
...
def classify_signal_sequence(
    ...
    *,
    ...
    Args:
        baseline_signals: 重构前信号序�?
        actual_signals: 重构后信号序�?
        noise_tol: 浮点噪声容差
        acceptable_diff: 可接受价格差�?
    Returns:
        报告字典，包�?
        - total: 总对比笔�?
        - category_counts: 各分类计�?       - critical_deviations: CRITICAL_DEVIATION详情列表
