...
def _acquire_file_lock(f):
    ...
    try:
        ...
        logger.warning("[EC-P1-05] 文件锁获取失�? %s，继续执行但跨进程安全无保障", e)
...
def _release_file_lock(f):
    ...
    try:
        ...
        logger.warning("[EC-P1-05] 文件锁释放失�? %s", e)


def classify_registered_instruments(storage) -> Tuple[List[str], Dict[int, List[str]]]:
    """�?underlying_future_id 对已注册合约做轻量分组，避免初始化阶段逐合约阻塞等待�?""
    ...
    logger.debug("[InitServices] 已注册合约数�? %d", len(registered_ids))
        ...
        if SubscriptionInstrumentService.is_option(inst_id):
            try:
                ...
                if not underlying_future_id:
                    logger.debug("[InitServices] 跳过缺少 underlying_future_id 的期�? %s", inst_id)
                    continue
...
class SubscriptionConfig:
    """订阅管理器配�?- 借鉴 OrderFlowConfig 设计"""
    
    # 重试策略
    max_retries: int = 3                          # 最大重试次�?
    retry_base_delay: float = 1.0                 # 基础延迟(�?
    retry_max_delay: float = 60.0                 # 最大延�?�?
    backoff_strategy: str = "exponential"         # 退避策�? exponential/linear/random_jitter
    
    # 批量写入
    db_batch_size: int = 100                      # DuckDB批量大小
    throttle_window: float = 1.0                  # 节流窗口(�?
    ...
    # 重试队列
    retry_queue_max_size: int = 500               # 重试队列最大容�?
    retry_interval: float = 5.0                   # 重试线程间隔(�?
    max_event_age_seconds: int = 1800             # 事件最大存活时�?30分钟)
    
    # 清理线程配置 (借鉴 OrderFlowAnalyzer)
    enable_cleanup_thread: bool = True            # 启用独立清理线程
    cleanup_interval: float = 60.0                # 清理检查间�?�?
    
    # R14-P1-LOG-13修复: alert_callback默认非None(空日志回�?
    alert_callback: Optional[Callable[[int, str], None]] = None  # 初始化时设为_default_alert_callback
    alert_threshold: int = 10                     # 累计失败超过此值触发告�?
    failure_rate_threshold: float = 0.1           # 失败率超�?0%触发告警
    
    # 监控
    enable_stats: bool = True                     # 启用统计信息
    stats_report_interval: float = 60.0           # 统计报告间隔(�?



# ========== 企业级订阅管理器 ==========

"""订阅管理�?- 合约解析 (_SubscriptionInstrumentMixin)"""
...
class SubscriptionInstrumentService:
    ...
    def parse_option(instrument_id: str) -> Dict[str, Any]:
        r"""
        解析期权合约(单一正则同时匹配连字符和紧凑格式, 品种ID直�?
        
        修复标准#125: 一种正则解析所有格�? 原样ID直通不改变格式.
        统一正则 r'^([A-Za-z]+)(\d{3,4})-?([CP])-?(\d+(?:\.\d+)?)$'
        同时匹配连字符格�?CU2603-C-5000)和紧凑格�?CU2603C5000).
        """
        clean_id = SubscriptionInstrumentService._strip_exchange_prefix(instrument_id)
        
        # 单一正则: 同时匹配连字符格式和紧凑格式
        # - 连字�? CU2603-C-5000 (-分隔, 行权价支持小�?
        # - 紧凑: CU2603C5000 (无分�? 行权价仅整数)
        # -? 使连字符可�? \d{3,4} 覆盖两种年月位数
        match = re.match(r'^([A-Za-z]+)(\d{3,4})-?([CP])-?(\d+(?:\.\d+)?)$', clean_id)
        if match:
            product = match.group(1)  # 直�? 保持原始大小�?
            ...
            # 格式标记: 通过原始ID中是否含-判断(不改变ID, 仅标记来源格�?
    ...
    # �?接口唯一：classify_instruments唯一实现，query_service两处已委托此�?
    @staticmethod
    def classify_instruments(instrument_ids: List[str]) -> Tuple[List[str], Dict[str, List[str]]]:
        """分类合约
        
        options_dict �?key 统一�?canonical underlying 标识 (product+year_month)�?
        �?'IO2605', 'al2605'，与 ParamsService._extract_canonical_underlying 输出一致�?
        """
        futures_list = []
        options_dict = {}
...
"""订阅管理�?- WAL管理 (_SubscriptionWALMixin)"""
...
class SubscriptionWALService:
    ...
    def _wal_heartbeat(self):
        """R15-P0-RES-10修复: 定期写入心跳记录，重启时检测异常退�?""
        if not self._config.enable_wal:
            return
        try:
    ...
    def _check_last_heartbeat(self):
        """R15-P0-RES-10修复: 重启时检测最后心跳时间判断异常退�?""
        ...
        try:
            ...
            if last_heartbeat_ts is not None:
                ...
                if gap > 120:
                    logger.warning("[SubscriptionManagerV2] R15-P0-RES-10: 检测到异常退�?心跳间隔%.0f�?120)", gap)
                    return True
            return False
    ...
    def _write_wal_async(self, record: Dict[str, Any]):  # [R22-P2-TS27]
        ...
        with self._wal_lock:
            try:
                # 直接写入内存队列,由后台线程批量落�?
                # R13-P2-LOG-08修复: 已在__init__中显式初始化_wal_queue，无需hasattr检�?
        ...
        try:
            ...
            with open(self._config.wal_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if not line:
                        continue
                    # R15-P1-DATA-07修复: WAL读取时验证CRC32校验�?
                    _crc_verify_ok = True
                    if '\t#crc:' in line:
    ...
    def stop_background_threads(self, join_timeout: float = 2.0, silent: bool = False) -> None:
        ...
        self._stop_watchdog.set()  # R27-P0-DR-05修复: 停止看门�?
    ...
    # R27-P0-DR-05修复: tick看门狗循�?�?检测tick断流并告�?
            ...
            try:
                ...
                for inst_id, stale_sec in stale_instruments:
                    if inst_id not in self._tick_stale_instruments:
                        ...
                        logger.warning(
                            "[R27-P0-DR-05] Tick断流告警: instrument=%s �?.1f秒无tick(阈�?%.1fs)",
                ...
                logger.error("[R27-P0-DR-05] 看门狗循环异�? %s", e)
    
    def _atexit_cleanup(self):
        """进程退出清�?- 静默模式避免测试退出时I/O错误"""
        try:
    ...
    def _enqueue_for_retry(self, task_data: Dict[str, Any], retry_count: int = 0):  # [R22-P2-TS28]
        ...
        # 兼容 facade 模式: �?facade 上被显式覆盖 _retry_queue，则使用 facade 的实�?
        _facade = self.__dict__.get('_facade')
    ...
    def _process_retry_queue(self):
        ...
        with self._retry_lock:
            # still_pending是元组列�? (task, count, next_retry_time, enq_time)
            # 按next_retry_time（索�?）排序，最早重试的排前�?
    ...
    def _check_alert(self):
        """检查是否触发告警（�?P1修复：加锁保护共享计数器�? # R17-P2-DOC-05"""
...
"""订阅管理�?- 核心订阅逻辑 (_SubscriptionCoreMixin)"""
...
class SubscriptionCoreService:
    ...
    def _do_subscribe(self, instrument_id: str, data_type: str) -> None:
        ...
        """
        # R13-P1-API-02修复: data_manager为None时入队等待，避免bind_platform_apis()前崩�?
        if self.data_manager is None:
            ...
            self._pending_subscriptions.append({'instrument_id': instrument_id, 'data_type': data_type})
            return
    ...
    def _ensure_instruments_loaded(self) -> bool:
        ...
        ⚠️ 关键设计�?
        - 合约元数据必须在 Storage 初始化前从配置文件完整加�?
        - 这个检查只验证配置是否已加载，不负责实际加�?
        - 实际加载�?strategy_core_service.on_init() 调用 ensure_products_with_retry 完成
    ...
    def subscribe_all_instruments(self, futures_list: List[str], 
        ...
        _max_instruments = CAPACITY_LIMITS.get('max_instruments', 500)
        _total_instruments = len(futures_list) + sum(len(opts) for opts in options_dict.values())
        if _total_instruments >= _max_instruments:
            logging.warning("[RES-P2-09] 合约订阅已达上限: %d/%d", _total_instruments, _max_instruments)
            return False
    ...
    def on_tick(self, instrument_id: str, last_price: float, volume: float = 0,
        ...
        """Tick 路由入口：先�?metadata 决定期权/期货，再转发�?TTypeService�?

        改造后不再每笔 Tick �?parse_option/parse_future 再路由，
        而是先查 instrument_id -> internal_id -> meta�?
        �?metadata 中的 type 字段决定走哪条路径�?
        ...
        if is_replay:
            ...
            return
        # R24-P0-IV-02修复: volume NaN/Inf/负值过�?
        import math as _math
        if not isinstance(volume, (int, float)) or _math.isnan(volume) or _math.isinf(volume) or volume < 0:
            volume = 0

        # R25-P2-IV-ext修复: last_price NaN/Inf/负值过滤（防止绕过_process_tick直接调用时源头污染）
        ...
        # R15-P0-PERF-01修复: 使用分片锁替代全局_tick_lock
        with self._get_shard_lock(instrument_id):
            # R15-P0-DATA-03修复: tick序列号去重，基于(instrument_id, volume)组合�?
            tick_seq_key = (instrument_id, volume)
            last_seq = self._last_tick_seq.get(instrument_id, -1)
            current_seq = int(volume) if volume is not None else 0
            # R23-ID-06-FIX: 换日volume归零误判保护
            # 当current_seq远小于last_seq时，判定为换日重置而非重复tick
            # 条件1: last_seq>1000且current_seq<last_seq*0.1 (高流动性品种换�?
            # 条件2: last_seq>0且current_seq==0 (volume归零，任何品种换�?
            if current_seq <= last_seq and last_seq >= 0:
                _is_day_reset = (last_seq > 1000 and current_seq < last_seq * 0.1) or (last_seq > 0 and current_seq == 0)
                if _is_day_reset:
                    logger.info("[R23-ID-06-FIX] 换日volume重置检�? inst=%s last_seq=%d current_seq=%d, 视为新日首tick",
                               instrument_id, last_seq, current_seq)
                    self._last_tick_seq[instrument_id] = current_seq
                else:
                    self._tick_dedup_drop_count += 1
                    if self._tick_dedup_drop_count % 1000 == 1:
                        logger.warning("[R15-P0-DATA-03] tick去重丢弃: instrument_id=%s seq=%d last_seq=%d total_dropped=%d",
                                       instrument_id, current_seq, last_seq, self._tick_dedup_drop_count)
                    return
            self._last_tick_seq[instrument_id] = current_seq
        ...
        try:
            ...
            try:
                ...
                if meta:
                    ...
                    # �?P0修复：原子引用t_type_service，避免并发替换导致None引用  # R17-P2-DOC-03: P0修复标记保留用于追溯
                    ...
                    if inst_type == 'option' and tts:
                        ...
                        # �?两ID原则：传�?internal_id 而非 instrument_id
    ...
    def log_subscription_success_summary(self) -> None:
        ...
        if stats['tick_missing_count'] > 0 and stats['tick_missing']:
            logger.warning("[Tick未收到合约] �?0�?")
            for inst in stats['tick_missing'][:20]:
                logger.warning("  %s", inst)  # R13-P2-LOG-01修复
            if stats['tick_missing_count'] > 20:
                logger.warning("  ... 还有 %d �?, stats['tick_missing_count'] - 20)  # R13-P2-LOG-01修复
