...
合并�?_ops_framework.py + _ops_knowledge_metrics.py + _ops_automation.py (2026-06-12)
"""
...
from ali2026v3_trading.infra._disk_monitor import get_disk_space_monitor
...
class OpsOperation:
    ...
    - dependency_check: 依赖检查函�?(OPS-P1-17)
    - timeout: 超时时间 (OPS-P1-13)
    - max_retries: 最大重试次�?(OPS-P1-14)
    """
...
class OpsOperationManager:
    ...
    - OPS-P1-08: 影响评估 �?操作前评估影响范�?
    - OPS-P1-09: 预演机制 �?dry_run模式执行
    - OPS-P1-10: 结果验证 �?操作后验证预期结�?
    - OPS-P1-11: 回滚方案 �?操作失败自动回滚
    - OPS-P1-12: 通知机制 �?操作开�?完成/失败通知
    - OPS-P1-13: 超时控制 �?操作执行超时自动终止
    - OPS-P1-14: 重试机制 �?可配置重试次数和间隔
    - OPS-P1-15: 幂等保证 �?操作ID去重
    - OPS-P1-16: 并发控制 �?操作�?
    - OPS-P1-17: 依赖检�?�?前置依赖验证
    - OPS-P1-18: 前置条件验证 �?pre_condition检�?
    - OPS-P1-19: 后置条件验证 �?post_condition检�?

    执行流程: pre_check -> dry_run(可�? -> execute -> post_check
    失败�? rollback -> 通知
    所有操作通过EventBus发布事件
    """
    ...
    def __init__(self):
        ...
        # OPS-P1-16: 并发控制 �?操作类型�?{operation_type: Lock}
        self._type_locks: Dict[str, threading.Lock] = {}
        # OPS-P1-07: 回退方案 �?操作前状态快�?{operation_id: snapshot}
    ...
    def execute_operation(self, operation: OpsOperation,
        ...
        with self._lock:
            if operation.operation_id in self._completed_ops:
                ...
                result['success'] = True  # 幂等：已成功过视为成�?
            ...
            if operation.dependency_check:
                try:
                    if not operation.dependency_check():
                        result['error'] = '依赖检查失�?
                        self._publish_event(operation, 'started', 'dependency_failed', '依赖检查失�?)
                        return result
                except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as e:
                    result['error'] = f'依赖检查异�? {e}'
    ...
    def _execute_with_retry_and_timeout(self, operation: OpsOperation) -> Dict[str, Any]:
        ...
        P2-12修复: 委托�?TimeoutGuard + BoundedRetry�?
        语义: 超时不重�?直接返回失败), 异常才重试�?
        """
    ...
    def _publish_event(self, operation: OpsOperation, phase: str,
        ...
        try:
            ...
            logger.debug("[OpsOperationManager] OPS-P1-12: EventBus推送异�? %s", e)
...
# P2修复: 运维就绪�?�?知识库、指标定义、SLA

# P2修复: 运维操作指南注释
# 运维操作指南:
# 1. 启动检�? 调用 StorageMaintenanceService.run_startup_checks() 确认数据完整�?
# 2. 日常巡检: 调用 StorageMaintenanceService.run_daily_maintenance() 执行日常维护
# 3. 磁盘监控: 调用 get_disk_space_monitor().check() 检查磁盘空�?
...
# 7. 健康检�? 调用 StorageMaintenanceService.get_health_status() 获取健康状�?
# 8. 紧急停�? 设置 strategy.my_is_running = False 停止策略
# 9. 日志排查: 检�?logs/strategy.log 中的ERROR级别日志
# 10. 版本确认: 调用 ConfigService.check_version_alignment() 确认版本一�?

# P2修复: 运维知识库数据结�?
_OPS_KNOWLEDGE_BASE: Dict[str, Dict[str, Any]] = {
    'startup_failure': {
        'symptoms': ['策略无法启动', '初始化超�?, '数据库连接失�?],
        'causes': ['数据库文件损�?, '配置文件缺失', '端口被占�?],
        'solutions': ['检查数据库文件完整�?, '恢复默认配置', '释放端口占用'],
        'severity': 'HIGH',
    },
    'high_memory_usage': {
        'symptoms': ['内存使用�?80%', '系统响应变慢', 'OOM告警'],
        'causes': ['数据缓存过大', '内存泄漏', '并发请求过多'],
        'solutions': ['清理数据缓存', '重启策略进程', '降低并发�?],
        'severity': 'MEDIUM',
    },
    'disk_space_low': {
        'symptoms': ['磁盘空间不足告警', '写入失败', '日志无法写入'],
        'causes': ['日志文件过大', '备份数据过多', '临时文件未清�?],
        'solutions': ['清理旧日�?, '删除过期备份', '清理临时文件'],
        'severity': 'HIGH',
    },
    'data_inconsistency': {
        'symptoms': ['持仓数据与实际不�?, '信号计算异常', '诊断报告异常'],
        'causes': ['数据写入中断', '并发写入冲突', 'Schema版本不匹�?],
        'solutions': ['运行数据自愈', '恢复备份', '升级Schema版本'],
        'severity': 'CRITICAL',
    },
    'signal_loss': {
        'symptoms': ['信号未触�?, '交易信号丢失', '过滤统计异常'],
        'causes': ['EventBus订阅失败', '回调异常', '速率限制触发'],
        'solutions': ['检查EventBus订阅状�?, '查看错误日志', '调整速率限制'],
        'severity': 'HIGH',
    },
    'circuit_breaker_triggered': {
        'symptoms': ['交易暂停', '断路器告�?, '策略降级运行'],
        'causes': ['回撤超限', '波动率异�?, '连续亏损'],
        'solutions': ['检查回撤指�?, '评估市场状况', '调整断路器参�?],
        'severity': 'CRITICAL',
    },
}
...
class OpsSLA:
    ...
    AVAILABILITY_MAX_DOWNTIME_MIN_PER_MONTH = 43.8  # 每月最大停机时�?分钟)
    ...
    # 监控SLA
    HEALTH_CHECK_INTERVAL_SEC = 60           # 健康检查间�?0�?
    ALERT_NOTIFICATION_DELAY_SEC = 30        # 告警通知延迟<30�?

    # 运维操作SLA
    PARAM_CHANGE_AUDIT_RETENTION_DAYS = 90   # 参数变更审计保留90�?
    OPS_LOG_RETENTION_DAYS = 30              # 运维日志保留30�?
    BACKUP_RETENTION_DAYS = 30               # 备份保留30�?
    ...
    if issue_type == 'disk_full':
        try:
            ...
            if os.path.exists(log_dir):
                ...
                result['details'] = f'清理了{cleaned}个超�?0天的日志文件'
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, IOError) as e:
        ...
        try:
            ...
            if pending > 100:
                ...
                result['details'] = 'EventBus状态正�?
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, ImportError) as e:
            result['details'] = f'检查失�? {e}'
        ...
        try:
            ...
            if query_facade.need_reload():
                ...
                result['details'] = '配置已重新加�?
            else:
                result['repaired'] = True
                result['action'] = 'config_ok'
                result['details'] = '配置已是最�?
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, ImportError) as e:
    ...
    def _init_default_procedures(self) -> None:
        self._procedures = {
            ...
            'strategy_upgrade': {
                'name': '策略升级',
                'steps': [
                    '1. 启用并行运行�?strategy_ecosystem.enable_parallel_run())',
                    '2. 部署新版本策�?,
