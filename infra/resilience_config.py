...
def check_multi_level_cache_consistency() -> bool:
    try:
        from ali2026v3_trading.config.config_params import _param_table_cache, DEFAULT_PARAM_TABLE, _param_table_lock
...
def check_indicator_freshness(max_age_seconds: float = 300.0) -> bool:
    try:
        from ali2026v3_trading.config.config_params import _indicator_timestamps
        import time
        now = time.time()
...
def check_risk_data_freshness(max_age_seconds: float = 60.0) -> bool:
    try:
        from ali2026v3_trading.config.config_params import _risk_data_timestamp
        import time
