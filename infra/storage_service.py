...
合并�?_helpers.py + _disk_monitor.py + _backup_restore.py + _storage.py (2026-06-12)
"""
...
class DuckDBBackupService:
    ...
    DR-P1-01: 每小时备份一次，保留最�?4个备�?
    DR-P1-03: 生成backup_manifest.json记录备份完整�?
    """

    BACKUP_INTERVAL_SEC = 3600.0  # 1小时
    MAX_BACKUPS = 24
    REMOTE_BACKUP_ENABLED = False  # DR-P1-03: 异地存储标记，需手动配置后启�?
    ...
    def _record_backup_manifest(self, backup_path: str) -> None:
        ...
        logger.info("[DR-P1-03] 备份清单已更�? %s (size=%d, hash=%s...)", backup_path, file_size, file_hash[:16])

    def verify_backup_integrity(self, backup_file: str) -> bool:
        """DR-P1-03: 验证备份文件完整�?""
        if not os.path.exists(backup_file):
            logger.warning("[DR-P1-03] 备份文件不存�? %s", backup_file)
            return False
        ...
        try:
            ...
            if actual_hash != expected_hash:
                logger.error("[DR-P1-03] 备份完整性校验失�? %s", backup_file)
                return False
            logger.info("[DR-P1-03] 备份完整性校验通过: %s", backup_file)
            return True
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, IOError) as e:
            logger.error("[DR-P1-03] 验证备份完整性异�? %s", e)
            return False
    ...
    def _rotate_backups(self) -> None:
        ...
        try:
            ...
            while len(backup_files) > self.MAX_BACKUPS:
                ...
                os.remove(oldest)
                logger.info("[DR-P1-01] 删除旧备�? %s", oldest)
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, IOError) as e:
...
class StorageCatalogService:
    ...
    def backfill_metadata_exchange(self) -> int:
        ...
        P2 Bug #111修复：明确返回值含义为"待更新行�?而非"已更新行�?
    ...
    def repair_option_underlying_product_references(self) -> int:
        ...
        P2 Bug #112修复：明确返回值含义为"待更新行�?而非"已更新行�?
        DuckDB不支持changes()，因此先COUNT待更新行数，再执行UPDATE
        """
        if not _HAS_DATA_SERVICE:
            return 0
    ...
    def ensure_option_product_catalog(self) -> int:
        """补齐并激�?option_products 中缺失的真实期权品种配置�?""
        ...
        try:
            ...
            logging.warning("[StorageMaintenance] 加载 ExchangeConfig 失败，跳�?option_products 补齐: %s", exc)
            return 0
    ...
    def drop_empty_instrument_tables(self) -> int:
        ...
        try:
            ...
            for internal_id in empty_ids:
                ...
                # subscriptions 表已废弃，DELETE 操作已移�?
                deleted += 1
    ...
    def _diagnose_contract(self, instrument_id: str, is_future: bool) -> Dict:
        ...
        try:
            if self.manager is not None and hasattr(self.manager, '_get_instrument_info'):
                ...
                result['subscribe_error'] = f'manager 不支�?_get_instrument_info'
                return result

            if not info:
                result['subscribe_error'] = f'合约 {instrument_id} 不存�?
                return result
        ...
        try:
            ...
            if backup_path:
                ...
                try:
                    ...
                    pass
                logger.info(
                    "[OPS-09] 数据库备份完�? %s (%.2fMB) caller=%s",
                    backup_path, result['file_size_mb'], caller_id,
                )
            else:
                result['error'] = '备份被跳过（间隔未到或数据库文件不存在）'
                logger.info("[OPS-09] 数据库备份跳�? 间隔未到 caller=%s", caller_id)
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, IOError) as e:
            result['error'] = str(e)
            logger.error("[OPS-09] 数据库备份失�? %s caller=%s", e, caller_id)
        return result
    ...
    def archive_old_data(self, table_name: str,
        ...
        try:
            ...
            try:
                ...
                if dry_run:
                    result['success'] = True
                    result['message'] = f'试运�? 将归档{rows_to_archive}�?
                    return result
...
class StorageChecksService:
    ...
    DIAGNOSTIC_CONTRACTS_2605 = [
        ...
        {'exchange': 'SHFE', 'future': 'CU2605', 'option': 'cu2605C100000'},  # SHFE 铜期�?(小写 +C+ 行权�?
        ...
        {'exchange': 'INE', 'future': 'LU2605', 'option': 'sc2605C660'},  # INE 原油期权 (小写 +C+ 行权�?
        {'exchange': 'GFEX', 'future': 'LC2605', 'option': 'lc2605-C-16000'},  # GFEX 工业硅期�?
    ]
    ...
    def run_startup_checks_fast_path(self, conn=None) -> None:
        ...
        正常重启时，如果 maintenance version 已是最新，只执行轻量健康检�?
        （序列同�?+ 缺表修复），跳过重维护逻辑�?
        只有 maintenance version 变化时，才执行全�?startup checks�?

        设计约束（修改必看十原则）：
        1. fast path 必须包含序列同步（轻量但关键，防�?ID 冲突�?
        2. fast path 必须包含缺表修复（轻量但关键，防止表缺失�?
        3. 重维护逻辑（orphan 恢复、FK 修复、订阅去重等）只�?version 变化时执�?
        4. 新增迁移脚本、缺表修复、索引补建时，必须声明是否允许在启动热路径执�?
        """
        try:
            logging.info("[StorageMaintenance] fast path 启动检查开�?..")

            if not _HAS_DATA_SERVICE:
                logging.warning("[StorageMaintenance] DataService 不可用，跳过维护检�?)
                return
            ...
            # DR-P2修复: 数据完整性抽样（检查最�?00条tick记录可读性）
            ...
            try:
                    ...
                    if tables_result:
                        for (tbl,) in tables_result:
                            try:
                                ...
                                continue
                    if sampled == 0:
                        logging.warning("[StorageMaintenance] DR-P2: 数据抽样失败(无可读tick记录)，需降级到完整检�?)
                ...
                logging.warning("[StorageMaintenance] DR-P2: 数据抽样异常(%s)，降级到完整检�?, samp_err)
                self.run_startup_checks(conn)
                return

            # 检�?maintenance version：匹配则跳过重维�?
            ...
            logging.info(
                "[StorageMaintenance] fast path: maintenance version 不匹�?(current=%s, expected=%s)，执行全量检�?,
                current, self.MAINTENANCE_VERSION,
            )
            self.run_startup_checks(conn)
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as exc:
            logging.warning("[StorageMaintenance] fast path 失败，降级到全量检�? %s", exc)
            try:
                self.run_startup_checks(conn)
    ...
    def run_startup_checks(self, conn=None) -> None:
        ...
        """
        try:
            logging.info("[StorageMaintenance] 开始启动检�?..")
    ...
    def run_periodic_reconciliation(self, position_service=None, exchange_query_fn: Optional[Callable] = None) -> Dict[str, Any]:
        ...
        for _feat in self._degradation_features:
            logging.info("[RES-P2-04] 降级特性已激�? %s", _feat)
        try:
            if position_service is None or exchange_query_fn is None:
                logging.debug("[R26-P0-DI-01] 对账跳过: position_service或exchange_query_fn未提�?)
                return result
        ...
        try:
            ...
            logging.debug("[R27-P1-DI-03] bar/tick一致性校验跳�? %s", e)
        ...
        try:
            ...
            logging.debug("[R27-P1] 订单状态同步跳�? %s", e)
        ...
        try:
            ...
            logging.debug("[R27-P1] 引用完整性校验跳�? %s", e)
        ...
        try:
            ...
            logging.debug("[R27-P1] 热加载状态查询跳�? %s", e)
        return result

    def run_periodic_diagnostic(self, conn=None) -> None:
        """
        运行 2605 合约日志诊断（手动调用，�?30 秒）(DuckDB 版本)
        ...
        try:
            ...
            logging.info("[RES-P2-10] 灾难恢复手册已加�? %d条流�?, len(DISASTER_RECOVERY_PROCEDURES) if isinstance(DISASTER_RECOVERY_PROCEDURES, (list, dict)) else 1)
        except ImportError:
            logging.debug("[RES-P2-10] ops_documentation不可�?)

        try:
            logging.info("\n" + "="*80)
            logging.info("📊 [2605 合约诊断] 开始检�?(�?30 秒自动输�?")
            logging.info("="*80)
            ...
            for idx, contract in enumerate(self.DIAGNOSTIC_CONTRACTS_2605, 1):
                ...
                if option_id and option_status:
                    ...
                    logging.warning(f"  �?期权合约不存�?)

            logging.info("\n" + "-"*80)
            logging.info("📋 汇总统�?")
            total_options_count = len(self.DIAGNOSTIC_CONTRACTS_2605)
            logging.info(f"  期货：{total_futures_subscribed}/{total_options_count} 已订阅，{total_futures_received}/{total_options_count} 有数�?)
            logging.info(f"  期权：{total_options_subscribed}/{total_options_count} 已订阅，{total_options_received}/{total_options_count} 有数�?)
            logging.info("="*80 + "\n")
    ...
    def _log_receive_status(self, status: Dict) -> None:
        ...
        if has_data:
            logging.info(f"  接收：[OK] 成功 ({kline_count}�?K 线，{tick_count}�?Tick)")
        elif status.get('received'):
            logging.warning(f"  接收：[WARN] 无数�?({kline_count}�?K 线，{tick_count}�?Tick)")
        else:
            logging.error(f"  接收：[FAIL] 失败 ({kline_count}�?K 线，{tick_count}�?Tick)")
    ...
    def reserve_next_global_id(self) -> int:
        ...
        线程安全：通过 _sequence_lock 保证�?改原子性�?
