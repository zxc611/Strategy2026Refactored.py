...
from infra.serialization_utils import json_dumps, safe_json_dumps
...
from infra.resilience import safe_callback_wrapper, SubscriberSnapshotGuard
...
__all__ = [
    ...
    'OpsOperationEvent', 'safe_import_event_bus', 'get_global_event_bus',
    'reset_global_event_bus', 'producer', 'consumer', 'get_producer_consumer_registry',
    'record_dataflow_latency', 'get_dataflow_perf_stats', 'detect_dataflow_anomaly',
    'get_dataflow_anomalies',
]
...
class EventBus:
    ...
    def __init__(self, max_workers: int = None, max_history_size: int = None, 
        ...
        self._event_error_log: List[Dict[str, Any]] = []
        ...
        if async_mode:
            ...
            try:
                ...
                with self._pending_events_lock:
                    self._pending_events -= 1
                self._event_error_log.append({
                    'event_type': event_type,
                    'error': str(e),
                    'timestamp': time.time(),
                })
                if len(self._event_error_log) > 100:
                    self._event_error_log = self._event_error_log[-100:]
                logging.error(
                    f"[EventBus][owner_scope=shared-service][source_type=event-tail] "
    ...
    def _persist_event(self, event_type: str, event: Any) -> None:
        ...
        try:
            ...
            event_data = {
                'type': event_type,
                'timestamp': timestamp,
                'data': safe_json_dumps({'event': str(event)}, fallback='{}')[:500]
            }
            
            import json
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(json_dumps(event_data))
    ...
    def get_event_error_log(self) -> List[Dict[str, Any]]:
        """P3.4降级版：获取事件发布错误日志"""
        return list(self._event_error_log)
