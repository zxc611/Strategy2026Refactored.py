...
from infra.shared_utils import normalize_instrument_id, CHINA_TZ
# ============================================================
...
def _current_trade_date_for_coverage() -> date:
    try:
        from data.quant_infra import ExchangeTime
...
class HealthCheckAPI:
    ...
    def _probe_component_status(self, component: str) -> str:
        ...
        try:
            if component == 'greeks_calculator':
                from governance.greeks_calculator import get_greeks_calculator
    ...
    def _push_health_status(self):
        ...
        while not self._push_stop.is_set():
            ...
            try:
                from infra.event_bus import get_global_event_bus, HealthStatusEvent
    ...
    def stop_push(self) -> None:
        ...
        if self._push_thread is not None and self._push_thread.is_alive():
            ...
            if self._push_thread.is_alive():
                logging.warning("[HealthMonitor] push_thread join超时(2s)")
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            from infra.service_container import get_service_container
            ...
            pass
        try:
            from infra.shared_utils import ThreadMgr
        ...
        try:
            from config.ui_service import UIDiagnosisTool
    ...
    def check_capacity_limits(self) -> Dict[str, Any]:
        ...
        if any_critical or any_warning:
            try:
                from risk.risk_service import alert, AlertLevel
...
class CodeQualityMetrics:
    ...
    def _get_component_failure_rate(self) -> float:
        """获取组件失败率"""
        try:
            from strategy_judgment._judgment_services import was_blocked
...
def _find_config_dir() -> Optional[str]:
    ...
    _candidates = [
        ...
        os.path.join(os.getcwd(), 'pyStrategy', 'demo', '', 'config'),
    ]
    if 'INFINITRADER_SIMULATIONX64' in _this_file.upper() or 'InfiniTrader_SimulationX64' in _this_file:
        ...
        _candidates.append(os.path.join(_base, 'pyStrategy', 'demo', '', 'config'))
...
class DiagnosisProbeManager:
    ...
    # ================================================================
    # FIX-58: 生命周期调用诊断探针
    # 根因: t_type_bootstrap.py的logging.critical在setup_logging()之前执行,
    #       日志去stderr不可见。10轮94处修复全部在修改从未被验证执行的代码。
    # 修复: 使用独立文件写入(参照HealthMonitor._log_health_event模式),
    #       不依赖Python logging模块的handler配置,直接open()写文件。
    #       在每个生命周期方法入口/返回处记录,下次重启后可确切知道C++调用了哪些方法。
    # ================================================================

    _lifecycle_log_path: Optional[str] = None
    _lifecycle_log_lock = threading.Lock()

    @classmethod
    def _get_lifecycle_log_path(cls) -> str:
        """获取生命周期诊断日志文件路径(延迟初始化)"""
        if cls._lifecycle_log_path is None:
            _base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            _log_dir = os.path.join(_base_dir, 'logs')
            os.makedirs(_log_dir, exist_ok=True)
            cls._lifecycle_log_path = os.path.join(_log_dir, 'lifecycle_probe.jsonl')
        return cls._lifecycle_log_path

    @classmethod
    def on_lifecycle_call(cls, method_name: str, phase: str = 'CALLED',
                          strategy_id: Any = None, trading: Any = None,
                          inited: Any = None, extra: Optional[Dict] = None) -> None:
        """生命周期方法调用探针

        在t_type_bootstrap和strategy_2026的每个生命周期方法入口/返回处调用。
        独立写入文件,不依赖logging模块,确保在setup_logging()之前也能记录。

        Args:
            method_name: 方法名(on_init/on_start/on_stop/pause/resume/on_destroy)
            phase: 阶段(CALLED/RETURNED/ERROR)
            strategy_id: 策略ID
            trading: trading状态
            inited: inited状态
            extra: 额外信息(strategy_id同步结果等)
        """
        try:
            import traceback as _tb
            _caller_stack = ''.join(_tb.format_stack()[-4:-1]).strip()
            _record = {
                'timestamp': datetime.now(CHINA_TZ).isoformat(),
                'probe': 'lifecycle',
                'method': method_name,
                ...
                'caller_stack': _caller_stack[:500],
                'extra': extra or {},
            }
            _path = cls._get_lifecycle_log_path()
            with cls._lifecycle_log_lock:
                with open(_path, 'a', encoding='utf-8') as _f:
                    safe_jsonl_append_line(_f, _record)
                    _f.flush()
                    try:
                        os.fsync(_f.fileno())
            ...
            # 最后防线: 用print到stderr,确保至少有某种记录
            try:
                import sys as _sys
                print(f"[LIFECYCLE-PROBE-FAIL] {_err}", file=_sys.stderr)
            except Exception:
                pass
...
def _get_runtime_state():
    ...
    try:
        ...
        if runtime_strategy is None:
            try:
                from config.config_params import _param_table_cache
...
def run_14_contracts_periodic_diagnostic(storage=None, query_service=None) -> None:
    ...
    for contract, info in MONITORED_CONTRACTS.items():
        try:
            ...
            try:
                # FIX: config_dir路径错误，原为''子目录（不存在），应为'config'子目录
