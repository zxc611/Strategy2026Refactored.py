...
class LifecycleCallbacks:
    ...
    def on_start(self) -> bool:
        ...
        try:
            ...
            if _ps is None and _bl is not None:
                try:
                    _ps = _bl.ensure_position_service()
                except Exception:
                    pass
            if _bl is None:
                                    ...
                                    for _product, _opt_ids in _tts_opt_dict.items():
                                        for _oid in (_opt_ids or []):
                                            ...
                                            try:
                                                ...
                                                _dbm = _ps.get_instrument_meta_by_id(_oid) if _ps else None
                                                _iid = (_dbm.get('internal_id') if _dbm else None) or _meta.get('internal_id')
                                                _ufid = (_dbm.get('underlying_future_id') if _dbm else None) or _meta.get('underlying_future_id')
                                                if _ufid is None and _dbm and _dbm.get('underlying_product'):
                                                    _up = _dbm['underlying_product']
                                                    _ym = _dbm.get('year_month', '')
                                                    if _up and _ym and (_up, _ym) not in _underlying_cache:
                                                        from ali2026v3_trading.config.config_exchange import make_platform_future_id
                                                        _fi = make_platform_future_id(_up, _ym)
                                                        _fm = _ps.get_instrument_meta_by_id(_fi) if _ps else None
                                                        _underlying_cache[(_up, _ym)] = (_fm.get('internal_id') if _fm else None)
                                                    _ufid = _underlying_cache.get((_up, _ym))
                                                _op = ((_dbm.get('product') if _dbm else None) or _meta.get('product'))
                                                _mo = ((_dbm.get('year_month') if _dbm else None) or _meta.get('year_month'))
                                                _ot = ((_dbm.get('option_type') if _dbm else None) or _meta.get('option_type'))
                                                _sk = ((_dbm.get('strike_price') if _dbm else None) or _meta.get('strike_price'))
                                                if _iid is None or _ufid is None or not _op or not _mo or not _ot or not _sk or _sk <= 0:
                                                    continue
