...
包含: WidthStrengthCache + SortEntry + _NoOpDiagnosisProbeManager
"""
...
try:
    from ali2026v3_trading.event_bus import KLineEvent as EventBusKLineEvent
except ImportError:
...
class WidthStrengthCache:
    ...
    def __init__(self, tracked_option_tick_ids: Optional[set] = None, params_service=None):
        self._lock = threading.RLock()  # ⚠️ 性能卡点6.3：全局RLock，V7本期不改为分片锁（改动面太大，需V8专项重构）
    ...
    def _resolve_internal_id(self, instrument_id: str) -> Optional[int]:
        ...
        Args:
            instrument_id: 平台合约代码（原样直通，不做大写化）
    ...
    def _classify_status(self, underlying_future_id: int, month: str, opt_type: str,
        ...
        if option_direction is None:
            logging.debug("[_classify_status] option_direction is None for future=%s month=%s opt_type=%s, using prev_price fallback",
    ...
    def _recalc_all_state_for_underlying(self, future_internal_id: int):
        ...
        Args:
            future_internal_id: 期货合约 internal_id（主键）
        """
        # ✅ 从 id_cache 获取 product（无需锁，只读外部服务）
        info = self._get_params().get_instrument_meta(future_internal_id)
        if not info:
            logging.warning(f"[WidthStrengthCache] Unknown future_internal_id: {future_internal_id}")
            return
        ...
        with self._lock:
            ...
            for iid in target_internal_ids:
                ...
                snapshot.append({
                    ...
                    'option_prev_price': self._option_prev_price.get(iid, 0.0),
                    'option_last_direction': self._option_last_direction.get(iid),
                    'old_status': self._current_status.get(iid),
                })
        
        # 阶段2：锁外重算 - 耗时的状态计算在锁外完成
        temp_status_counts = defaultdict(lambda: defaultdict(self._new_status_bucket))
        temp_sync_otm_count = defaultdict(lambda: defaultdict(int))
        sort_bucket_updates = []
        
        for item in snapshot:
            iid = item['iid']
            opt_info = item['opt_info']
            ...
            if old_status and old_status in temp_status_counts[month][opt_type]:
                temp_status_counts[month][opt_type][old_status] = max(0, temp_status_counts[month][opt_type][old_status] - 1)
            temp_status_counts[month][opt_type][status] += 1
            
            is_sync = self._compute_sync_flag(opt_info, option_price, item['option_last_direction'])
            if is_sync:
                is_otm = self._is_out_of_the_money(opt_type, future_price, strike)
                if is_otm:
                    temp_sync_otm_count[month][opt_type] += 1
            
            sort_bucket_updates.append({
                'iid': iid,
                'opt_info': opt_info,
                'future_internal_id': future_internal_id,
                'month': month,
                'opt_type': opt_type,
                'status': status,
                'is_sync': is_sync,
    ...
    def print_status_diagnosis(self, future_internal_id: int = None, top_n: int = 10):
        ...
        with self._lock:
            ...
            for fid in futures_to_diagnose:
                ...
                info = self._get_params().get_instrument_meta(fid)
                if not info:
