...
class StrategyMonitoringLayer:
    ...
    def check_position_risk(self) -> None:
        ...
        # FIX-20260708-CLOSE-BREAK: 移除_is_running检查，暂停时仍执行持仓风控检查
        if getattr(provider, '_destroyed', False):
            return
    ...
    def emergency_stop(self, caller_id: str = "unknown", reason: str = "") -> Dict[str, Any]:
        ...
        # Step 1: 暂停策略
        # FIX-20260708-PAUSE-ROOT: 彻底修复emergency_stop的四元状态不同步问题
        # 根因1: 原代码无锁修改_is_paused/_is_trading，与pause()/resume()的带锁修改竞态
        # 根因2: 原代码转换到STOPPED，导致无法resume（resume只接受PAUSED）
        # 根因3: 原代码未设置_is_running=False，四元状态不完整
        # 修复: 获取p._lock，使用_lifecycle_mgr，转换到PAUSED，同步四元状态
        try:
            _lock = getattr(provider, '_lock', None)
            if _lock is not None:
                with _lock:
                    # ⚠️ FIX-20260707-PAUSE-SAFE: 四元状态原子同步
                    ...
                    from ali2026v3_trading.lifecycle.lifecycle_state_machine import StrategyState as _SS
                    _transition_fn = getattr(provider, 'transition_to', None)
                    if callable(_transition_fn):
                        _transition_fn(_SS.PAUSED)
