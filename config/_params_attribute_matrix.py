# [M1-91] 参数属性矩阵验证
# MODULE_ID: M1-003
"""参数服务 - 属性矩阵验�?(AttributeMatrixMixin)"""
...
class AttributeMatrixService:
    ...
    def load_attribute_matrix(self, yaml_path: Optional[str] = None) -> Dict[str, Any]:
        """加载 parameter_attribute_matrix.yaml 并校验当前参�?
    ...
    def validate_with_attribute_matrix(self) -> Dict[str, Any]:
        """基于 attribute_matrix 校验当前 _params 中的所有参�?
    ...
    def eval_dynamic_param(self, key: str, context: Optional[Dict[str, Any]] = None) -> Any:
        ...
        根据运行�?context（如实时波动率、时间、持仓比例等）计算参数值�?
        若计算失败或表达式不存在，降级返回静�?default 值�?
        ...
        Args:
            key: 参数键名
            context: 运行时变量字典，�?{'iv_percentile': 65, 'volatility': 0.25}

        Returns:
            计算后的参数值；无动态表达式则返回当前静态�?
        """
