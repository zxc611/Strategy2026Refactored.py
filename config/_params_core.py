...
class ParamsService:
    ...
    def __init__(self, logger_func: Optional[Callable] = None):
        ...
        # P0 Bug #9修复：在__init__中调用init_instrument_cache，确保缓存容器已初始�?
    ...
    def __getattr__(self, name):
        ...
        # OPS-P1-03修复: 参数变更通知 �?P1-05修复: 统一走EventBus，不再需要本地回调列�?

        # UPG-P1-07修复: 热更新事务性保�?�?参数备份
        ...
        try:
            ...
            with self._lock:
                if path:
                    ...
                    logging.info(f"[ParamsService.force_refresh] 强制刷新全部参数 ({count}�?")
            return True
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as e:
            ...
            with self._lock:
                ...
                for _path in list(self._params.keys()):
                    try:
                        ...
                        logging.debug("[R3-L2] ...")
                        pass
            logging.info("[ParamsService] 配置热重载成�?)
            return True
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as e:
            logging.error("[ParamsService] 配置热重载失�? %s", e)
            return False
    ...
    def resolve_param_version(cli_version: str = None, env_version: str = None,
        ...
        """
        手册15节要�? 支持通过环境变量或CLI指定参数版本(�?-param-version v1.2)，便于快速回滚�?
        优先�? CLI > 环境变量 > 默认路径
        """
        import os
    ...
    def get_list(self, key: str, default: Optional[List] = None) -> List[Any]:
        """获取列表参数（线程安全）"""
        # R24-P2-DF-05修复: 区分"未传默认"�?显式None"，避免default or []覆盖显式None意图
        with self._lock:
    ...
    # UPG-P1-07修复: 热更新事务性保�?�?备份/提交/回滚方法
    def hot_update_begin(self) -> Dict[str, Any]:
        """UPG-P1-07修复: 热更新开始，备份当前参数状�?
        ...
        if not self._hot_update_txn_lock.acquire(blocking=False):
            raise RuntimeError("R21-P2-UG-12: 热更新事务冲突，已有热更新正在进�?)
        try:
        ...
        try:
            with self._param_backup_lock:
                if self._param_backup is not None:
                    logging.info(
                        "UPG-P1-07: 热更新已提交, 清除备份(原参数数�?%d)",
                        len(self._param_backup),
                    )
                    self._param_backup = None
                else:
                    logging.debug("UPG-P1-07: 热更新提交时无备�?可能已提交或未开�?")
        finally:
            self._hot_update_txn_lock.release()

    def hot_update_rollback(self) -> bool:
        """UPG-P1-07修复: 热更新回滚，恢复到备份状�?
        ...
        try:
            with self._param_backup_lock:
                if self._param_backup is None:
                    logging.warning("UPG-P1-07: 热更新回滚失�? 无可用备�?)
                    return False
            ...
            logging.critical("R21-P2-UG-12: 热更新回滚异常，强制释放事务�? %s", e)
            return False
        finally:
            self._hot_update_txn_lock.release()
    ...
    def hot_update(self, params: Dict[str, Any], notify: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.error(
                "UPG-P1-09: 事务性热更新失败, 已回�?%s, error=%s",
                rolled_back, e,
            )
    ...
    # OPS-P1-03修复: 参数变更通知回调 �?P1-05修复: 统一走EventBus
    ...
    def _fire_param_change_callbacks(self, key: str, old_value: Any, new_value: Any,
        ...
        try:
            ...
            logging.debug("[ParamsService] OPS-P1-03: EventBus参数变更推送异�? %s", e)
    ...
    # �?ID唯一：clear_cache统一接口，服�?ParamsService
...
_params_service_lock = threading.Lock()  # R1-1修复: 补回拆分时遗漏的锁定�?


def reset_params_service() -> None:
    """R27-CP-03-FIX: 重置ParamsService单例，用于测试隔�?""
...
def get_params_by_category(category: str) -> Dict[str, Any]:
    ...
    Args:
        category: 参数分类名称，支�?'shadow'/'quantitative'/'intuition'

    Returns:
        Dict[str, Any]: 对应分类的参数字�?
    """
...
def get_params_by_risk_level(risk_level: str = "all") -> Dict[str, Any]:
    ...
    Args:
        risk_level: 风险等级，支�?'low'/'medium'/'high'/'all'

    Returns:
        Dict[str, Any]: 符合风险等级的参数字�?
    """
    ps = get_params_service()
...
def get_params_by_strategy_type(strategy_type: str = "all") -> Dict[str, Any]:
    ...
    Args:
        strategy_type: 策略类型，支�?'box_extreme'/'spring'/'trend'/'arbitrage'/'market_making'/'all'

    Returns:
        Dict[str, Any]: 符合策略类型的参数字�?
    """
    ps = get_params_service()
    all_params = ps.get_all()
...
# ============================================================================
# 参数读取工具函数
# �?strategy_core_service.py 提取，解决循环导入：
