...
config_dataclasses.py _配置数据类定义
...
# P1-53修复: strategy.duckdb路径统一为模块级常量，消息处重复
...
# 参数数据类（__params_dataclasses.py_
# ============================================================================
...
class ParamAuditObserver(ParamObserver):
    ...
    记录结构化审计日志，包含：谁/何时/改了什的旧值→新回调用栈摘要。'
    审计日志写入专用 logger (params_audit)，可与普通日志分离归档档
    """
