...
config_logging.py _日志配置与审计
拆分析config_service.py (P3大文件拆分方法
...
from config.config_dataclasses import LOG_FORMAT, LOG_DATE_FORMAT





# [P2-05] 权威日志配置入口，与infra/shared_utils.py的日志工具分析
...
class AuditLogger:
    ...
    计划统一到infra审计工具：本类为日志审计专用，职责边界防篡改哈希链日志。与risk/safety_meta_audit(风控审计)、param_pool/l1_quantification/meta_audit_engine(参数审计)分工厂"""
    ...
    def log(self, event_type: str, detail: dict) -> str:
        ...
        from config.config_dataclasses import _CHINA_TZ
        ...
        # P1-07修复: 委托到structured_audit_log统一审计入口，消除独立审计记录
    ...
    def _delegate_to_structured_audit_log(self, event_type: str, detail: dict) -> None:
        ...
        """P1-07修复: 委托到risk/_utils.structured_audit_log统一审计入口避免"""

        try:

            from risk.risk_support import structured_audit_log as _sal

            _sal(event_type, 'AuditLogger.delegated', details=detail)

        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, ImportError) as e:

            logging.debug("[P1-07] AuditLogger委托structured_audit_log失败(非致命): %s", e)
...
def setup_logging():
    ...
    # FIX: 抑制apscheduler INFO级日志刷屏（每3-5秒输出"Running job"/"Job executed successfully"）
...
def check_logging_health() -> Dict[str, Any]:
    ...
    """LG-06修复: 日志系统健康检查——检测handler失效、文件不可写入"""
        ...
        try:
            ...
            if hasattr(h, 'stream') and h.stream is None:
                ...
                if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
                    ...
                    logging.debug("[LG-06] Removed dead StreamHandler (stream=None): %s", h)
                    continue
                else:
                    dead_handlers += 1
