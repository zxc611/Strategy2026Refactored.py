...
合并来源�?6_ui.py + ui_mixin.py
合并策略：提取UI控制核心功能，简化界面逻辑

核心功能�?
1. UILogicService - UI逻辑/样式/模式切换服务
2. UICreationService - UI创建/启动服务
3. UIMixin - 策略UI混入�?26个方�? �?由_UIMixinLogic + _UIMixinCreation组合
4. StrategyUI - 独立UI控制面板
5. UIDiagnosisTool - UI诊断工具
...
class UILogicService:
    ...
    包含：__init__, _get_ui_lock, 类属�? _refresh_output_mode_ui_styles,
                    ...
                    for k, v in data.items():
                        ...
                        if hasattr(self.params, k):
                            ...
                            if original is not None and isinstance(original, (int, float)) and isinstance(v, (int, float)):
                                ...
                                raise TypeError(f"参数{k}类型不匹�? 期望{type(original).__name__}, 实际{type(v).__name__}")
                            setattr(self.params, k, v)
                    messagebox.showinfo("成功", "参数已保�?)
                    editor.destroy()
                except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as e:
    ...
    def _on_backtest_click(self) -> None:
        ...
        try:
            ...
            self._log_error(f"打开回测参数编辑器失�? {e}")
...
class UICreationService:
    ...
    def __getattr__(self, name):
        delegated = [
            ...
            'params', '_get_ui_lock', '_ui_lock', '_ui_root',
    ...
    def _create_ui_in_main_thread(self, root: Any) -> None:
        ...
        try:
            ...
            BTN_WIDTH = 12
            btn_debug = tk.Button(debug_frame, text="开盘调�?, width=BTN_WIDTH)
            ...
            def _do_safe_pause():
                try:
                    from ali2026v3_trading.infra.health_monitor import ControlActionLogger as _CAL
            ...
            self._refresh_output_mode_ui_styles()
            self._log_info(f"UI界面已在主线程中创建，当前模�?{getattr(self.params, 'output_mode', 'debug')}, auto_trading={getattr(self, 'auto_trading_enabled', False)}")
                ...
                def _ui_mainloop_thread():
                    try:
                        ...
                        if root:
                            ...
                            root.mainloop()
                            self._log_info("UI线程mainloop已退�?)
                        else:
                            self._log_warning("Tk root创建失败，使用轮询模�?)
...
# =============================================================================
# UIMixin - 策略UI混入类（组合门面�?
# =============================================================================

class UIMixin:
    """策略UI混入�?- 提供Tkinter界面控制功能（Facade组合，消灭Mixin继承�?

    [P1-2归属] 被Strategy2026(BaseStrategy, UIMixin)继承 | ServiceContainer可选注册为'ui'服务
    | 与StrategyCoreService无直接关�?StrategyCoreService仅继�?核心Mixin)
...
class StrategyUI:
    ...
    def _build_status_panel(self) -> None:
        ...
        try:
            ...
            status_frame = ttk.LabelFrame(self.root, text="状�?, padding=10)
    ...
    def _update_status(self) -> None:
        ...
        try:
            ...
            status = []
            status.append(f"运行状�? {getattr(self.strategy, 'my_is_running', False)}")
            status.append(f"暂停状�? {getattr(self.strategy, 'my_is_paused', False)}")
            status.append(f"交易状�? {getattr(self.strategy, 'my_trading', True)}")
...
class UIDiagnosisTool:
    ...
    def check_thread_safety() -> Dict[str, Any]:
        ...
        try:
            ...
            # Tkinter不是线程安全�?
            result["safe"] = False
            result["issues"].append("Tkinter主循环必须在主线程运�?)
