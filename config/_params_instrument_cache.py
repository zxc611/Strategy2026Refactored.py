...
class InstrumentCacheService:
    ...
    def validate_contracts_loaded(self, contract_ids: List[str], contract_type: str = "contract") -> List[str]:
        ...
        Returns:
            failed_contracts: 加载失败的合约列�?
            
        Raises:
            RuntimeError: 如果有合约加载失�?
        """
        # 确保缓存已加载（使用新架构字典判断）
        if not self._instrument_id_to_internal_id:
            self._lazy_load_from_db()
    ...
    def _lazy_load_from_db(self) -> None:
        ...
        try:
            ...
            ds = get_data_service()
            if ds is None:
                logging.debug("[ParamsService] DataService尚未初始化，跳过惰性加�?)
                return
            ...
            if not hasattr(ds, '_get_connection'):
                logging.warning("[ParamsService] DataService实例不完整，跳过惰性加�?)
                return
            
            self.load_caches_from_db(ds)
            logging.info("[ParamsService] 惰性加载缓存完成：%d 个合�?, len(self._instrument_id_to_internal_id))
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, ImportError) as e:
            logging.debug("[ParamsService] 惰性加载缓存失�? %s", e)
    ...
    def load_instrument_list(self, params: Any, source: str = 'param_cache') -> Optional[Dict[str, Any]]:
        ...
        Args:
            params: 参数对象（dict或对象）
            source: 加载来源�?param_cache' �?'output_files'
            
        Returns:
            Dict: {'futures_list': [...], 'options_dict': {...}} �?None
        """
        try:
            futures_metadata = {}
            options_metadata = {}
            ...
            if source == 'output_files':
                ...
                for attempt in range(1, max_retries + 1):
                    ...
                    if load_success and (futures_from_file or options_from_file):
                        ...
                        logging.warning(f"[ParamsService] 配置文件加载失败，{retry_delay}s后重�?({attempt}/{max_retries})...")
                        import threading
                        threading.Event().wait(retry_delay)  # [R22-P2-TIME06] 替代time.sleep以便可中�?
                    else:
                        error_details = []
                        for fixed_config_file in _fixed_config_files:
    ...
    def load_caches_from_db(self, data_service) -> None:
        ...
        def _cache_temp_instrument_info(instrument_id: str, info: Dict[str, Any]) -> None:
            """复用_normalize_id_field进行ID标准�?""
            cached_info = dict(info)
            cached_info['instrument_id'] = instrument_id

            # �?复用_normalize_id_field统一ID标准�?
            self._normalize_id_field(cached_info)
            cid = cached_info.get('internal_id')
        ...
        logging.info(f"[ParamsService] 总合约数: {futures_loaded + options_loaded} �?(期货={futures_loaded}, 期权={options_loaded})")
        ...
        try:
            ...
            logging.warning(f"[ParamsService] 查询期货品种元数据失�? {e}")
        ...
        try:
            ...
            logging.warning(f"[ParamsService] 查询期权品种元数据失�? {e}")
        ...
        logging.info(f"加载缓存：{len(self._instrument_id_to_internal_id)}个合约，{len(self._product_cache)}个品�?)
