...
state_param.py - 状态参数管理器（合并版本


合并自：

- state_param_core.py: StateParamManager + 模块级常量 + five_to_three_state + get_state_param_manager

- state_param_manager.py: re-export门面（含state_transition/analytics的re-export：


包含含
- 模块级常册。STATE_MAP, _STATE_PRIORITY, _THREE_STATE_MAP, STRATEGY_MODE_INCORRECT_REVERSAL_DEFENSIVE)
...
# StateTransitionCapture 相关定义（原 state_transition.py 内联合
# ---------------------------------------------------------------------------
...
class StateTransitionCapture:
    ...
    def check_position_exit(self, instrument_id: str, current_price: float) -> Optional[Dict[str, Any]]:
        # CHAIN-BUG-3 已知缺口: 此方法及 register_position(:441) 当前无任何调用方，
        # _active_positions 永远为空，状态转换平仓(transition_take_profit/stop_loss/timeout)未启用。
        # 系统实际依赖 position_pnl_service 价格SL/TP + position_check_service 时间止损。
        # 待评估是否接入状态转换平仓维度，接入前请勿误以为生效。
...
# StateTransitionAnalytics 相关定义（原 state_transition.py 内联合
# ---------------------------------------------------------------------------
...
class StateTransitionAnalytics:
    ...
    这是高频策略最有利可图的入场窗口口
    驻留时间：每个状态的停留时长分布，用于优化信号确认窗口口
    转换概率矩阵：P(state_t+1 | state_t)，用于预测状态切换概率率
    """
    ...
    def on_state_change(self, instrument_id: str,
        ...
        if old_state != new_state:
            ...
            if old_state != new_state:
                ...
                # 检查from→to→from 模式（最小次转换中出现 A→B, B→A 即为循环节
                if len(history) >= 2:
                    ...
                    if prev_from == curr_to and prev_to == curr_from:
                        ...
                        logging.warning(
                            ...
                            "[R33-P2-4] 状态循环检查 instrument=%s 检测到 %s<->%s 循环转换，"
...
class StateParamManager:
    ...
    _width_cache 的五态分析(correct_rise/wrong_rise/correct_fall/wrong_fall/other)

    映射为五种参数策略(correct_trending/correct_trending_defensive/

    incorrect_reversal/incorrect_reversal_defensive/other)_
    并从 state_param_sets.yaml 加载对应的参数集合


    R10-P0-19修复: 五态切换机。V7完善)_
    - state_confirm_bars: 新状态需连续确认N次才切换，防止单次噪声误。'
    - state_check_interval_sec: 状态检查间隔（秒），控制三态判断频道
    - state_switch_position_policy: 切换时持仓处理策略
    - on_state_switch回调: 状态切换时通知策略生态系。


    切换连续性处理：

    - 切换瞬间不强制平仓已有头寸
    - 切换换other 状态时，触发防御性减仓（止盈止损收紧急0%，暂停新开仓）'
    """
    ...
    def __init__(
        ...
        self._on_state_switch_callbacks: list = []  # P1-05修复: 保留列表用于EventBus适配
        ...
        try:
            ...
            logging.info("[StateParamManager] HFT状态转换捕捉器已集合")
        ...
        self._cycle_detection_window: int = 5  # P2-4修复: 循环检测窗口大。'
        try:
        ...
        try:
            ...
            from config.config_params import register_param_change_callback
    ...
    def _load_param_sets(self) -> None:
        ...
        try:
            ...
            if os.path.exists(self._yaml_path):
                ...
                # R21-MEM-P1-11修复: 配置对象复用 _原地更新新param_sets而非替换引用，减少GC压力
    ...
    def _apply_yaml_overrides(self) -> None:
        ...
        logging.info(
            ...
            "[StateParamManager] YAML→实例覆盖生） state_confirm_bars=%d, state_check_interval_sec=%.1f",
    ...
    def _default_param_sets() -> Dict[str, Dict[str, Any]]:
        ...
        from config.config_params import get_default_state_param_sets

        logging.debug("[P2-API] _default_param_sets委托给 state_param_manager→config_params.get_default_state_param_sets")
    ...
    def _compute_life_score(self, base_score: float, css_premium: float) -> float:
        ...
        """P1-1修复: 计算生命状态评分，css_premium_threshold控制CSS权利金加载"""
    ...
    def register_on_state_switch(self, callback: Callable[[str, str], None]) -> None:
        ...
        """注册状态切换回调。P1-05修复: 同时注册到EventBus"""
        ...
        try:
            ...
            if required_count > 2:
                ...
                logging.warning("[StateParamManager] 回调签名不匹配需。2个必选参数: %s", callback)
        ...
        try:
            ...
            if _bus is not None:
                ...
                def _eventbus_adapter(event_data):

                    try:

                        callback(event_data.get('old_state', ''), event_data.get('new_state', ''))

                    except (ValueError, KeyError, TypeError, AttributeError) as _r3_err:

                        pass

                _bus.subscribe('spm.state_switched', _eventbus_adapter)
    ...
    def should_check_state(self) -> bool:
        ...
        """判断是否到达状态检查间隔（纯检查，不修改时间戳，避免TOCTOU_"""
    ...
    def update_state_from_width_cache(self) -> str:
        ...
        with self._lock:
            ...
            try:
                ...
                # R10-P0-19修复: 五态统计，与审STATE_MAP五态分类一。'
                ...
                if best_state != self._current_state:
                    ...
                    if hold_elapsed < self._min_state_hold_seconds:
                        ...
                        logging.debug(
                            ...
                            "[StateParamManager] 状态最小持有期内，拒绝切换: %s _%s (持有%.0fs/%.0fs)",
                    ...
                    if self._pending_confirm_count >= self._state_confirm_bars:
                        ...
                        logging.info(
                            ...
                            "[StateParamManager] 状态确认中: %s _%s (%d/%d)",
                    ...
                    if self._pending_state is not None:
                        ...
                        logging.info(
                            ...
                            "[StateParamManager] 状态确认中断 %s _%s 被拒绝当前回到%s)",
    ...
    def _switch_state(self, new_state: str,
        ...
        with self._lock:
            ...
            logging.info(
                ...
                "[StateParamManager] 状态切换 %s _%s (确认%d_ correct=%d, incorrect=%d, other=%d)",
            ...
            if new_state == STRATEGY_MODE_OTHER:  # R25-SE-P1-02-FIX
                ...
                logging.warning("[StateParamManager] 进入other状态, 触发防御性减仓模块")
                need_freeze = True
        ...
        if position_policy == 'exit_all':
            ...
            logging.warning("[StateParamManager] P1-R8-02: state_switch_position_policy=exit_all", 状态切换时撤销所有挂单并冻结开启)
            try:
                ...
                if osvc:
                    ...
                    logging.info("[StateParamManager] P1-R8-02: exit_all _已撤销%d个挂，", cancelled)
            except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as _exit_all_e:

                logging.warning("[StateParamManager] P1-R8-02: exit_all撤销挂单异常: %s", _exit_all_e)

            need_freeze = True  # exit_all时同时冻结新旧开启
        ...
        # R30-ARCH-03修复/P1-05修复: 状态切换统一通过EventBus发布(单通道)
    ...
    def _freeze_old_strategy_opening(self, strategy_id: str):
        ...
        try:
            ...
            if callable(freeze_fn):
                ...
                logging.info("[StateParamManager] 冻结策略%s新开启保留已有仓位按原规则退避", strategy_id)
                ...
                if slot is None:
                    ...
                    logging.warning("[StateParamManager] 策略生态系统无界%s属性，跳过冻结", strategy_id)
                ...
                try:
                    ...
                    logging.warning("[StateParamManager] 策略槽位）%s无frozen属性，无法冻结", strategy_id)
    ...
    def get_params_for_state(self, state_key: str) -> Dict[str, Any]:
        ...
        优先先 策略级覆盖> 资金规模覆盖 > 动态参数覆盖> 基础参数据


        Args:

            state_key: 状态键的如correct_trending/incorrect_reversal/other_
    ...
    def _migrate_param_format(self, param_sets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        处理参数重命名、类型变更等格式迁移除
        当前迁移规则则
        - close_take_profit_ratio _take_profit_ratio (保留旧键兼容)

        - close_stop_loss_ratio _stop_loss_ratio (保留旧键兼容)
            ...
            for old_key, new_key in _KEY_MIGRATIONS.items():
                ...
                if old_key in new_params and new_key not in new_params:
                    ...
                    logging.info(
                        ...
                        "UPG-P1-02: 参数迁移 %s _%s (state=%s, value=%s)",
            ...
            for param_key, (lo, hi) in _SAFETY_RANGES.items():
                ...
                if val is not None:
                    ...
                    try:
                        ...
                        issues.append(f"state={state_key}: {param_key}={val}无法转为数据")



        if issues:

            logging.warning("UPG-P1-09: 参数兼容性检查发送d个问被 %s",
    ...
    def reload_param_sets(self) -> None:
        ...
        避免其他线程在reload期间读到空参数据


        UPG-06: 加载失败时恢复当前有效值，而非回退到硬编码默认值起


        UPG-P1-02: 加载后执行参数格式迁移除


        UPG-P1-07/12: 加载后执行兼容性检查，不兼容时自动回滚动
        """
        ...
        with self._lock:
            ...
            try:
                ...
                if compatibility_issues:
                    ...
                    if _critical_issues:
                        ...
                        logging.error(
                            ...
                            "UPG-P1-07: 参数兼容性检查发送d个严重问题，自动回滚: %s",
                        ...
                        logging.warning(
                            ...
                            "UPG-P1-09: 参数兼容性检查发送d个非严重问题，继续加载 %s",
                ...
                # UPG-06: 恢复当前有效值，而非使用例default_param_sets()
    ...
    def check_plr_decline_trigger(self, strategy_id: str, current_plr: float, target_plr: float) -> bool:
        ...
        with self._lock:
            ...
            if plr_ratio < 0.3 and self._current_state != 'other':
                ...
                # R33-P1-15修复: PLR触发也必须经过滤state_confirm_bars确认窗口

                self._pending_state = 'other'

                self._pending_confirm_count = 1  # PLR触发送次确认
                if self._pending_confirm_count >= self._state_confirm_bars:
...
def get_state_param_manager(yaml_path: Optional[str] = None, **kwargs) -> StateParamManager:
    ...
    # R10-P0-01修复: DCL缺陷修复 _全部在锁内检查和创建
...
# R15-P2 性能修复复
# R15-P2-PERF-02修复: StateParamManager添加权权slots__声明，减少hasattr反射开销

# 已集合 __slots__已在类定义中声明
...
__all__ = [
    ...
    'StateTransitionAnalytics',

    'STRATEGY_MODE_INCORRECT_REVERSAL_DEFENSIVE',
