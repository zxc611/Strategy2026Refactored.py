...
合并自：
- state_param_core.py: StateParamManager�?+ 模块级常�?+ five_to_three_state + get_state_param_manager
- state_param_manager.py: re-export门面（含state_transition/analytics的re-export�?

包含�?
- 模块级常�?_STATE_MAP, _STATE_PRIORITY, _THREE_STATE_MAP, STRATEGY_MODE_INCORRECT_REVERSAL_DEFENSIVE)
...
STRATEGY_MODE_INCORRECT_REVERSAL_DEFENSIVE = 'incorrect_reversal_defensive'  # R10-P0-19修复: 提取为常�?
_STATE_MAP = {
...
# ---------------------------------------------------------------------------
# StateTransitionCapture 相关定义（原 state_transition.py 内联�?
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# StateTransitionAnalytics 相关定义（原 state_transition.py 内联�?
# ---------------------------------------------------------------------------
...
class StateParamManager:
    ...
    �?width_cache 的五态分�?(correct_rise/wrong_rise/correct_fall/wrong_fall/other)
    映射为五种参数策�?(correct_trending/correct_trending_defensive/
    incorrect_reversal/incorrect_reversal_defensive/other)�?
    并从 state_param_sets.yaml 加载对应的参数集�?

    R10-P0-19修复: 五态切换机�?V7完善)�?
    - state_confirm_bars: 新状态需连续确认N次才切换，防止单次噪声误�?
    - state_check_interval_sec: 状态检查间隔（秒），控制三态判断频�?
    - state_switch_position_policy: 切换时持仓处理策�?
    - on_state_switch回调: 状态切换时通知策略生态系�?

    切换连续性处理：
    - 切换瞬间不强制平仓已有头�?
    - 切换�?other 状态时，触发防御性减仓（止盈止损收紧�?0%，暂停新开仓）
    """

    # R13-P2-LOG-15修复: 历史上限改为类属性，可被子类或构造参数覆�?
    ...
    def __init__(
        ...
        strategy_id: Optional[str] = None,  # P2-R11-01修复: 策略级参数隔�?
        state_confirm_bars: int = 5,
        state_check_interval_sec: float = 60.0,  # [R23-P1-05-FIX] �?80s降至60s，与config_params.CACHE_TTL对齐
        ...
        history_max_len: Optional[int] = None,  # R13-P2-LOG-15修复: 可配置历史上�?
    ):
        self._lock = threading.RLock()
        # P2-R11-01修复: 保存策略ID，用于策略级参数隔离
        self._strategy_id = strategy_id or "default"
        # R13-P2-LOG-15修复: 实例级历史上限，优先使用构造参数，否则使用类属�?
        ...
        try:
            ...
            logging.info("[StateParamManager] HFT状态转换捕捉器已集�?)
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError, ImportError) as e:
            logging.warning("[StateParamManager] HFT状态转换捕捉器初始化失�? %s", e)
    ...
    def _refresh_dynamic_param_overrides(self) -> None:
        try:
            ...
            logging.debug("[StateParamManager] 刷新动态参数覆盖失�? %s", e)
        ...
        try:
            if os.path.exists(self._yaml_path):
                ...
                logging.warning("[StateParamManager] YAML file not found: %s, using defaults",
                                self._yaml_path)
                self._param_sets = self._default_param_sets()
        except (OSError, ValueError) as e:  # R31-P2-10修复: 缩窄异常捕获范围，仅捕获IO和解析错�?
    ...
    def _default_param_sets() -> Dict[str, Dict[str, Any]]:
        ...
        logging.debug("[P2-API] _default_param_sets委托�? state_param_manager→config_params.get_default_state_param_sets")
        return get_default_state_param_sets()

    def _compute_life_score(self, base_score: float, css_premium: float) -> float:
        """P1-1修复: 计算生命状态评分，css_premium_threshold控制CSS权利金加�?""
        if css_premium <= 0:
            return base_score
    ...
    def check_entry_button(self, signal_strength: float) -> bool:
        """P1-2修复: 信号入场确认按钮，信号强度需超过entry_button阈�?""
    ...
    def register_on_state_switch(self, callback: Callable[[str, str], None]) -> None:
        ...
        try:
            ...
            if required_count > 2:
                logging.warning("[StateParamManager] 回调签名不匹�?需�?2个必选参�?: %s", callback)
                return
        except (ValueError, TypeError) as e:
            logging.warning("[StateParamManager] 回调签名检查失�? %s", e)
        with self._lock:
    ...
    def update_state_from_width_cache(self) -> str:
        with self._lock:
            ...
            try:
                ...
                total = 0
                # R10-P0-19修复: 五态统计，与_STATE_MAP五态分类一�?
                ...
                # R10-P0-19修复: non_other_total包含所有四类非other状�?
                ...
                if best_state != self._current_state:
                    ...
                    if self._pending_state is not None:
                        ...
                        logging.info(
                            "[StateParamManager] 状态确认中�? %s �?%s 被拒�?当前回到%s)",
    ...
    def _switch_state(self, new_state: str,
        ...
        with self._lock:
            ...
            if len(self._state_history) >= self._cycle_detection_window:
                ...
                for pair, count in pair_counts.items():
                    if count >= 2:
                        ...
                        logging.warning(
                            "[StateParamManager] P2-4: 检测到状态循环振�?%s�?s (窗口�?d�?, 考虑调整state_confirm_bars或min_state_hold_seconds",
                            pair[0], pair[1], count
                        )
                        break
            ...
            if len(self._state_history) >= self._history_max_len and len(self._state_history) % self._history_max_len == 0:
                logging.warning(
                    "[StateParamManager] 状态切换历史deque已满(maxlen=%d), 自动淘汰最旧记�?# R31-P2-08修复",
                    self._history_max_len,
                )

            logging.info(
                "[StateParamManager] 状态切�? %s �?%s (确认%d�? correct=%d, incorrect=%d, other=%d)",
            ...
            if new_state == STRATEGY_MODE_OTHER:  # R25-SE-P1-02-FIX
                logging.warning("[StateParamManager] ⚠️ 进入other状态，触发防御性减仓模�?)
                need_freeze = True
            ...
            if self._hft_transition_capture:
                try:
                    ...
                    logging.debug("[StateParamManager] HFT状态转换捕捉异�? %s", hft_e)

            callbacks = list(self._on_state_switch_callbacks)
            # R10-P1-01: 快照语义——回调执行期间新注册的回调不会被当前轮触�?
        ...
        if position_policy == 'exit_all':
            logging.warning("[StateParamManager] P1-R8-02: state_switch_position_policy=exit_all, 状态切换时撤销所有挂单并冻结开�?)
            try:
                ...
                if osvc:
                    ...
                    logging.info("[StateParamManager] P1-R8-02: exit_all �?已撤销%d个挂�?, cancelled)
            ...
            need_freeze = True  # exit_all时同时冻结新旧开�?
        elif position_policy == 'migrate_to_new_rules':
            logging.info("[StateParamManager] P1-R8-02: state_switch_position_policy=migrate_to_new_rules, 持仓参数迁移到新状�?)
            self._refresh_dynamic_param_overrides()
    ...
    def _freeze_old_strategy_opening(self, strategy_id: str):
        try:
            ...
            if callable(freeze_fn):
                ...
                logging.info("[StateParamManager] 冻结策略%s新开�?保留已有仓位按原规则退�?", strategy_id)
            else:
    ...
    def get_params_for_state(self, state_key: str) -> Dict[str, Any]:
        ...
        优先�? 策略级覆�?> 资金规模覆盖 > 动态参数覆�?> 基础参数�?

        Args:
            state_key: 状态键�?如correct_trending/incorrect_reversal/other�?
            ...
            for param_key, (lo, hi) in _SAFETY_RANGES.items():
                ...
                if val is not None:
                    try:
                        ...
                        issues.append(f"state={state_key}: {param_key}={val}无法转为数�?)

        if issues:
            logging.warning("UPG-P1-09: 参数兼容性检查发�?d个问�? %s",
        ...
        with self._lock:
            ...
            try:
                ...
                if compatibility_issues:
                    ...
                    if _critical_issues:
                        logging.error(
                            "UPG-P1-07: 参数兼容性检查发�?d个严重问题，自动回滚: %s",
                        ...
                        logging.warning(
                            "UPG-P1-09: 参数兼容性检查发�?d个非严重问题，继续加�? %s",
        ...
        with self._lock:
            ...
            if plr_ratio < 0.3 and self._current_state != 'other':
                ...
                if _hold_elapsed < self._min_state_hold_seconds:
                    ...
                    return False
                logging.info(
                    "[StateParamManager] PLR严重下降触发状态切�? %s plr=%.2f/target=%.2f ratio=%.2f, state=%s->other(需确认)",
                    strategy_id, current_plr, target_plr, plr_ratio, self._current_state
                )
                # R33-P1-15修复: PLR触发也必须经过_state_confirm_bars确认窗口
                self._pending_state = 'other'
                self._pending_confirm_count = 1  # PLR触发�?次确�?
                if self._pending_confirm_count >= self._state_confirm_bars:
