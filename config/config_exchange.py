...
from infra.shared_utils import normalize_year_month, CHINA_TZ
...
def _get_option_underlying_product(option_product: str) -> str:
    try:
        from product_initializer import _get_option_underlying_product as _impl
...
def ensure_products_with_retry(data_service, max_retries: int = 5) -> Dict[str, int]:
    try:
        from lifecycle.product_initializer import ensure_products_with_retry as _impl
...
tick_size_by_product = {
    ...
    "T": 0.005, "TF": 0.005, "TS": 0.002, "TL": 0.01,
    # ---- SHFE (上海期货交易所) ----
    "CU": 10, "AL": 5, "ZN": 5, "PB": 5, "NI": 10,
    "AU": 0.02, "AG": 1, "RB": 1, "HC": 1, "SS": 5,
    "RU": 5, "BU": 2, "SP": 2, "FU": 1, "WR": 1,
    "SN": 10, "BR": 5, "AO": 1, "AD": 5, "OP": 2,
    # ---- INE (上海国际能源交易中心) ----
    "SC": 0.1, "BC": 10, "NR": 5, "LU": 1, "EC": 0.5,
    # ---- DCE (大连商品交易所) ----
    "A": 1, "B": 1, "M": 1, "Y": 2, "P": 2,
    "C": 1, "CS": 1, "L": 1, "V": 1, "PP": 1,
    "EB": 1, "EG": 1, "PG": 1, "I": 0.5,
    "J": 0.5, "JM": 0.5, "JD": 1,
    "BB": 0.05, "FB": 0.5, "BZ": 1,
    "LH": 5, "LG": 1, "RR": 1,
    # ---- CZCE (郑州商品交易所) ----
    "CF": 5, "SR": 1, "MA": 1, "TA": 2, "OI": 1,
    "RM": 1, "SA": 1, "FG": 1, "PF": 2, "SF": 2,
    "SM": 2, "UR": 1, "AP": 1, "CJ": 5, "CY": 5,
    "JR": 1, "WH": 1, "PM": 1, "RI": 1, "RS": 1,
    "PK": 2, "PX": 2, "SH": 1, "PR": 2, "PL": 1,
    "ZC": 0.2,
    # ---- GFEX (广州期货交易所) ----
    "SI": 5, "LC": 20, "PS": 5, "PD": 0.05, "PT": 0.05,
}
