...
_CANARY_DEPLOYMENT_CONFIG: Dict[str, Any] = {
    ...
    'canary_duration_sec': 3600,      # 灰度观察�?�?
    'rollback_on_error_rate': 0.05,   # 错误率超�?%自动回滚
    'metrics_to_monitor': [           # 灰度期间监控的指�?
...
def update_canary_config(**kwargs) -> None:
    ...
    for key, value in kwargs.items():
        if key in _CANARY_DEPLOYMENT_CONFIG:
            ...
            logging.warning("[ParamsService] P2修复: 未知灰度配置�? %s", key)
...
def hot_update_with_canary(params: Dict[str, Any],
    ...
    if not is_canary:
        logging.info(
            "[ParamsService] P2修复: 热更新跳�?非灰度品�? instrument=%s",
            instrument_id,
        )
