...
"""
config_loader.py �?合并�?config_json_loader / config_yaml_loader / config_option_loader
包含:
  [JSON加载/热重载]
...
# ============================================================================
# Section 1: JSON加载/热重�?(源自 config_json_loader.py)
# ============================================================================
...
def load_default_params_from_json(json_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not os.path.isfile(path):
        ...
        if os.path.isfile(_marker_file):
            logging.error("[R24-P2-DF-02] 参数JSON文件丢失(曾部�?: %s，回退到DEFAULT_PARAM_TABLE", path)
        else:
            logging.info("[R24-P2-DF-02] 参数JSON首次部署(文件不存�?: %s，使用DEFAULT_PARAM_TABLE", path)
            try:
    ...
    logging.info("[config_json_loader] 从JSON加载参数完成: %s，共 %d 个参�?, path, len(result))
...
def check_config_hot_reload() -> bool:
    ...
    try:
        ...
        if current_mtime > _config_file_watchdog_last_mtime:
            ...
            _config_file_watchdog_last_mtime = current_mtime
            reset_param_cache()
            logging.info("[R4-D-10] 配置热更�?%s 已变更，缓存已刷�?, path)
            try:
                ...
                logging.warning("SER-P1-14: 热重载后序列化格式验证失�? %s", _ser_err)
...
# ============================================================================
# Section 2: YAML加载与迁�?(源自 config_yaml_loader.py)
# ============================================================================

def _load_yaml_file(yaml_path: str) -> Dict[str, Any]:
    """P2-30修复: 统一使用yaml_safe_load，消除ruamel fallback绕过守卫的风�?""
    try:
...
def load_plr_thresholds() -> Dict[str, Any]:
    ...
    if not os.path.exists(_yaml_path):
        ...
        if os.path.exists(_fallback_path):
            ...
            logging.error("[ConfigService] R32-P2-09: param_configs.yaml 不存�?(param_pool/ �?参数�? 均未找到, PLR评判将无阈�?")
            return {}
    data = _load_yaml_file(_yaml_path)
...
def load_option_params_from_file() -> Dict[str, Any]:
    ...
    for file_path in possible_paths:
        ...
        try:
            ...
            logging.debug("[config_option_loader] 路径安全验证失败，跳过可选文�? %s", ve)
            continue
        try:
            ...
            logging.info("[config_option_loader] 总行权价组合�?%d", total_strikes)
            return data
    ...
    logging.info("[config_option_loader] 未找到可用的auto_generated_option_params.json，使用默认期权参�?)
    logging.debug("[R24-P2-DF-09] 期权参数为空，期权策略可能使用模块内默认�?)
    return {}
    ...
    if option_params:
        with _param_table_lock:
            ...
            if products_from_file:
                ...
                logging.info("[config_option_loader] 已合并期权品种列�?%d 个品�?, len(products_from_file))
                logging.info("[config_option_loader] 最终期权品�?%s", _OPTION_PARAM_RUNTIME_PATCH['option_products'])
...
# ============================================================================
# Section 4: 配置解析与优先级 (�?config_resolver.py)
# ============================================================================
    ...
    if load_json_fn is not None:
        try:
            ...
            if json_override_count > 0:
                logging.debug("[R4-D-01] JSON配置覆盖�?d 个参�?, json_override_count)
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as e:
            logging.warning("[R4-D-01] JSON配置加载失败，使用代码默认�?%s", e)
    ...
    for key in list(result.keys()):
        ...
        if env_val is not None:
            ...
            try:
                ...
                logging.warning("[R4-D-01] 环境变量 %s=%s 类型转换失败，保持原�?, env_key, env_val)

    if env_override_count > 0:
        logging.info("[R4-D-01] 环境变量覆盖�?d 个参�?, env_override_count)
...
_DOCUMENTED_ENV_VARS = {
    'PARAM_OVERRIDE_MAX_KLINE': {'description': '覆盖max_kline参数(最大K线数�?', 'default': '200', 'type': 'int'},
    ...
    'PARAM_OVERRIDE_DEBUG_MODE': {'description': '覆盖debug_mode参数(调试模式开�?', 'default': 'False', 'type': 'bool'},
    ...
    'PARAM_OVERRIDE_CIRCUIT_BREAKER_PAUSE_SEC': {'description': '覆盖circuit_breaker_pause_sec参数(断路器暂停时�?', 'default': '180.0', 'type': 'float'},
...
def apply_environment(params: Dict[str, Any], env: str) -> Dict[str, Any]:
    ...
    for key, value in env_configs.items():
        merged[key] = value
    logging.info("[config_resolver] 已应用环境覆�?%s，覆�?d 个参�?, env, len(env_configs))
    return merged
