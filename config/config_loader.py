...
from infra.serialization_utils import json_loads, yaml_safe_load

from infra.shared_utils import safe_int
...
def load_default_params_from_json(json_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    from infra.security import _validate_path_safety
...
def check_config_hot_reload() -> bool:
    ...
    from config.config_params import get_cached_params, reset_param_cache, _notify_param_change
...
def load_plr_thresholds() -> Dict[str, Any]:
    try:
        from param_pool import load_plr_thresholds_data
        data = load_plr_thresholds_data()
        if data:
            logging.info("[config_loader] P1-4: load_plr_thresholds() via param_pool API")
        return data
    except ImportError:
        logging.warning("[config_loader] param_pool.load_plr_thresholds_data 不可用，回退到直接读取YAML")
        _module_dir = os.path.dirname(os.path.abspath(__file__))
        _project_dir = os.path.dirname(_module_dir)
        _yaml_path = os.path.join(_project_dir, "param_pool", "param_configs.yaml")
        if not os.path.exists(_yaml_path):
            logging.error("[config_loader] R32-P2-09: param_configs.yaml 不存在, PLR评判将无阈值")
            return {}
        data = _load_yaml_file(_yaml_path)
        plr_data = data.get('plr_thresholds', data) if isinstance(data, dict) else data
        logging.info("[config_loader] P1-4: load_plr_thresholds() 加载成功, path=%s", _yaml_path)
        return plr_data
...
def merge_option_params_to_default():
    ...
    from config.config_params import DEFAULT_PARAM_TABLE, _param_table_lock, _OPTION_PARAM_RUNTIME_PATCH
...
def get_params_metadata(json_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    if json_path is None:
        ...
        try:
            ...
            from config.config_loader import _resolve_params_json_path
...
def validate_params_with_metadata(params: Dict[str, Any], json_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    constraint_rules = {
        ...
        'C2':  lambda p: __import__('config.config_params', fromlist=['validate_param_range']).validate_param_range('close_stop_loss_ratio', p.get('close_stop_loss_ratio', 0.3)),  # P1-57: 统一校验

        'C3':  lambda p: __import__('config.config_params', fromlist=['validate_param_range']).validate_param_range('max_risk_ratio', p.get('max_risk_ratio', 0.8)),  # P1-57: 统一校验
