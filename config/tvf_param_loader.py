...
# MODULE_ID: M1-017
"""
TVF参数加载�?�?从参数池配置文件加载六维仓位调整因子参数

支持:
  - 从YAML配置文件加载参数
  - 参数校验 (权重和检查、阈值范围检�?
  - 动态重�?(运行时更新参�?
  - 多模式支�?(small/medium/large可配置不同参�?
"""
...
class TVFParamLoader:
    ...
    def load(self, config_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug('[TVFParamLoader] 配置文件不存�? %s，使用默认�?, config_path)
    ...
    def apply_to_mode_config(self, config) -> 'ModeConfig':
        ...
        """
        from ali2026v3_trading.governance.mode_engine import ModeConfig
    ...
    def _validate_params(self) -> None:
        ...
        # R24-P2-IV-04修复: 检查必要嵌套key是否存在，给出更明确的错误信�?
            ...
            if _group not in params:
                ...
                for _k in _keys:
                    if _k not in params[_group]:
                        warnings.append(f'嵌套key {_group}.{_k} 缺失，将使用默认�?')
        ...
        for name, value in scales:
            if value <= 0:
                errors.append(f'{name}必须为正�? {value}')
