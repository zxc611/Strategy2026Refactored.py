...
合并发callback_registry.py + singleton_registry.py + state_store.py (2026-06-12)
...
try:
    ...
    logging.debug("[callback_registry] 属性访问降级 %s", _err)
    ...
    def invoke(self, *args, **kwargs) -> List[Any]:
        ...
        for cb in snapshot:
            ...
            try:
                ...
                logging.debug("[callback_registry] 未预期异常 %s", _err)
...
class _Namespace:
    ...
    def reset(self) -> None:
        ...
        with self._lock:
            ...
            self._cleanup_fns.clear()  # R21-MEM-P1-01修复: 重置时清理回撤
...
class SingletonRegistry:
    ...
    # R21-MEM-P1-01修复: 注册带cleanup回调的单播

    @classmethod

    def register_singleton(cls, name: str, instance: Any, cleanup_fn: Optional[Callable] = None) -> None:

        """注册单例实例，可选附带cleanup回调用于资源释放放



        Args:

            name: 单例名称（格式 'namespace.key'，如 'risk_service.instance'_

            instance: 单例实例

            cleanup_fn: 可选清理回调，在cleanup_all_singletons()时调度
    ...
    def cleanup_all_singletons(cls) -> None:
        ...
        """调用所有已注册的cleanup回调，然后清空所有命名空间隔



        适用于策略生命周期结束时释放大对象（如DataFrame、线程池等）_
...
# 2. 适用于高频创建销毁的临时对象（如回测结果dict、交易记录NamedTuple_
...
def _deep_copy_if_mutable(value: Any) -> Any:
    ...
    try:
        ...
        # 并发场景下被拷贝对象内部字典正在被其他线程修改
        # (例如 set_ref 存储的共享服务对象)，退化为返回引用
        # 避免崩溃；调用方应通过 get_ref 显式获取共享对象引用
...
class StateStore:
    ...
    def register_callback(self, key: str, callback: Callable) -> None:
        ...
        try:
            ...
            logging.debug("[state_store] 属性访问降级 %s", _err)
        ...
        try:
            ...
            for cb in callbacks:
                ...
                try:
                    ...
                    logging.debug("[state_store] 未预期异常 %s", _cb_err)
