...
from ali2026v3_trading.infra.shared_utils import UNIFIED_EXPIRY_SLIPPAGE_MULTIPLIERS
...
from ali2026v3_trading.infra.shared_utils import DEFAULT_RISK_FREE_RATE, DAYS_PER_YEAR_CALENDAR
from ali2026v3_trading.governance.greeks_calculator import _norm_cdf as _bs_norm_cdf
...
def calc_trade_fee(contract_type: str, open_time: pd.Timestamp, close_time: pd.Timestamp,
    ...
    try:
        from ali2026v3_trading.infra.commission_utils import calc_trade_fee as _calc_trade_fee
...
"""
from ali2026v3_trading.infra.commission_utils import (
    P0_IRON_RULES,
    REASON_MULTIPLIERS,
...
from ali2026v3_trading.param_pool._param_grids import _BacktestPosition, _BacktestState
from ali2026v3_trading.param_pool._param_grids import (
...
def __getattr__(name):
    ...
    if name in ('_prepare_df_for_subprocess', '_worker_init', 'cleanup_global_data',
                '_worker_task', 'run_cycle_resonance_backtest_sweep', 'run_cr_params_sweep'):
        from ali2026v3_trading.param_pool import ts_backtest_strategies as _mod
        val = getattr(_mod, name)
                  ...
                  '_execute_round', '_validate_params_via_params_service', 'build_and_save_life_dict',
                  'main_scheduler'):
        from ali2026v3_trading.param_pool import ts_result_writer as _mod
        val = getattr(_mod, name)
    elif name in ('check_l2_conflict', 'compute_alpha_confidence_interval',
                  'optimize_l2_params_step1', 'run_step2_smoke_test'):
        from ali2026v3_trading.param_pool import validation_l2_hyperparams as _mod
        val = getattr(_mod, name)
    elif name in ('run_deep_validation_tiered', 'validate_doomed_tests',
                  'validate_market_friendliness_baseline'):
        from ali2026v3_trading.param_pool import validation_deep_orchestrator as _mod
        val = getattr(_mod, name)
    elif name in ('_select_top_k_train', 'validate_default_values_in_grids',
                  'validate_shadow_param_independence', '_check_two_stage_stop',
                  '_BacktestPosition', '_BacktestState', '_try_open'):
        from ali2026v3_trading.param_pool import backtest_runner_base as _mod
        val = getattr(_mod, name)
    else:
        raise AttributeError(f"module {__name__!r} has no attribute {name!r}")
    globals()[name] = val
