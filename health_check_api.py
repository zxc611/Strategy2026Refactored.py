...
class StructuredJsonlLogger:
    ...
    def log_signal(self, signal: Dict[str, Any], strategy_id: Optional[str] = None) -> None:
        ...
        safe_jsonl_append_line(self._signal_log_f, record)  # R14-P1-LOG-14修复: 使用缓存句柄 + P1-16截断保护
    ...
    def log_order(self, order: Dict[str, Any]) -> None:
        ...
        self._rotate_if_needed(self._order_log_path)
        safe_jsonl_append_line(self._order_log_f, record)  # R14-P1-LOG-14修复 + P1-16截断保护
...
class HealthCheckAPI:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        cfg = config or {}
        self._start_time = datetime.now(CHINA_TZ)  # [R27-AUDIT] P2修复: 使用CHINA_TZ常量替代内联时区
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "timestamp": now,
            "uptime_seconds": (datetime.now(CHINA_TZ) - self._start_time).total_seconds(),  # [R27-AUDIT] P2修复
        ...
        # P2-3修复: 僵尸线程检测
        try:
            from ali2026v3_trading.shared_utils import ThreadMgr
            # ThreadMgr没有instance()单例，通过HealthMonitor的全局单例间接获取
        ...
        # R33-P2-7修复: 集成UIDiagnosisTool到健康检查
        try:
            from ali2026v3_trading.ui_service import UIDiagnosisTool
...
class OperationsAPI:
    ...
    def __init__(self):
        ...
        # R21-OPS-18/20: 集成容量压测框架和SOP框架
        try:
            from ali2026v3_trading.maintenance_service import get_capacity_stress_test, get_sop_manager
    ...
    def run_diagnostics(self, scope: str = 'all') -> Dict[str, Any]:
        ...
        try:
            ...
            if scope in ('all', 'system'):
                import psutil  # R17-P2-DEP-02: psutil更新日志信息泄漏风险，避免在日志中输出psutil版本/路径等内部信息
        ...
        # R33-P2-7修复: 集成UIDiagnosisTool到运维诊断
        try:
            from ali2026v3_trading.ui_service import UIDiagnosisTool
