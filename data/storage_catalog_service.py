...
from infra.shared_utils import sanitize_sql_identifier, sanitize_sql_value, CHINA_TZ
...
try:
    from data.data_service import get_data_service, _HAS_DATA_SERVICE
except ImportError:
...
class StorageCatalogService:
    ...
    def drop_empty_instrument_tables(self) -> int:
        ...
        # FIX-P0-15: 启动保护 - 防止在 tick 到达前误删所有合约记录
        # 该函数会删除 ticks_raw/klines_raw 中无数据的合约记录，
        # 若在启动阶段调用（tick 尚未推送），所有合约记录都会被误删，
        # 导致后续 tick 找不到 info 而被永久丢弃。
        import time as _time_mod
        _startup_guard_key = '_drop_empty_guard_until'
        _guard_until = getattr(self, _startup_guard_key, 0)
        _now = _time_mod.time()
        if _now < _guard_until:
            logging.warning(
                "[FIX-P0-15] drop_empty_instrument_tables 在启动保护期内被调用(剩余%d秒), 跳过执行以防止误删合约记录",
                int(_guard_until - _now),
            )
            return 0
