...
class HistoricalDataManager:
    ...
    def load_historical_klines_once(self, instruments: List[str], provider: Any, provider_source: str) -> None:
        ...
        from strategy.strategy_historical import load_historical_klines_with_stop
    ...
    def shutdown_historical_services(self) -> None:
        ...
        alive_threads = []
        # FIX-20260715-PATH-A: 延长 join 超时 3s→5s
        # 根因: 实盘运行时 hkl-retry/kline-load-tick-fallback 可能正在执行重试/等待，
        #       3s 不足以等待其退出 → pause/on_stop 后线程仍存活
        # 修复: join 超时延长至 5s，与 pause 循环超时对齐
        ...
        if alive_threads:
            ...
            logging.info(
                f"[HKL][strategy_id={strategy_id}] All background historical threads exited cleanly"
            )
