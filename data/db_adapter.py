...
# FIX-20260702: 本地化 sanitize，切断 db_adapter -> shared_utils -> _helpers -> shared_utils 循环导入
def _sanitize_sql_identifier(identifier: str) -> str:
    """本地清理SQL标识符，防止注入并避免对 shared_utils 的顶层依赖。"""
...
def connect(db_path: str, read_only: bool = False) -> Any:
    ...
    FIX-20260702: 同进程对同一 DuckDB 文件混用 read-only/read-write 会触发
    `_duckdb.cp312-win_amd64.pyd` 原生崩溃(0xc0000409)，因此 read_only=True
    统一降权为读写连接。
    """
    import duckdb
    if read_only:
        logger.debug("db_adapter.connect read_only=True ignored; using read-write mode")
...
def get_duckdb_connection_type() -> Any:
    ...
    Usage:
        from data.db_adapter import get_duckdb_connection_type
...
def execute_parquet_query(conn: Any, parquet_path: str, query: str,
    ...
    """
    full_query = f"SELECT * FROM read_parquet({_sanitize_sql_value(parquet_path)}) AS {_sanitize_sql_identifier(table_alias)}"
    if query.strip().upper().startswith("SELECT"):
        subquery = query.strip()
        combined = f"WITH {_sanitize_sql_identifier(table_alias)} AS (SELECT * FROM read_parquet({_sanitize_sql_value(parquet_path)})) {subquery}"
        return execute(conn, combined)
    return execute(conn, full_query)


def get_db_adapter(config: ConnectionConfig = None, pool_key: str = "default") -> Any:
    """Get a database adapter (pooled connection) [convenience alias].

    Alias for get_connection_pool(). Preferred entry point for business
    modules that just need a connection without caring about pool details.
    """
    return get_connection_pool(config=config, pool_key=pool_key)
...
def manage_schema(conn: Any, table_name: str, columns: Dict[str, str],
    ...
    if exists:
        return
    cols_def = ", ".join(f"{_sanitize_sql_identifier(col)} {dtype}" for col, dtype in columns.items())
    clause = "IF NOT EXISTS" if if_not_exists else ""
    sql = f"CREATE TABLE {clause} {_sanitize_sql_identifier(table_name)} ({cols_def})"
    execute(conn, sql)
