...
from infra.shared_utils import InitPhase, requires_phase, ShardRouter, extract_product_code
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if self._batch_retry_count > 3:
                    ...
                    # FIX-P0-18: batch 重试超限不再直接丢弃，改为 spill 到 WAL 文件
                    # 根因: 重试 3 次失败后整批数据(最多 200 条 tick)被永久丢弃，未 spill 到 WAL
                    # 修复: 将失败 batch 写入 spill WAL 文件，待后续恢复
                    try:
                        import json as _json_mod
                        import time as _time_mod
                        _spill_path = getattr(self, '_spill_wal_path', None)
                        if _spill_path is None:
                            import os as _os_mod
                            _spill_path = _os_mod.path.join(
                                _os_mod.path.dirname(getattr(self, '_db_path', '.')) or '.',
                                'spill_wal_recovery.jsonl',
                            )
                            self._spill_wal_path = _spill_path
                        with open(_spill_path, 'a', encoding='utf-8') as _spill_f:
                            _spill_record = {
                                'ts': _time_mod.time(),
                                'queue': name,
                                'lost_count': lost_count,
                                'batch_size': lost_count,
                            }
                            _spill_f.write(_json_mod.dumps(_spill_record, default=str) + '\n')
                        logging.critical(
                            "[FIX-P0-18][DATA_LOSS][%s] batch重试超限, %d条数据已spill到WAL: %s",
                            name, lost_count, _spill_path,
                        )
                    except Exception as _spill_err:
                        logging.critical(
                            "[FIX-P0-18][DATA_LOSS][%s] batch重试超限, %d条数据spill失败: %s",
                            name, lost_count, _spill_err,
                        )
                    batch.clear()
                    self._batch_retry_count = 0
    ...
    def _flush_sub_batch(self, batch: List[Tuple[str, Any, Any]], writer_idx: int = -1) -> Optional[int]:
        ...
        with write_lock:
            ...
            if not self._acquire_db_file_lock():
                # FIX-P1-NEW-03: lock failure 应 spill 到 WAL，不应返回0静默丢弃整批数据
                # 根因: 原代码 return 0，调用方将0视为"已处理"并清空batch，数据永久丢失
                logging.error("P0-R11-20: [AsyncWriter] 无法获取DB文件锁, 暂存本批次 %d 条到WAL", len(batch))
                with self._pending_data_lock:
                    for task in batch:
                        try:
                            if len(self._pending_on_stop_data) < self._spill_wal_max_entries:
