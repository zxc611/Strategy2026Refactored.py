...
class SortEntry:
    ...
    current_status: str = field(compare=False, default='other')
    ...
    def create(internal_id: int, instrument_id: str, option_type: str,
               ...
               last_tick_ts: float = 0.0, current_status: str = 'other') -> 'SortEntry':
        if option_type == 'CALL':
...
try:
    from infra.event_bus import KLineEvent as EventBusKLineEvent
except ImportError:
...
class WidthCacheDiagnosisMixin:
    ...
    def _get_tick_entry_snapshot(self) -> Dict[str, Any]:
        try:
            from infra import health_monitor as hm
