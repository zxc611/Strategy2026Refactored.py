# MODULE_ID: M1-056-SA
"""
signal_source_a.py — 信号源A：单品种月份排序（IntraProductSorter）

依据文档：docs/audit/三层期权五态排序方案_最终落地方案_20260624.md 第三章

职责：读取单一品种下挂期权五态数据，按月份加权排序，输出最优月份
...
class IntraProductSorter:
    ...
    def determine_tier(
        ...
        if wilson >= TIER1_WILSON_THRESHOLD and coverage >= TIER2_COVERAGE_THRESHOLD:
            return 1
        if correct_up_pct >= TIER2_CORRECT_UP_THRESHOLD and coverage >= TIER2_COVERAGE_THRESHOLD:
            return 2
