...
class WidthCacheStateService:
    ...
    def _classify_status(self, underlying_future_id: int, month: str, opt_type: str,
        ...
        # FIX-10 RC-12: 期货rising=None时不再直接返回'other'，改为用期权自身价格变化作回退分类
        # 根因: 期货price<=0或tick延迟时_future_rising被设置为None，导致80%期权被分类为'other'
        #       进而coverage=0.2（1个月分类数据/MAX_MONTHS=5），形成80%无法有效分类的死循环。
        # 修复: 期货方向未知时，用期权自身价格变化推断option_direction，并假设与期货同步进行分类
        #       （CALL涨→correct_rise, CALL跌→correct_fall, PUT涨→correct_rise, PUT跌→correct_fall）
        #       这避免了80%的'other'分类，同时保留五态分类的语义结构。
        future_rising_raw = future_state.get('rising')
        if future_rising_raw is None:
            # 期货方向未知，用期权自身价格变化作回退
            ...
            # 假设期权方向与期货同步（期权涨→期货涨, 期权跌→期货跌）
            if option_direction is not None:
                future_rising = bool(option_direction)
                option_rising = option_direction
                # 简化分类：假设同步，期权涨→correct_rise, 期权跌→correct_fall
                if option_rising:
                    return 'correct_rise'
                else:
                    return 'correct_fall'
        ...
        future_rising = bool(future_rising_raw)
    ...
    def _compute_sync_flag(self, info: Dict[str, Any], current_price: float,
        ...
        # FIX-G R12-5: 期货rising=None时三级回退（与_classify_status FIX-10 RC-12保持一致）
        # 根因（彻底版）: 期货price<=0时_future_rising=None（见L739），期权首tick时option_direction=None
        #       （见_update_option_direction L476-477），二者同时成立导致return False→
        #       _sync_otm_count永不增加→width_resonance=0→SPM._last_resonance_strength=0→
        #       FIX-S读取值始终为0→HFT追击信号弱化→FIX-DD TTL更新无targets→0消费
        # 修复: 三级回退，与_classify_status的FIX-10 RC-12逻辑统一（原则2四唯一）：
        #   1) option_direction已知 → 用它作future_rising回退
        #   2) option_direction未知但期权价格变化方向已知 → 用价格变化推断option_direction
        #   3) 三级回退都失败才return False
        _future_rising_raw = future_state.get('rising')
        if _future_rising_raw is None:
            if option_direction is None:
                # 三级回退-2: 期权价格变化推断方向（与_classify_status L361-366一致）
                _opt_internal_id = info.get('internal_id')
                _opt_prev_price = self._option_prev_price.get(_opt_internal_id, 0.0) if _opt_internal_id is not None else 0.0
                if current_price > 0 and _opt_prev_price > 0:
                    if current_price > _opt_prev_price:
                        option_direction = True
        ...
        # FIX-20 RC-24: 首个有效tick时期权方向未知，用期货方向作回退默认值
        # 根因: _update_option_direction首个tick返回None→_compute_sync_flag返回False
        #       →_sync_otm_count永不增加→width_resonance=0→resonance_strength=0→S1 0信号
        # 修复: CALL用future_rising作默认方向，PUT用not future_rising（期权Delta特性）
        # 注: future_rising已在上方的FIX-G代码块中设置
        if option_direction is None:
            option_direction = future_rising if opt_type == 'CALL' else (not future_rising)

        future_price = future_state['price']
        # FIX-V R7-2: future_price<=0守卫，防止零价tick导致CALL被误判OTM
        # 根因: FIX-G使future_rising=None时继续执行到此，但future_price可能为0.0
        #       →_is_out_of_the_money('CALL', 0.0, strike)→0<strike→True→误计_sync_otm_count
