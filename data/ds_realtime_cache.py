...
try:
    from ali2026v3_trading.config.config_params import _MISSING_VALUE_DEFAULTS
except ImportError:
...
class RealTimeCache:
    ...
    def __init__(self, max_recent_ticks: int = 100, wal_path=None, flush_windows=None,
        ...
        # FIX-P2-2: 缓存预热 — 初始化时标记活跃合约的缓存时间戳，避免首次查询miss
        # 证据: 初始化时缓存命中率66.20% (hits=662, misses=338)
        # 根因: 未预热的合约首次查询时_cache_timestamps为空，导致cache miss
    ...
    # FIX-P2-2: 缓存预热方法 — 注册合约后标记缓存时间戳
    def warmup_cache(self, instrument_ids: List[str]) -> int:
        if self._warmup_done or not instrument_ids:
            return 0
        _now = time.time()
        _count = 0
        with self._lock:
            for inst_id in instrument_ids:
                if inst_id not in self._cache_timestamps:
                    self._cache_timestamps[inst_id] = _now
                    _count += 1
        self._warmup_done = True
        if _count > 0:
            logging.info("[FIX-P2-2] 缓存预热完成: %d个合约已标记缓存时间戳", _count)
        return _count
        ...
        with self._get_shard_lock(symbol):
            ...
            if tick is not None:
                ...
                if (time.time() - cache_ts) > self._cache_ttl_sec:
                    # FIX-R37-CACHE-NOISE: 不活跃合约缓存过期是预期行为，降为debug避免日志噪音
                    logging.debug("[R23-FR-02-FIX] 缓存过期: symbol=%s age=%.1fs > ttl=%.1fs",
