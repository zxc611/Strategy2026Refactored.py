...
class AnalyticsService:
    ...
    def __init__(self, params: Optional[ParamsProtocol] = None,
        ...
        # 期权合约数据（外部注入）
        self.option_instruments: Dict[str, List[Any]] = {}
        self.future_to_option_map: Dict[str, str] = {}
    ...
    def _get_option_group_options(self, future_id: str) -> List[Any]:
        ...
        try:
            ...
            fut_u = str(fut).upper()
            opts_u = {str(k).upper(): v for k, v in self.option_instruments.items()}
            ...
            # 产品映射查找
            m = re.match(r"^([A-Z]+)(\d{3,4})$", fut_u)
            if m:
                prod = m.group(1)
                suffix = m.group(2)
                opt_prod = str(self.future_to_option_map.get(prod, "") or "").upper()
                if opt_prod:
                    mapped_id = f"{opt_prod}{suffix}"
                    if mapped_id in opts_u:
                        return opts_u[mapped_id]
    ...
    def _resolve_calc_flag(self, flag_name: str, legacy_flag: str, default: bool = False) -> bool:
        """解析计算层月份过滤标志。"""
        try:
    ...
    def _extract_product_code(self, symbol: str) -> str:
        """提取品种前缀。"""
        norm = self._normalize_future_id(symbol)
        match = re.match(r"^([A-Z]+)", norm)
        return match.group(1) if match else ""

    def _to_option_group_id(self, future_symbol: str) -> str:
        """将期货月份ID映射为期权分组ID，如 IF2604 -> IO2604。"""
        norm = self._normalize_future_id(future_symbol)
        match = re.match(r"^([A-Z]+)(\d{3,4})$", norm)
        if not match:
            return norm
        product = match.group(1)
        suffix = match.group(2)
        option_product = str(self.future_to_option_map.get(product, "") or "").upper()
    ...
    def _get_month_bucket(self, symbol: str) -> Optional[str]:
        ...
        reverse_map = {
            str(opt).strip().upper(): str(fut).strip().upper()
            for fut, opt in (self.future_to_option_map or {}).items()
            if str(opt).strip()
        }
        ...
        for candidate_product in candidate_products:
            ...
            specified_id = self._normalize_future_id(str(entry[0]))
            next_id = self._normalize_future_id(str(entry[1]))
    ...
    def _normalize_future_id(self, future_id: str) -> str:
        """标准化期货合约代码"""
        if not future_id:
            return ""
        val = str(future_id).strip().upper()
        if "|" in val:
            return val.split("|")[-1]
        return val
    ...
    def _get_option_type(self, option: Any) -> str:
        ...
        try:
            if isinstance(option, dict):
                opt_type = option.get("OptionType") or option.get("OptionsType") or ""
                return str(opt_type).strip().upper()
            return str(getattr(option, "option_type", "") or "").strip().upper()
        except Exception as e:
    ...
    def _get_strike_price(self, option: Any) -> float:
        ...
        try:
            if isinstance(option, dict):
                for key in ['StrikePrice', 'strike', 'strike_price', 'Strike']:
                    if key in option:
                        return safe_float(option[key], 0.0)
            return safe_float(
                getattr(option, 'strike_price', 0) or
                getattr(option, 'strike', 0) or
                getattr(option, 'StrikePrice', 0), 0.0
            )
        except Exception as e:
            ...
            if isinstance(option, dict):
                for key in ['last', 'last_price', 'close', 'price']:
                    if key in option:
                        return safe_float(option[key], 0.0)
            return safe_float(
                getattr(option, 'last', 0) or
                getattr(option, 'close', 0) or
                getattr(option, 'price', 0), 0.0
            )
        except Exception as e:
    ...
    def set_future_to_option_map(self, mapping: Dict[str, str]) -> None:
        """设置期货到期权产品映射"""
        self.future_to_option_map = mapping

    def get_stats(self) -> Dict[str, Any]:
        """获取计算统计信息"""
        with self._calc_lock:
            return dict(self._calc_stats)
    ...
    def _clean_expired_cache(self) -> None:
        ...
        with self._calc_lock:
            # 清理option_type_cache
            expired_keys = [
                k for k, (_, timestamp) in self._option_type_cache.items()
                if now - timestamp > self._cache_ttl
            ]
            for key in expired_keys:
                del self._option_type_cache[key]
            
            # 清理out_of_money_cache
            expired_keys = [
                k for k, (_, timestamp) in self._out_of_money_cache.items()
                if now - timestamp > self._cache_ttl
            ]
            for key in expired_keys:
