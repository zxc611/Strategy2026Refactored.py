...
class E7UnexplainedReturnChecker:
    ...
    def check(self, pnl_attribution: Dict[str, float]) -> Dict[str, Any]:
        ...
        denominator = explained_pnl if abs(explained_pnl) > 1e-10 else total_pnl
...
class ParameterDriftDetector:
    ...
    def detect_drift(self, param_history: List[Dict[str, float]]) -> Dict[str, Any]:
...
class StateEDensityDecayTracker:
    ...
    def track(self, state_e_count: int = 0, total_state_count: int = 0,
              day: int = 0, strategy_id: str = "", state_data: Any = None) -> Dict[str, Any]:
        if _HAS_EVALUATION_CLASSES and strategy_id and state_data is not None:
            try:
                _eval_inst = _EvalStateEDensityDecayTracker()
                return _eval_inst.track(strategy_id, state_data)
            except Exception:
                pass
