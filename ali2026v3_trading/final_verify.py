...
print("\nP0-4: 手续费5维模型")
try:
    from 参数池.task_scheduler import _compute_commission, FEE_STRUCTURE
    # 验证品种区分
...
print("\nP0-5: 滑点固定1bps")
try:
    from 参数池.task_scheduler import SLIPPAGE_BPS
...
print("\nP0-7: 合约换月成本")
try:
    from 参数池.task_scheduler import detect_rollover_gaps, compute_rollover_cost
...
print("\nP0-10: 日最大回撤硬停止")
try:
    from 参数池.task_scheduler import _check_positions
...
print("\nP0-14: intuition参数自动拦截")
try:
    from 参数池.task_scheduler import _run_final_checks
    import inspect
...
print("\nP0-16: LHS采样率")
try:
    from 参数池.optuna_multiobjective_search import latin_hypercube_sample
    import numpy as np
    _n_dims = 35
...
print("\nP0-17: state_confirm_bars")
try:
    from 参数池.task_scheduler import PARAM_DEFAULTS
...
print("\nQ1: other状态开仓阻断")
try:
    from 参数池.task_scheduler import run_backtest
    import inspect
...
try:
    with open('risk_service.py', 'r', encoding='utf-8') as f:
        src = f.read()
    # 搜索safe_get_float调用
    has_03 = 'safe_get_float' in src and '0.3' in src
    print(f"  max_risk_ratio默认值=0.3: {has_03}")
    results['Q2'] = 'FIXED' if has_03 else 'REMAINING'
except Exception as e:
    print(f"  [ERROR] {e}")
    results['Q2'] = 'UNKNOWN'
...
print("\nN1: 质量标记消费")
try:
    from 参数池.task_scheduler import _try_open, _check_positions
    import inspect
