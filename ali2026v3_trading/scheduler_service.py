...
class SchedulerService:
    ...
    def cancel_job(self, job_id: str) -> bool:
        ...
        with self._lock:
            ...
            if job_id in self._once_jobs:
                self._once_jobs[job_id].cancelled = True
                return True
            return False
    ...
    def add_once_job(self, func: Callable, delay: float,
        ...
        def _delayed_run():
            elapsed = 0.0
            step = 0.5
            while elapsed < delay and self._running and not job_info.cancelled:
    ...
    def cancel_once_job(self, job_id: str) -> bool:
        """取消一次性任务"""
        with self._lock:
            if job_id in self._once_jobs:
                self._once_jobs[job_id].cancelled = True
...
def is_trading_day(target_date: datetime.date, holiday_dates: Optional[set] = None) -> bool:
    """
    判断是否为交易日
    ...
    if target_date.weekday() >= 5:  # 周末
        return False
    if holiday_dates and target_date in holiday_dates:  # 节假日
        return False
    return True
...
def is_market_open_safe(obj: object) -> bool:
    """
    安全判断是否开盘；遵从"最长开盘时间"原则（任一配置的交易所开盘即视为开盘）
    ...
    """
    try:
        # 检查对象自身是否有 is_market_open 方法
        fn = getattr(obj, "is_market_open", None)
        if callable(fn):
            return bool(fn())
    except Exception as e:
        logging.error(f"[SchedulerService.is_market_open_safe] Error: {e}")
    ...
    try:
        ...
        now_dt = datetime.now()
        holidays = set()  # TODO: 从配置加载
        
        # 遍历所有交易所，只要有一个处于交易时段，即视为开盘
        # TODO: 需要实现 get_trading_sessions 功能
        for exch in exchanges:
            pass  # Placeholder
    except Exception as e:
        logging.error(f"[SchedulerService.is_market_open_safe] Exception: {e}")
...
def minutes_to_next_open(
    ...
    for day_offset in range(0, 8):  # 检查未来 8 天
    ...
    if not candidates:
        logging.error(f"[SchedulerService.minutes_to_next_open] No valid trading sessions found")
        return 1440.0  # 默认返回 24 小时
