...
# OPTIONAL-DEPENDENCY: concurrent_log_handler用于高并发日志写入优化
try:
...
class AuditLogger:
    ...
    def log(self, event_type: str, detail: dict) -> str:
        ...
        detail_json = _json.dumps(detail, ensure_ascii=False, sort_keys=True)
...
class DatabaseConfig:
    ...
    db_type: str = "duckdb"
    db_path: str = str(Path(__file__).parent.parent / "data" / "strategy.duckdb")
    connection_pool_size: int = 5  # P1-3修复: 保留字段，DuckDB单连接模型下不生效；仅作接口兼容预留
    ...
    def __post_init__(self):
        if self.connection_pool_size != 5:
            logging.warning(
                "[ConfigService] P1-3: connection_pool_size=%d 在DuckDB单连接模型下不生效，"
                "该字段为接口兼容保留，实际连接池大小由DuckDB引擎决定",
...
def load_plr_thresholds() -> Dict[str, Any]:
    ...
    if not os.path.exists(_yaml_path):
        _fallback_path = os.path.join(_module_dir, "参数池", "plr_thresholds.yaml")
