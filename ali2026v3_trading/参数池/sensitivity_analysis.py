    ...
    def save_to_duckdb(
        ...
        try:
            ...
            for sr in results:
                d = sr.to_dict()
                con.execute(
                    f"INSERT INTO {safe_table_name} VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",  # P2-10: 预留灵敏度分析查询表，暂无业务SELECT消费
                    [
...
# P2-R8-08修复: SensitivityAnalyzer单例模式 — 避免多重实例化创建重复DuckDB连接
_sensitivity_analyzer_instance: Optional[SensitivityAnalyzer] = None
...
def get_sensitivity_analyzer(
    ...
    """P2-R8-08: SensitivityAnalyzer单例工厂 — 全局唯一实例，避免重复DuckDB连接"""
    global _sensitivity_analyzer_instance
    with _sensitivity_analyzer_lock:
        if _sensitivity_analyzer_instance is None:
            import threading as _thr_sa
            _sensitivity_analyzer_instance = SensitivityAnalyzer(
                db_path=db_path,
        ...
        return _sensitivity_analyzer_instance


def reset_sensitivity_analyzer() -> None:
    """P2-R8-08: 重置单例（用于测试或配置变更）"""
    global _sensitivity_analyzer_instance
    with _sensitivity_analyzer_lock:
        _sensitivity_analyzer_instance = None
