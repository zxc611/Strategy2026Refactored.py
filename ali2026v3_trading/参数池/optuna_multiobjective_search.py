...
参数空间对齐 task_scheduler.py 的PARAM_GRID_ROUND1 + PARAM_GRID_ROUND2 + PULLBACK_GRID)共6+13+10=29维连续+离散混合空间(含IV边界+max_valid_bars+策略分层时间参数+daily_loss_hard_stop_pct),5000次TPE采样可覆盖等效1M网格的帕累托前沿。
修复清单:
    ...
    def objective(trial: optuna.Trial) -> Tuple[float, float, float]:
        ...
        if enable_judgment:
            try:
                ...
                from 策略评判.strategy_judgment_engine import JudgmentVerdict
...
def main():
    ...
    # T-06修复: 将LHS采样维度扩展至全参数空间(27维,而非7维)
    _n_dims_for_lhs = 27  # T-06修复: 修正维度声明, 对齐实际参数空间(17核心+辅助+10pullback)
    ...
    try:
        ...
        # LHS采样点数由# LHS采样点数由lhs_n_startup=max(50,n_dims*3,n_trials//5)决定
        # 当n_trials=5000时 lhs_n_startup=1000, 远超n_dims=27, 确保每个维度有充分覆盖
        # 维度数(len(param_ranges_for_lhs)=27), 对齐完整参数空间(17核心+辅助+10pullback)
        # T-06修复后维度从7扩展至27, 确保全参数空间覆盖
    except Exception as e:
