...
# R31-P0-07修复标记: 此模块为P0级死代码 — 全仓零生产引用
# 仅被test_l1_l2_pipeline.py测试文件引用
# 待决策: 集成到回测引擎tick持久化管线 或 删除
import warnings
warnings.warn(
    "duckdb_tick_storage.DuckDBTickStorage is dead code (P0-07): "
    "zero production references. Pending integration or removal decision.",
    DeprecationWarning,
    stacklevel=2,
)
...
def _resolve_db_path(db_path: str) -> str:
    ...
    """
    env_pattern = re.compile(r'ENV:(\w+)', re.IGNORECASE)
    resolved = db_path
...
class DuckDBTickStorage:
    ...
    SCHEMA_MINUTE_BARS = '''
        CREATE MATERIALIZED VIEW IF NOT EXISTS mv_minute_bars AS
        SELECT
            symbol,
    ...
    SCHEMA_DAILY_BARS = '''
        CREATE MATERIALIZED VIEW IF NOT EXISTS mv_daily_bars AS
        SELECT
            symbol,
            trade_date,
            first(price) AS open,
    ...
    def _init_tables(self) -> None:
        try:
            ...
            try:
                ...
                logger.debug("分钟Bar物化视图已存在")
            try:
                self._conn.execute(self.SCHEMA_DAILY_BARS)
            except Exception:
                logger.debug("日Bar物化视图已存在")
            self._conn.execute('''
