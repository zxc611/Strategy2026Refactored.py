...
# EC-P2-12: multiprocessing freeze_support for Windows
...
class L2Optimizer:
    ...
    def evaluate_state_accuracy(self, params: Dict[str, Any],
        ...
        confirm_bars = params.get('state_confirm_bars', 5)
        ...
        # P1-R8-05修复: 优先接入真实状态诊断(state_param_manager)
        _use_real_diagnosis = False
        try:
            from ali2026v3_trading.state_param_manager import get_state_param_manager
            _spm = get_state_param_manager()
            if _spm is not None and hasattr(_spm, 'update_state_from_width_cache'):
                _use_real_diagnosis = True
        except Exception:
            pass
        ...
        if bar_data is not None and hasattr(bar_data, '__len__'):
            ...
            for i in range(confirm_bars, n - effective_lookahead):
                try:
                    ...
                    if _use_real_diagnosis:
                        try:
                            raw_state = _spm.update_state_from_width_cache()
                            _STATE_MAP = {
                                ...
                                'STRATEGY_MODE_INCORRECT_REVERSAL_DEFENSIVE': 'incorrect_reversal_defensive',
                                'STRATEGY_MODE_OTHER': 'other',
                            }
                            state = _STATE_MAP.get(raw_state, 'other')
                        except Exception:
                            ...
                            if non_other_ratio >= non_other_threshold:
                                state = 'correct_trending_defensive' if price_direction == 'fall' else 'correct_trending'
                            elif non_other_ratio < (1 - non_other_threshold):
                                state = 'incorrect_reversal_defensive' if price_direction == 'fall' else 'incorrect_reversal'
                            else:
                                state = 'other'
                    else:
                        non_other_ratio = getattr(row, 'non_other_ratio', 0.5)
        ...
        return {
            ...
            'diagnosis_source': 'state_param_manager' if _use_real_diagnosis else 'simplified_inline',
        }

    def check_l2_statistical_power(self, bar_data: Any = None) -> Dict[str, Any]:
        """P1-R8-06修复: 增强统计功效检查，包含regime切换和跨fold重叠度"""
        if bar_data is None:
            return {'power_sufficient': False, 'reason': 'no_data'}
        try:
            n = len(bar_data) if hasattr(bar_data, '__len__') else 0
            length_ok = n >= self._min_state_transitions
            if not length_ok:
                return {'power_sufficient': False, 'reason': f'n={n}<{self._min_state_transitions}'}
            regime_transitions = 0
            if hasattr(bar_data, 'columns') and 'state' in getattr(bar_data, 'columns', []):
                states = bar_data['state'].values if hasattr(bar_data, 'values') else list(bar_data['state'])
                for i in range(1, len(states)):
                    if states[i] != states[i-1]:
                        regime_transitions += 1
            elif hasattr(bar_data, 'columns') and 'non_other_ratio' in getattr(bar_data, 'columns', []):
                nor = bar_data['non_other_ratio'].values if hasattr(bar_data, 'values') else list(bar_data['non_other_ratio'])
                _threshold = 0.5
                for i in range(1, len(nor)):
                    if (nor[i] >= _threshold) != (nor[i-1] >= _threshold):
                        regime_transitions += 1
            regime_ok = regime_transitions >= 100
            cross_fold_overlap = 0.0
            if hasattr(self, '_cross_fold_overlap'):
                cross_fold_overlap = self._cross_fold_overlap
            fold_ok = cross_fold_overlap >= 0.60
            power_sufficient = length_ok and regime_ok and fold_ok
            reasons = []
            if not regime_ok:
                reasons.append(f'regime_transitions={regime_transitions}<100')
            if not fold_ok:
                reasons.append(f'cross_fold_overlap={cross_fold_overlap:.2f}<0.60')
            return {
            ...
            return {'power_sufficient': False, 'reason': str(_e)}
    ...
    def optimize_step1(self, bar_data: Any = None,
        ...
        power_sufficient = self.check_l2_statistical_power(independent_bar_data)
...
STRATEGY_IDS = ['s1_hft', 's2_minute', 's3_box_extreme', 's4_box_spring', 's5_arbitrage', 's6_market_making']
...
class TwelveStrategyRunner:
    ...
    def run_single_strategy(self, strategy_id: str, variant: str,
        ...
        if bar_data is not None:
            try:
                ...
                if variant == 'shadow_a':
                    run_params['close_take_profit_ratio'] = run_params.get('close_take_profit_ratio', 1.5) * SHADOW_VARIANT_A_TP_FACTOR
                    run_params['close_stop_loss_ratio'] = run_params.get('close_stop_loss_ratio', 0.5) * SHADOW_VARIANT_A_SL_FACTOR
                elif variant == 'shadow_b':
                    run_params['close_take_profit_ratio'] = run_params.get('close_take_profit_ratio', 1.5) * SHADOW_VARIANT_B_TP_FACTOR
                    run_params['close_stop_loss_ratio'] = run_params.get('close_stop_loss_ratio', 0.5) * SHADOW_VARIANT_B_SL_FACTOR
    ...
    def validate_q3_cross_period_stability(
        ...
        cv = std_acc / mean_acc if mean_acc > 1e-10 else float('inf')
