...
class DBConnectionMixin:
    ...
    _MAX_POOL_SIZE = 50  # 默认值，__init__中尝试从config_service覆盖
    ...
    def _get_connection(self, read_only: bool = False) -> duckdb.DuckDBPyConnection:
        ...
        if _current_pool_size >= self._MAX_POOL_SIZE:
            ...
            if _reclaimed == 0:
                logger.error("[R25-BV-02-FIX] 连接池耗尽且无法回收, 抛出ConnectionError")
                raise ConnectionError(f"R25-BV-02: 连接池耗尽(size={_current_pool_size}, max={self._MAX_POOL_SIZE})")
        conn_attr = 'conn'
