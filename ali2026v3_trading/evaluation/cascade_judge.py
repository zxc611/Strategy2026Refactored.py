...
class CascadeJudge:
    ...
    def __init__(
        ...
        # R4-J-01修复: 补全13维度权重配置，确保总和=1.0
        # 原始4维度: profit_ratio(0.60) + sortino(0.40) + calmar(0.30) + sharpe(0.30) = 1.60
        # 归一化后: profit_ratio(0.375) + sortino(0.25) + calmar(0.1875) + sharpe(0.1875) = 1.0
        raw_total = profit_ratio_weight + sortino_weight + calmar_weight + sharpe_weight
        if abs(raw_total - 1.0) > 1e-4 and raw_total > 1e-10:
            # 自动归一化，确保权重和=1.0
            profit_ratio_weight /= raw_total
            sortino_weight /= raw_total
            calmar_weight /= raw_total
            sharpe_weight /= raw_total
        self.weights = {
    ...
    def judge(self, metrics: BacktestMetrics) -> CascadeReport:
        ...
        _dim_scores = {
            "profit_ratio_norm": self._safe_sigmoid(_dim_values["profit_ratio"], self.sigmoid["pr_center"], self.sigmoid["pr_scale"]),
            "sortino_norm": self._safe_sigmoid(_dim_values["sortino"], self.sigmoid["sortino_center"], self.sigmoid["sortino_scale"]),
            "calmar_norm": self._safe_sigmoid(_dim_values["calmar"], self.sigmoid["calmar_center"], self.sigmoid["calmar_scale"]),
            "sharpe_norm": self._safe_sigmoid(_dim_values["sharpe"], self.sigmoid["sharpe_center"], self.sigmoid["sharpe_scale"]),
        }
    ...
    def _calculate_score(self, profit_ratio: float, sortino: float,
        ...
        """
        s = self.sigmoid
        pr_norm = self._safe_sigmoid(profit_ratio, s["pr_center"], s["pr_scale"])
        sortino_norm = self._safe_sigmoid(sortino, s["sortino_center"], s["sortino_scale"])
        calmar_norm = self._safe_sigmoid(calmar, s["calmar_center"], s["calmar_scale"])
        sharpe_norm = self._safe_sigmoid(sharpe, s["sharpe_center"], s["sharpe_scale"])
