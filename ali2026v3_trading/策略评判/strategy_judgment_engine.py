...
BLOCKING_DIMENSIONS_DEFAULT = {DIM_RISK_BUDGET_COMPLIANCE, DIM_EXTREME_SURVIVAL, DIM_BEHAVIOR_CONSISTENCY}
...
class StrategyJudgmentEngine:
    ...
    def judge(
        ...
        try:
            ...
            if eviction_decision.get('should_evict', False):
                blockers.append(f"[E-09] 策略应被淘汰: {eviction_decision.get('reason', '')}")
        ...
        return JudgmentReport(
            ...
            judgment_timestamp=datetime.now(timezone(timedelta(hours=8))).isoformat(),
    ...
    def _resolve_scale_config(self, capital_scale: CapitalScale) -> Dict[str, Any]:
        try:
            ...
            logging.debug("[StrategyJudgmentEngine] _resolve_scale_config fallback: %s", _resolve_err)
            pass
    ...
    def _run_deep_validations(
        ...
        try:
            ...
            try:
                ...
                # R14-P1-DEAD-09修复: 空数据验证无意义，跳过并记录
                rsv_result = None
                logging.warning("[JudgmentEngine] R14-P1-DEAD-09: ReverseStrategyValidator.validate([])空数据，跳过")
            ...
            try:
                ...
                # R14-P1-DEAD-09修复: 空数据验证无意义，跳过并记录
                epv_result = None
                logging.warning("[JudgmentEngine] R14-P1-DEAD-09: ExecutionPathValidator.validate([])空数据，跳过")
...
class ChicoryEvictionPolicy:
    """E-09修复: 二次松弛机制三档权重驱逐策略

    W0(基础评分): 0.60 — 策略自身评分
    W1(活跃度衰减): 0.25 — 半衰期30天指数衰减
    W2(违反惩罚): 0.15 — 每次违反扣0.1
    """
    W0_BASE = 0.60
    W1_DECAY = 0.25
    W2_VIOLATION = 0.15
    ...
    def eviction_score(self, strategy_score: float, age_days: int,
                       violation_count: int = 0) -> float:
        import math
        base_score = strategy_score
        # R21-ALIGN: 与生产系统quant_core.py的R21-MATH-P2-04修复对齐 — exp参数裁剪防溢出
    ...
    def should_evict(self, strategy_score: float, age_days: int,
                     violation_count: int = 0, threshold: float = 0.3) -> bool:
...
class MarqueeThreshold:
    ...
    phase1(strict): n_samples<60, 要求score>=0.80
    phase2(moderate): 60<=n_samples<120, 要求score>=0.70
    phase3(relaxed): n_samples>=120, 要求score>=0.60
    """
    ...
    def __init__(self, phase1_threshold: float = 0.80, phase2_threshold: float = 0.70,
    ...
    # E-10修复: MarqueeThreshold渐亮阈值 — 三阶段机制
    @staticmethod
    def marquee_threshold(score: float, n_samples: int) -> str:
        """三阶段渐亮阈值:
        phase1(strict): n_samples<60, 要求score>=0.80
        phase2(moderate): 60<=n_samples<120, 要求score>=0.70
        phase3(relaxed): n_samples>=120, 要求score>=0.60
        ...
        if n_samples < 60:
            ...
            threshold = 0.70
            phase = "phase2_moderate"
        else:
            threshold = 0.60
            phase = "phase3_relaxed"
        passed = score >= threshold
        return phase if passed else f"{phase}_FAILED(threshold={threshold})"
