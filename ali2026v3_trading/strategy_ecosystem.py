...
class StrategyEcosystem:
    ...
    def route_capital(self, active_state: str) -> Dict[str, float]:
        ...
        with self._lock:
            ...
            # P1-R9-13修复: 使用alpha_ratio调整master槽分配，alpha高则倾向主策略
            if cr.alpha_ratio > 0.0 and cr.alpha_ratio <= 1.0:
                _alpha_adj = cr.alpha_ratio * 0.1
                allocations['master'] = allocations.get('master', 0.0) * (1.0 + _alpha_adj)
                _non_master_total = sum(v for k, v in allocations.items() if k != 'master')
                if _non_master_total > 1e-6:
                    _scale = (1.0 - allocations['master']) / _non_master_total
                    for k in allocations:
                        if k != 'master':
                            allocations[k] *= _scale
            ...
            try:
                from ali2026v3_trading.参数池.cycle_resonance_module import get_cycle_resonance_module
            ...
            # R3-P-01修复: s5_arbitrage/s6_market_making路由从CapitalRoute实例读取
            allocations['arbitrage'] = cr.arbitrage_base
            allocations['market_making'] = cr.market_making_base
    ...
    def record_spring_trade(self, direction: str):
        ...
        logging.info("[R16-P2-2.4] BoxSpring信号传递: direction=%s active_strategy=%s spring_position_count=%d",
                     direction, self._active_strategy, self._spring.position_count)
        with self._lock:
            ...
            # R16-P1-17: 记录子策略交易统计
            _sub_id = 's1_hft' if 'hft' in str(getattr(self._master, '_active_strategy', '')).lower() else 's2_resonance'
            if hasattr(self._master, 'record_sub_strategy_trade'):
                self._master.record_sub_strategy_trade(_sub_id, pnl if 'pnl' in dir() else 0.0)
...
# TODO: 将 df.reset_index() 改为 df.loc[:, columns] 或 df.set_index() + df.loc[] 模式
