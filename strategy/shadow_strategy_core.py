...
from infra.shared_utils import compute_content_hash  # R4-3: 统一哈希计算
...
class ShadowStrategyCoreService:
    ...
    def __init__(
        ...
        try:
            from infra.serialization_utils import yaml_safe_load  # R3-1修复
    ...
    def shutdown(self, wait: bool = True, timeout: float = 10.0) -> None:
        ...
        except Exception as _r3_err:  # FIX-39: 扩展异常类型，防止OSError/RuntimeError中断shutdown
            logging.debug("[R3-L2] silent except triggered: %s", _r3_err)
            pass
        # 关闭线程池
        if hasattr(self, '_log_writer_executor') and self._log_writer_executor is not None:
            try:  # FIX-39: 确保shutdown异常不阻断_log_writer_executor置None
                import sys as _sys
                ...
                logging.warning("[ShadowEngine.shutdown] _log_writer_executor.shutdown异常(非阻断): %s", _lw_err)
            finally:
                self._log_writer_executor = None  # FIX-39: 无论shutdown是否异常都置None
            try:
                logger.info("[R10-P0-16] _log_writer_executor shutdown complete")
            except Exception as _pass_err_0:
                logging.debug("[shutdown] 异常(非阻断): %s", _pass_err_0)
    def _atexit_shutdown(self) -> None:
        """atexit回调：进程退出时自动shutdown线程池，防止资源泄漏"""
        try:
            self.shutdown(wait=False)
        except Exception as _r3_err:  # FIX-40: 扩展异常类型，防止OSError跳过线程池shutdown
            logging.debug("[R3-L2] silent except triggered: %s", _r3_err)
            pass
