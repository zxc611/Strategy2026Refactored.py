...
from strategy.box_detector import BoxDetector, BoxProfile, ExtremeState, BoxStrategyParams
from strategy_judgment.strategy_judgment_facade import CapitalScale
...
from strategy.strategy_ecosystem.services import (
...
try:
    from param_pool.optimization.cycle_sharpe import LiveStrategySelector
    from infra.metrics_registry import mark_module_loaded
    mark_module_loaded('strategy_ecosystem_live_strategy_selector')
except ImportError as _ie:
    LiveStrategySelector = None
    try:
        from infra.metrics_registry import mark_module_failed
...
class CapitalRoutingService:
    ...
    def _load_capital_route_params(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                's1_hft': params.get('capital_route_s1_hft', 0.25),       # FIX-EE R12-3: 与config_params.py对齐
        ...
        with self._lock:
            ...
            if not cr.dynamic_enabled:
                return {
                    ...
                    'divergence': _route_params.get('s7_divergence', cr.divergence_base),
                    'other': _route_params.get('s3_box', cr.other_base),
                    'spring': _route_params.get('s4_spring', self._spring.capital_allocation),
            ...
            if self._capital_scale == CapitalScale.SMALL:
                spring_base = _route_params.get('s4_spring', self.CAPITAL_ALLOC_SMALL_SPRING)
                master_base = _route_params.get('s1_hft', self.CAPITAL_ALLOC_SMALL_MASTER)
                divergence_base = _route_params.get('s7_divergence', self.CAPITAL_ALLOC_SMALL_REVERSE)
                other_base = _route_params.get('s3_box', self.CAPITAL_ALLOC_SMALL_OTHER)
            elif self._capital_scale == CapitalScale.LARGE:
                spring_base = _route_params.get('s4_spring', self.CAPITAL_ALLOC_LARGE_SPRING)
                master_base = _route_params.get('s1_hft', self.CAPITAL_ALLOC_LARGE_MASTER)
                divergence_base = _route_params.get('s7_divergence', self.CAPITAL_ALLOC_LARGE_REVERSE)
                other_base = _route_params.get('s3_box', self.CAPITAL_ALLOC_LARGE_OTHER)
            else:
                spring_base = _route_params.get('s4_spring', cr.spring_base)
                master_base = _route_params.get('s1_hft', cr.master_base)
                divergence_base = _route_params.get('s7_divergence', cr.divergence_base)
                other_base = _route_params.get('s3_box', cr.other_base)
            ...
            try:
                ...
                if crm:
                    _SLOT_TO_CRM_STRATEGY = {'master': 's2_resonance', 'divergence': 's7_divergence', 'other': 's1_hft', 'spring': 's4_spring'}
                    for slot_name in ['master', 'divergence', 'other', 'spring']:
                        try:
                    ...
                    if _crm_phase is not None:
                        ...
                        # RC-49: 策略类型键名已标准化为 s1_hft/s2_resonance/...，生态位名称保持 master/spring/other/divergence 不变
                        _SLOT_MAP = {'s2_resonance': 'master', 's4_spring': 'spring', 's1_hft': 'other', 's7_divergence': 'divergence'}
                        _ranked_allocs = []
    ...
    def _precompute_capital_route_ratios(self, base_capital: float, ratios: Dict[str, float]) -> Dict[str, float]:
        ...
        from strategy.strategy_ecosystem import _precompute_capital_route_ratios as _impl
        return _impl(base_capital, ratios)

    def _precompute_division(self, numerator: float, denominator: float, key: str = '') -> float:
        """预计算浮点除法并缓存"""
        from strategy.strategy_ecosystem import _precompute_division as _impl
...
class StrategyEcosystem:
    ...
    def __init__(
        ...
        from strategy.lifecycle_service import LifecycleService as StrategyLifecycleService
        ...
        from strategy.strategy_monitoring_layer import EcosystemMonitoringService
