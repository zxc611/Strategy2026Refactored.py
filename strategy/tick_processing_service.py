...
class TickProcessingService:
    ...
    # FIX-7 RC-8: 降低tick超时阈值 500ms→200ms，更早触发背压降级避免系统卡顿
    # 根因: 实测tick处理时间938ms远超500ms阈值，但500ms阈值过于宽松导致背压降级触发滞后
    #       系统在938ms内已累积大量排队tick，引发级联超时
    # 修复: 降低阈值到200ms，更早触发背压降级走轻量路径，减少单tick处理时间
    #       配合启动期豁免(30s)和历史K线加载豁免，不影响正常启动流程
    ...
    def init_tick_handler(self) -> None:
        ...
        from infra.shared_utils import get_shard_router
    ...
    def on_tick(self, tick: Any) -> None:
        ...
        _now = time.time()
        # FIX-KK (R10-3-5): 首tick时间戳在tick处理开始时设置，而非仅在过载时设置
        # 根因: _first_tick_ts仅在_tick_elapsed_ms > threshold时设置(L406-409)，
        #       如果首批tick处理快(<200ms)，_first_tick_ts保持None，
        #       直到首个慢tick才设置→启动豁免期起点延迟→首批慢tick被误判为过载丢弃
        # 修复: 在tick处理入口处设置_first_tick_ts，确保首个tick即记录时间戳
        if getattr(self, '_first_tick_ts', None) is None:
        ...
        try:
            ...
            if _tick_elapsed_ms > self._TICK_OVERLOAD_THRESHOLD_MS:
                ...
                if (_now - _first_tick_ts) < _startup_grace_sec or _ext_kline_loading:
                    ...
                    # FIX-P0-2: 连续过载时延长丢弃窗口(2s→10s)，减少classify计数器暴增
    ...
    def _route_shard_index(self, instrument_id: str) -> int:
        ...
        from infra.shared_utils import ShardRouter, extract_product_code
    ...
    def _get_tick_field(self, tick: Any, field_name: str, default: Any = None) -> Any:
        ...
        from infra.shared_utils import get_tick_field as _get_tick_field_impl
    ...
    def _normalize_tick(self, tick: Any) -> Dict[str, Any]:
        ...
        from infra.shared_utils import normalize_tick as _normalize_tick_impl
    ...
    def _tick_probe_layer(self, tick: Any) -> str:
        ...
        from infra.health_monitor import record_tick_probe, DiagnosisProbeManager
