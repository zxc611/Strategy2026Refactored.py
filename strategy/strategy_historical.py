...
from infra.shared_utils import safe_int, safe_float, CHINA_TZ
...
def load_historical_klines_with_stop(
    ...
    for batch_index, start in enumerate(range(0, len(instruments), effective_batch_size), start=1):
        ...
        _batch_api_error = [False]  # FIX-31 RC-39: 跟踪批次内是否有API异常(非no_data)
            ...
            with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix=f"hkl-batch{batch_index}") as executor:
                ...
                for future in as_completed(futures):
                    ...
                    if result.get('success'):
                        ...
                        with _counter_lock:
                            failed_count += 1
                            if result.get('error'):  # FIX-31: 仅API异常计入empty_batch_limit
        ...
        # 根因: empty_batch_limit=20将"合约无历史数据"(API正常返回空)误判为"API故障"，20批后停止→S3期权K线从未被尝试
        # 修复: 区分no_data(API正常)和error(API故障)，仅API故障触发empty_batch_limit提前终止
        if batch_success_count == 0 and batch_fetched_klines == 0 and batch_enqueued_klines == 0 and _batch_api_error[0]:
...
from config.params_service import get_param_value as _get_param_value
...
class HistoricalKlineMixin:
    ...
    def _check_and_start_historical_load_on_tick(self) -> None:
        ...
        #       若tick再到达仍会创建新线程，导致暂停失效
        # 修复: 方法入口增加暂停/销毁检查，与hkl-retry中的检查(L682)保持一致
        if getattr(self, '_is_paused', False) or getattr(self, '_destroyed', False):
            logging.info(
                "[HKL][strategy_id=%s] tick-triggered load skipped: strategy paused or destroyed",
                self.strategy_id
            )
            return
        with self._historical_loader_lock:
