...
from config.config_params import json_dumps
from infra.shared_utils import atomic_replace_file, CHINA_TZ
...
class CheckpointService:
    ...
    def save_checkpoint(self) -> bool:
        ...
        try:
            ...
            try:
                from demo import __version__ as _strategy_version
            except ImportError:
...
class RecoveryService:
    ...
    def auto_recovery_flow(self) -> bool:
        ...
        try:
            ...
            if _ps is not None:
                try:
                    # FIX-NN4 (R37-ROOT): 幂等保护，防止_recover_positions被多次调用
                    # 根因: auto_recovery_flow和watchdog_restart都可能调用_recover_positions，
                    #       第二次调用会重新重放JSONL，导致状态不一致
                    if getattr(_ps, '_positions_recovered', False):
                        logging.info("[DR-P1-04] 持仓状态已恢复过(幂等跳过)")
                    elif hasattr(_ps, '_recover_positions'):
                        _ps._recover_positions()
                        _ps._positions_recovered = True
    ...
    def recover_from_checkpoint(self) -> Dict[str, Any]:
        ...
        try:
            with open(_checkpoint_path, 'r', encoding='utf-8') as f:
                from config.config_params import json_loads
        ...
        try:
            ...
            if _saved_state:
                from lifecycle.lifecycle_state_machine import StrategyState, _state_key
