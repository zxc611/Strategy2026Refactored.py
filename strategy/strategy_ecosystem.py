...
class StrategyEcosystem:
    ...
    def __init__(
        ...
        # 生产环境决策频率由K线周期 × state_confirm_bars自然决定
    ...
    def _propagate_capital_scale_fallback(self, scale_str: str) -> None:
        ...
        try:
            from ali2026v3_trading.signal.signal_service import SignalService, get_signal_service
        ...
        with self._lock:
            for req in self._revival_requests:
                ...
                if slot is None:
                    return False
                # INV-07: 验证状态转换合法性
                if not slot.validate_state_transition(SlotState.STANDBY):
                    logger.warning(
    ...
    def switch_active_strategy(self, new_state: str) -> Dict[str, Any]:
        with self._lock:
            ...
            if old_slot is not None and old_slot.position_count > 0:
                ...
                if not old_slot.validate_state_transition(SlotState.HANDOVER):
                    logger.warning(
                        "[INV-07] 策略%s从%s→handover转换不合法, 仍执行切换保护",
                        old_active, old_slot.state,
                    )
                old_slot.freeze()
                old_slot.state = SlotState.HANDOVER
            ...
            self._master.state = SlotState.ACTIVE if new_active == 'master' else SlotState.STANDBY
            self._reverse.state = SlotState.ACTIVE if new_active == 'reverse' else SlotState.STANDBY
            self._other.state = SlotState.ACTIVE if new_active == 'other' else SlotState.STANDBY
    ...
    def check_mutual_exclusion(
        ...
        with self._lock:
            ...
            if strategy_id == 'master' and self._reverse.position_count > 0:
                if self._reverse.state == SlotState.ACTIVE:
            ...
            if strategy_id == 'reverse' and self._master.position_count > 0:
                if self._master.state == SlotState.ACTIVE:
            ...
            if strategy_id == 'spring':
                for other_id in ('master', 'reverse'):
                    ...
                    if other_slot and other_slot.position_count > 0 and other_slot.state == SlotState.ACTIVE:
