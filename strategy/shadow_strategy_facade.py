...
from strategy._shadow_types import (
    ShadowTradeRecord,
    AlphaMetrics,
    ShadowParamsSnapshot,
)
import strategy._shadow_types as _shadow_types_mod
from strategy.shadow_strategy_core import (
    ShadowStrategyCoreService,
)
from strategy._shadow_strategy_signal import (
    ShadowStrategySignalService,
)
from strategy._shadow_strategy_pnl_metrics import (
...
class ShadowStrategyEngine:
    ...
    def _compute_sharpe(returns, annualize_factor=252, risk_free_rate=None):
        ...
        from strategy._shadow_strategy_pnl_metrics import ShadowStrategyPnLMetricsService
...
def get_shadow_strategy_engine(**kwargs) -> ShadowStrategyEngine:
    ...
    单例状态存储在 _shadow_types 中，本模块不持有可变状态。
    """
    with _shadow_types_mod._shadow_engine_lock:
        if _shadow_types_mod._shadow_engine is None:
            if getattr(_shadow_types_mod, '_shadow_engine_init_failed', False):
                return None
            try:
                _shadow_types_mod._shadow_engine = ShadowStrategyEngine(**kwargs)
            except Exception as _init_err:
                import logging
                logging.error("[ShadowStrategyEngine] 单例创建失败: %s", _init_err)
                _shadow_types_mod._shadow_engine_init_failed = True
                return None
        return _shadow_types_mod._shadow_engine
