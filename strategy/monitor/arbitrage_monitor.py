    ...
    def save_snapshot(self, signal: SimulatedArbitrageSignal) -> str:
        with self._lock:
            ...
            try:
                ...
                # FIX-33 RC-43: 添加成功日志，与S6 MarketMakingMonitor对齐
                # 根因: S5 save_snapshot无成功日志→日志统计显示0快照(实际已保存15个文件)
                logger.info("[S5-ARB-V1] 快照已保存: %s (size=%d bytes)", filepath, os.path.getsize(filepath))
