...
from strategy.box_spring_detector import BoxSpringDetectorService, BoxSpringDetectorMixin
from strategy.box_spring_executor import BoxSpringExecutorService, BoxSpringExecutorMixin
...
class BoxSpringStrategy:
    ...
    def on_tick(self, instrument_id: str, price: float, high: float = 0.0,
        ...
        if price <= 0:
            return
        # FIX-OO6 (S4-HedgeRatio): _hedge_ratio_fail_paused死标志修复
        # 根因: L350设置_hedge_ratio_fail_paused=True但全局无消费方→纸面修复
        # 修复: on_tick入口处检查，连续失败暂停时跳过后续处理
        if getattr(self, '_hedge_ratio_fail_paused', False):
            logging.debug("[BoxSpring] hedge_ratio连续失败暂停中，跳过tick: inst=%s", instrument_id)
            return
