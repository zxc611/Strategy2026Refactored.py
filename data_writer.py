...
class DataWriterMixin:
    ...
    def batch_insert_ticks(self, ticks_data, instrument_id: str = None, use_arrow: bool = True) -> int:
        ...
        try:
            ...
            if 'does not exist' in str(e) and 'ticks_raw' in str(e):
                try:
                    logger.warning("[AUTO-REPAIR] ticks_raw表不存在，尝试自动重建...")
    ...
    def ensure_config_coverage_for_today(self, trade_date: Optional[date] = None) -> Dict[str, int]:
        ...
        try:
            ...
            """, [target_date, _prev_date, target_date, target_date, _prev_date, target_date, target_ts]).fetchone()
        ...
        try:
            ...
            counts = conn.execute("""
                SELECT
                    (SELECT COUNT(*) FROM instruments_registry),
                    (SELECT COUNT(DISTINCT instrument_id) FROM ticks_raw WHERE date IN (?, ?)),
                    (SELECT COUNT(DISTINCT instrument_id) FROM ticks_raw
                       WHERE date IN (?, ?) AND COALESCE(sync_status, '') <> 'simulated_coverage'),
                    (SELECT COUNT(DISTINCT instrument_id) FROM ticks_raw
                       WHERE date IN (?, ?) AND sync_status = 'simulated_coverage'),
                    (SELECT COUNT(DISTINCT internal_id) FROM klines_raw WHERE trade_date = ?),
                    (SELECT COUNT(DISTINCT internal_id) FROM klines_raw
                       WHERE trade_date = ?
                         AND NOT (open IS NULL AND high IS NULL AND low IS NULL AND close IS NULL AND volume = 0))
                """, [target_date, _prev_date, target_date, _prev_date, target_date, _prev_date, target_date, target_date]).fetchone()
