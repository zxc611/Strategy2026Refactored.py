...
from ali2026v3_trading.shared_utils import sanitize_sql_identifier
...
class DefaultDataAccess:
    ...
    def save_trade(
        ...
        if ensure_idempotent and order_id:
            try:
                from ali2026v3_trading.order_persistence import SelfTradeDetector
    ...
    def write_records(
        ...
        try:
            from ali2026v3_trading.db_adapter import connect, execute
            from ali2026v3_trading.ds_realtime_cache import _resolve_duckdb_file
    ...
    def execute_query(
        ...
        try:
            from ali2026v3_trading.db_adapter import connect_in_memory, connect, execute, fetchall
            from ali2026v3_trading.ds_realtime_cache import _resolve_duckdb_file
            if in_memory:
    ...
    def register_dataframe(self, name: str, df: Any) -> None:
        try:
            from ali2026v3_trading.db_adapter import connect, register_df
            from ali2026v3_trading.ds_realtime_cache import _resolve_duckdb_file
            db_path = _resolve_duckdb_file()
            conn = connect(db_path)
            try:
    ...
    def get_storage_stats(self) -> StorageStatsResult:
        try:
            from ali2026v3_trading.db_adapter import connect, fetchall
            from ali2026v3_trading.ds_realtime_cache import _resolve_duckdb_file
            import os
            db_path = _resolve_duckdb_file()
