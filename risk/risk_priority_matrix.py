...
# [M1-34] RPN��������������
"""
Phase0增强: RPN风险优先数矩�?�?定量风险评估
RPN = 概率(1-5) × 影响(1-5) × 可检测�?1-5)
CRITICAL(�?6) > HIGH(�?4) > MEDIUM(�?5) > LOW(<15)
"""
...
class RiskPriorityMatrix:
    ...
    def _init_default_risks(self) -> None:
        self._risks = [
            RiskItem('拆分引入新BUG', 3, 4, 3, '每Sprint后运行全量回�?单元测试+契约测试', 'Phase 1-4'),
            RiskItem('循环依赖再现', 3, 4, 3, '每Sprint后运行import_graph DAG验证;CI/CD阻断', 'Phase 2-3'),
            RiskItem('测试覆盖不足', 4, 3, 3, '拆分前先补测�?覆盖率≥80%再拆;先写测试再重�?, 'Phase 1-4'),
            RiskItem('人员技能不�?, 3, 3, 4, '提前进行技能矩阵评�?关键Sprint配备备份人员', 'Phase 1-4'),
            RiskItem('_LifecycleMixin MRO断裂', 2, 5, 3, '拆分前确认所有调用方已迁移到类方法委�?A/B灰度验证', 'Phase 2-Sprint 6'),
            RiskItem('WAL/JSONL格式兼容�?, 3, 3, 3, '拆分前归档现有WAL;新格式向下兼容旧格式读取;格式版本�?, 'Phase 2-Sprint 4'),
            RiskItem('双写不一�?, 3, 4, 2, '6个月双写过渡�?定期对账脚本+差异告警', 'Phase 2'),
            RiskItem('Phase并行冲突', 2, 3, 4, 'Phase 3和Phase 4操作不同模块,可部分并行但需协调', 'Phase 3-4'),
            RiskItem('可观测性缺�?, 3, 5, 1, 'Phase 0先部署可观测性基础设施(Sprint 0独立前置)', '全程'),
            RiskItem('Facade性能退�?, 2, 3, 2, '委托方法用__slots__+描述符缓存优�?性能基线对比', 'Phase 2'),
        ]
    ...
    def to_report(self) -> str:
        lines = ['# RPN风险优先数矩阵\n']
        lines.append('| # | 风险 | 概率 | 影响 | 可检测�?| RPN | 严重�?| 触发Phase |')
        lines.append('|---|------|:--:|:--:|:----:|:---:|:------:|----------|')
