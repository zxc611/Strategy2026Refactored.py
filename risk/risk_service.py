...
- 旧架构：08_position.py (~600_ + 10_gate.py (~200行风控相） = ~700_

- 新架构：risk_service.py (~400_
...
# UPG-P1-04修复: API版本标记装饰器器P1-21: 统一从infra.shared_utils导入，删除本地重复定义
...
def _audit_annotation(category='', phase1_status='', note=''):
    ...
    """审计标注装饰器器不改变类行为，仅用于H.1.3格式清单审计追踪"""
...
def validate_risk_level_transition(current: RiskLevel, target: RiskLevel) -> RiskLevel:
    ...
    if target == RiskLevel.CRITICAL and current in (RiskLevel.LOW, RiskLevel.MEDIUM):
        ...
        logging.warning("[R23-P1-02-FIX] 风控级别非法跳跃: %s_s, 强制经过RESTRICTED中间状态, current.name", target.name)
...
@_audit_annotation(category='lightweight_dataclass', phase1_status='stable', note='响应dataclass_方执行6行，无隐式依赖')
...
@_audit_annotation(category='lightweight_dataclass', phase1_status='stable', note='指标dataclass_方法法2行，无隐式依赖')
...
# CC-03/AP-02修复: 子服务导入（延迟导入避免循环依赖赖
...
class RiskService:
    ...
    # AUDIT: lightweight_helper, phase2_target=ali2026v3_trading.risk_engine.log_deduplicator, 2方法, 19_

    # DEPRECATED: 独立模块 risk_engine.log_deduplicator 已替换 # noqa: F811  # pylint: disable=useless-suppression
    ...
    def __init__(self, params: Any = None, position_manager: Any = None, strategy: Any = None):
        ...
        # 升A路径T1.2: RiskDashboardService集成 _注册心跳消除死代理
    ...
    def get_dashboard_service(self) -> 'RiskDashboardService':
        ...
        """获取RiskDashboardService实例 _升A路径T1.2: 消除死代理"""
    ...
    def _fire_alert(self, event_type, detail):
        ...
        # P1-05修复: 优先通过EventBus发布RiskAlertEvent，保留直接回调作为兼容过滤
    ...
    def record_partial_fill_progress(self, order_id, instrument_id, traded_volume, total_volume, strategy_id) -> None:

        return self._config_provider.record_partial_fill_progress(order_id, instrument_id, traded_volume, total_volume, strategy_id)

    def _start_dashboard_timer(self) -> None:  # [R27-AUDIT] P1修复: 添加hasattr保护防止委托链断开
    ...
    def stop(self) -> None:
        ...
        # R25-P1-CF-05修复: stop()添加hasattr防御，防止损config_provider无stop方法时AttributeError
    ...
    def check_before_trade(self, signal: Dict[str, Any]) -> RiskCheckResponse:
        ...
        # R33-P1-7修复: 集成crack_validation到生产策略链表
    ...
    def _record_and_publish(self, signal: Dict[str, Any], _check_dedup_key: str, _cyclic_guard) -> RiskCheckResponse:
        ...
        # P1-42修复: 拆分为记录EventBus发布，消除与直接EventBus.publish的双通道
    ...
    def _publish_risk_event_via_eventbus(self, signal: Dict[str, Any], result: 'RiskCheckResponse') -> None:
        ...
        """P1-42修复: 通过EventBus统一发布风控事件，消除独立发布路由"""
    ...
    def check_regulatory_compliance(self, position_data: Optional[Dict[str, Any]] = None) -> ComplianceResult:
        ...
        """合规检查链R2-3: 统一走。check_service委托路径"""
        ...
        try:
            ...
            logging.error("[RiskService] check_regulatory_compliance委托链异常 %s", _err)

            return {'compliant': False, 'reason': f'风控委托链异常 {_err}'}
    ...
    def check_capital_sufficiency(self, equity: float, required_margin: float = 0.0,
        ...
        """资金充足性检查链R2-3: 统一走。check_service委托路径"""
        ...
        try:
            ...
            logging.error("[RiskService] check_capital_sufficiency委托链异常 %s", _err)

            return {'sufficient': False, 'reason': f'风控委托链异常 {_err}'}



    def check_exchange_status(self, exchange: str = "AUTO") -> ExchangeStatusResult:

        """交易所状态检查链R2-3: 统一走。check_service委托路径"""
        ...
        try:
            ...
            logging.error("[RiskService] check_exchange_status委托链异常 %s", _err)

            return {'status': 'CLOSED', 'reason': f'风控委托链异常 {_err}'}
    ...
    def check_last_trading_day(self, instrument_id: str, days_to_expiry: int) -> RiskCheckResponse:
        # CHAIN-BUG-4 已知缺口: 此方法当前无任何平仓触发路径调用，
        # 配置 last_trading_day_close_ahead_days(=3) 未落地。
        # 当前期权平仓仅靠 _check_option_expiry 在 days_to_expiry<=0(已到期) 触发，
        # 不会在到期前N天提前平仓。待产品确认 ahead_days 是否适用于期权后接入。
...
def get_risk_service(params: Any = None, position_manager: Any = None,
    ...
    首次以params=None初始化后，后续传入真实params会被更新到实例中断
    ...
    with _risk_service_lock:
        ...
        if scope not in _risk_service_instances:
            ...
            logging.info(
                ...
                '[RiskService] 首次初始化完成scope=%s params_type=%s position_manager=%s',
            ...
            # R13-P0-API-02修复: 后续调用时，只要传入非None的params就更新（无论原params是否为None_

            # 原逻辑仅在。instance.params is None时更新，导致首次params=None初始化后。'
            # 第二次传入真实params时若。instance.params已被设为{}则不会更新
            ...
            if params is not None:
                ...
                if instance.params is None or instance.params != params:
                    ...
                    logging.info('[RiskService] 参数已更新scope=%s params_type=%s',
    ...
    # AP-03: SingletonRegistry注册（非关键路径，失败不影响服务返回撤
...
def reset_risk_service(scope_id: Optional[str] = None) -> None:
    ...
    # [R23-P1-07-FIX] reset后发布引用失效通知，提醒其他模块重新获取实现
...
# P-04修复: RiskDashboardService独立全局单例 _Greeks聚合+PnL归因+压力测试

class RiskDashboardService:

    """风险仪表盘服务务Greeks聚合、PnL归因、压力测量



    独立全局单例，供RiskService和外部仪表盘调用例
    ...
    def aggregate_greeks(self, positions: Dict[str, Any]) -> Dict[str, float]:
        ...
        """P1-61修复: Greeks聚合委托到position_greeks.aggregate_greeks_exposure（唯一权威实现现"""
...
# OPS-05修复: 告警分级 _P0/P1/P2分类

# OPS-06修复: P0告警自动升级 _超时后自动升级通知

# OPS-07修复: 告警去重/聚合 _防止告警风暴

# OPS-14修复: 紧急操作审批机。_confirm_daily_resume需审批

# OPS-15修复: 操作审计日志 _记录谁在何时做了什。'
# ============================================================================
...
# R27-CP-05-FIX / R1-4修复: structured_audit_log从infra.risk_audit_utils导入，消除对已清空archive目录的依赖

from ali2026v3_trading.infra.security_service import structured_audit_log  # R27-CP-05-FIX
