...
from infra.shared_utils import CHINA_TZ as _CHINA_TZ, api_version, safe_int, safe_float
...
try:
    from position.position_service import PositionLimitConfig as PositionLimit
except ImportError:
    PositionLimit = None

from risk.risk_check_service import RiskCheckService
from risk.risk_compute_service import RiskComputeService
from risk.risk_circuit_breaker import SafetyMetaLayer, get_safety_meta_layer, cleanup_safety_layer
from risk.risk_config_provider import RiskConfigProvider
...
class RiskService:
    ...
    def _fire_alert(self, event_type, detail):
        try:
            from infra.event_bus import get_global_event_bus, RiskAlertEvent
            bus = get_global_event_bus()
            if bus is not None:
    ...
    def check_before_trade(self, signal: Dict[str, Any]) -> RiskCheckResponse:
        ...
        if _daily_hard_stop is None:
            try:
                from risk.risk_circuit_breaker import get_safety_meta_layer  # [FIX-20260712-AUDIT-P1] 修正循环导入：从risk_circuit_breaker导入而非自身模块
                _sid = str(getattr(self, '_scope_id', '') or 'global')
                # FIX-64 D3 P1: 修正参数名 scope_id → strategy_id（与 strategy_business_layer.py L746 同类问题）
        ...
        if _daily_hard_stop:
            ...
            if not _dry_run:
                try:
                    from risk.risk_support import get_safety_meta_layer as _get_sml
                    _sml = _get_sml()
                    if _sml is not None:
    ...
    def _publish_risk_event_via_eventbus(self, signal: Dict[str, Any], result: 'RiskCheckResponse') -> None:
        try:
            from infra.event_bus import get_global_event_bus, RiskEvent
            bus = get_global_event_bus()
            if bus is not None:
...
def get_risk_service(params: Any = None, position_manager: Any = None,
    ...
    避免风控检查因参数缺失而失效。
    """
    from infra.security_service import _normalize_risk_scope_id
    ...
    with _risk_service_lock:
        if scope not in _risk_service_instances:
            ...
            # FIX-66 D4 P2: 设置 _scope_id 属性，供 get_safety_meta_layer() 获取策略级 scope
            # 根因: RiskService.__init__ 未设置 _scope_id 属性
            #       → risk_service.py L350 getattr(self, '_scope_id', '') 恒返回 ''
            #       → get_safety_meta_layer(None, strategy_id='global') 所有策略共享全局实例
            #       → scope 隔离失效
            # 修复: 工厂函数创建实例后立即设置 _scope_id
            try:
                _risk_service_instances[scope]._scope_id = scope
            except Exception:
                pass
            logging.info(
...
class RiskDashboardService:
    ...
    def aggregate_greeks(self, positions: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            from position.position_greeks import aggregate_greeks_exposure
