...
from infra.shared_utils import safe_float, safe_int, safe_get_float, safe_get_int  # P1-20: 统一类型转换
...
from infra.shared_utils import api_version


def structured_audit_log(event_type: str, action: str, details: Dict[str, Any] = None,
                         severity: str = "INFO") -> None:
    try:
        from risk.risk_service import audit_chain_append
    ...
    def load_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if saved_date and saved_date == today:
                ...
                # FIX-20260715-CROSS-DAY-RESET: 跨日自动重置hard_stop和new_open_blocked
                # 根因: 旧逻辑(FIX-20260706-STATE-SPLIT)跨日保留hard_stop=True，与
                # safety_meta_equity.py:_update_equity_metrics的跨日重置逻辑(FIX-20260712-P1)矛盾。
                # on_init阶段_update_equity_metrics重置hard_stop=False后，on_start阶段
                # load_state又恢复hard_stop=True，导致状态覆盖，所有开仓被永久阻断。
                # 修复: 对齐FIX-20260712-P1，跨日时直接重置，当日如再次触及回撤阈值会重新触发。
                ...
                if _saved_hard_stop:
                    logging.warning(
                        "[SafetyMetaLayer] FIX-20260715-CROSS-DAY-RESET: 跨日hard_stop已自动重置"
                        "(%s -> %s)。当日回撤监控重新开始，如再次触及阈值将重新触发。",
                        saved_date, today,
                    )
                logging.info(
