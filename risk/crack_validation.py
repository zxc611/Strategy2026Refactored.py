...
# DEPRECATED(P0-20): validate_logic_reversal_no_future别名已删除（权威实现在param_pool/checks_individual.py）�?
# 本模块仍被生产代码引�? risk/risk_service.py(check_safety_meta_layer, validate_circuit_breaker_vs_time_stop)�?
# param_pool/validation_deep_orchestrator.py(validate_indicator_multiscale_consistency)。待上述引用迁移后可删除�?
...
def validate_shadow_b_stability(
    ...
    if len(arr) < 2:
        return {'passed': False, 'ci_width': float('inf'), 'reason': '样本�?2'}
...
def apply_tick_rounding(
    ...
    if direction == 'up':
        ...
        # R27-P2-FP-02修复: round()银行家舍入→确定性舍�?
...
def validate_tick_rounding_impact(
    ...
    """P2-裂缝13修复: 验证取整前后Sharpe绝对差异 < max_sharpe_diff(默认0.2)�?

    原实现使用相对比�?min_ratio=0.7)，与裂缝要求不符�?
    修复后改用绝对差�? abs(sharpe_before - sharpe_after) < max_sharpe_diff�?
    保留min_ratio参数以兼容旧调用方�?
    """
...
def compute_cascade_slippage(
    ...
    新开仓滑�?= min(原滑�?× (1 + sqrt(平仓�?平均成交�?) × cascade_multiplier, cap)
...
def validate_indicator_multiscale_consistency(
    ...
    """验证原生频率指标下采�?vs 重采样后直接计算的相关系�?> min_corr�?""
...
def validate_alpha_confidence_overlap(
    ...
    """验证�?影子Sharpe置信区间重叠比例 <= max_overlap_ratio�?

    若重�?50%，标记alpha_unreliable，资金分配回退到基础权重�?
    """
...
def validate_shadow_param_orthogonality(
    ...
    if len(main_param_vector) == 0 or len(shadow_param_vector) == 0:
        return {'passed': False, 'kl_divergence': 0.0, 'reason': '空向�?}
...
def validate_attribution_residual_significance(
    ...
    触发E7条件: t检验p<alpha �?mean(|residual|) > residual_pct_threshold * mean(|pnl|)
    """
    ...
    passed = True  # 验证本身总是通过，结果告知E7是否应触�?
    return {
        'passed': passed,
        't_stat': t_stat,
        'p_value': p_value,
