...
from .risk_support import safe_get_float as _safe_get_float, safe_get_int as _safe_get_int
...
class RiskComputeService:
    ...
    def compute_simplified_pnl(self, entry_price: float, exit_price: float,
        ...
        if commission_per_lot is None:
            try:
                from infra.commission_utils import get_commission_per_lot
    ...
    def calculate_risk_metrics(self):
        from risk.risk_service import RiskMetrics
        try:
    ...
    def compute_mode_position_size(
        ...
        try:
            from governance.mode_engine import ModeEngine, PredictiveStateEngine
    ...
    def compute_decision_score(
        ...
        try:
            from order.order_flow_bridge import OrderFlowBridge
        ...
        try:
            from governance.mode_engine import PredictiveStateEngine
        ...
        if abs(total_weight) < FLOAT_COMPARE_TOLERANCE:
            logging.warning("[RiskService] 权重总和接近0，使用等权重")
            n = len(w)
    ...
    def _compute_position_scale(self, composite_score: float,
        ...
        if scores.get('alpha_decay', 1.0) < 0.3:
            if not getattr(self._rs, '_alpha_decay_recovery_confirmed', False):
                ...
                logging.warning("[R24-P0-TR-07] Alpha衰减已人工确认恢复，跳过惩罚")
    ...
    def _compute_life_score(self, hmm_state: Optional[str] = None) -> float:
        ...
        if estimator is None or hmm_state is None:
            if estimator is None and hmm_state is not None:
                logging.warning("[R5-E-08] _compute_life_score降级: estimator不可用，返回中性评估0.5")
            return 0.5
    ...
    def compute_otm_sync_exposure(self, conn=None) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.debug("[P1-8] OTM同步风险敞口: %d个标的分组", len(exposure))
