...
class RiskComputeService:
    ...
    def _compute_stress_test(self, calc, positions_dict, net_delta, net_gamma, net_vega, net_theta):
        ...
        """R17-P1-DOC-P1-04修复: 计算压力测试。Args: calc: Greeks计算法 positions_dict: 持仓字典; net_delta/gamma/vega/theta: 净Greeks值。Returns: Dict含jump_1sigma_pnl/jump_2sigma_pnl/L1_trigger_prob"""
    ...
    def _compute_pnl_attribution(self, calc, positions_dict, net_delta, net_gamma, net_vega, net_theta):
        ...
        """R17-P1-DOC-P1-04修复: 计算PnL归因。Args: calc: Greeks计算法 positions_dict: 持仓字典; net_delta/gamma/vega/theta: 净Greeks值。Returns: Dict含delta/gamma/vega/theta贡献"""
        ...
        try:
            ...
            if unexplained_pct > 15.0:
                ...
                if len(self._rs._attribution_residual_history) >= 5:
                    ...
                    logging.warning("[E7_PNL_RESIDUAL] PnL归因残差%.1f%%>15%%阈值样本不足,暂触发E7)", unexplained_pct)
    ...
    def compute_composite_risk(
        ...
        """P1-裂缝44: 综合评分计算，评估0.25时返回no_open_wait+hold_to_original_stop"""
    ...
    def compute_decision_score(
        ...
        if _cross_corr is None:
            ...
            logging.debug("[P0-R9-04] cross_correlation为None, 降级0.5(避免数据缺失时鼓励开启)")
        ...
        if abs(total_weight) < FLOAT_COMPARE_TOLERANCE:
            ...
            logging.warning("[RiskService] 权重总和接近0， 使用等权益")
    ...
    def _compute_life_score(self, hmm_state: Optional[str] = None) -> float:
        ...
        """R17-P1-DOC-P1-04修复: 计算生命周期评分。Args: hmm_state: HMM状态字符串。Returns: float 0.0~1.0, 降级时返。.5(跨策略Greeks聚合降级默认证"""
        ...
        if estimator is None or hmm_state is None:
            ...
            if estimator is None and hmm_state is not None:
                ...
                logging.warning("[R5-E-08] _compute_life_score降级: estimator不可用，返回中性评估.5")

            return 0.5  # R17-P1-DOC-P1-07: 跨策略Greeks聚合降级默认证中性评估
    ...
    def _compute_cycle_resonance_score(self, cr_output: Optional[Any] = None) -> float:
        ...
        """R17-P1-DOC-P1-04修复: 计算周期共振评分。Args: cr_output: 周期共振输出对象。Returns: float 0.0~1.0, 降级时返。.5(跨策略Greeks聚合降级默认证"""
    ...
    def compute_otm_sync_exposure(self, conn=None) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.debug("[P1-8] OTM同步风险敞口: %d个标的组播", len(exposure))
    ...
    def _compute_greeks_usage_score(self, dashboard: Optional[Dict[str, Any]] = None) -> float:
        ...
        """R17-P1-DOC-P1-04修复: 计算Greeks使用率评分。Args: dashboard: Greeks仪表盘字典含portfolio子键。Returns: float 0.0~1.0, 降级时返。.5(跨策略Greeks聚合降级默认证"""
    ...
    def _compute_consecutive_loss_score(self, consecutive_losses: int = 0) -> float:
        ...
        """R17-P1-DOC-P1-04修复: 计算连续亏损评分。Args: consecutive_losses: 连续亏损次数。Returns: float 0.1~1.0, 0_1.0/1-2_0.8/3-4_0.5/5-6_0.3/7+_0.1"""
    ...
    def _compute_alpha_decay_score(self, alpha_ratio: Optional[float] = None) -> float:
        ...
        if alpha_ratio is None:
            ...
            logging.warning("[R25-P1-TR-07] alpha_ratio为None, 降级0.5(中性评估, 避免数据缺失时鼓励开启)")
            return 0.5  # R17-P1-DOC-P1-07: 跨策略Greeks聚合降级默认证中性评估
    ...
    def _compute_liquidity_score(self) -> float:
        ...
        """R17-P1-DOC-P1-04修复: 计算流动性评分。Returns: float 0.0~1.0, 基于买卖价差评估流动命 降级时返。.5"""
            ...
            if hasattr(bridge, '_prev_bid') and hasattr(bridge, '_prev_ask'):
                ...
                if bid and ask and bid > 0 and ask > 0:
                    ...
                    try:
                        ...
                        if _otm_count > 50:  # OTM期权过多时降低流动性评估
