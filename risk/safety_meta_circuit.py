...
from infra.shared_utils import safe_int, safe_float, CHINA_TZ as _CHINA_TZ, stable_mean, stable_variance  # P2-13: 统一CHINA_TZ; P2-32: 统一stable_mean/stable_variance

from risk.risk_support import (
...
class CircuitBreakerService:
    ...
    def _evaluate_circuit_conditions(self, now: float, equity_series: deque,
        ...
        if hasattr(self.owner, '_isolation_level'):
            ...
            from config.config_params import ISOLATION_LEVELS
    ...
    def _trigger_circuit_break(self, now: float, drop_pct: float, threshold: float,
        ...
        try:
            ...
            from config.config_params import get_security_responder
    ...
    def _reset_circuit_state(self, drop_pct: float, threshold: float,
        ...
        try:
            ...
            from infra.event_bus import get_global_event_bus, CircuitBreakerTriggeredEvent
    ...
    def _force_position_reduction_on_circuit_breaker(self) -> None:
        ...
        try:
            ...
            # FIX-28 RC-36: dry_run模式下跳过强制减仓，避免虚拟持仓被减仓导致equity异常下降
