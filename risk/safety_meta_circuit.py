...
from ali2026v3_trading.risk.safety_meta_audit import (
...
class CircuitBreakerService:
    """断路器服�?�?从SafetyMetaLayer提取的断路器核心逻辑

    负责：断路器触发/评估/重置/状态持久化/冷静�?影子模式/算法熔断
    """
    ...
    def __init__(self, params: Any, owner: Any):
        ...
        # [P0-29修复] 暂停/冷静状态完全委托给 _risk_cb_half_open，不再维护平行属性�?
    ...
    def _evaluate_circuit_conditions(self, now: float, equity_series: deque,
        ...
        if now < _calm_until:
            ...
            logging.info(
                "[SafetyMetaLayer] 熔断冷静期内，忽略二次触�?(冷静期剩�?.0fs)",
                _calm_until - now,
            )
            return None
    ...
    def _trigger_circuit_break(self, now: float, drop_pct: float, threshold: float,
        ...
        # [P0-29修复] 通过 _risk_cb_half_open 统一管理暂停状�?
        ...
        # 计算当前冷静期截止时间（�?force_open 之前，因�?force_open 会重�?opened_at�?
        ...
        # _calm_period_duration 相对�?_risk_cb_half_open.opened_at（此时已等于 now�?
        ...
        logging.warning(
            "[SafetyMetaLayer] �?速率断路器触发！1min回撤=%.2f%%, 阈�?%.2f%%, 暂停%.0f�? 冷静�?.0f�? 影子观察%.0f�?,
        ...
        try:
            ...
            if _blocked:
                logging.critical("[SEC-P1-12] 安全自动阻断已激�? source=%s", _strategy_id_ns)
    ...
    def _reset_circuit_state(self, drop_pct: float, threshold: float,
        ...
        try:
            ...
            logging.debug("[SafetyMetaLayer] OPS-P1-04: EventBus断路器告警推送异�? %s", _ops04_e)
        self.save_state({})
    ...
    def _cancel_pending_on_circuit_breaker(self) -> None:
        ...
        try:
            ...
            if spm and hasattr(spm, 'reset_state_cache'):
                ...
                logging.info("[SafetyMetaLayer] 断路器触发后已清空state_param_manager状态缓�?)
        except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as e:
        ...
        try:
            ...
            if wc and hasattr(wc, 'reset_cache'):
                wc.reset_cache()
                logging.info("[SafetyMetaLayer] 断路器触发后已清空width_cache状态缓�?)
    ...
    def _force_position_reduction_on_circuit_breaker(self) -> None:
        try:
            ...
            if reduced_count > 0:
                logging.critical(
                    "[SafetyMetaLayer] INV-P1-10: 熔断后强制减仓完�?reduced=%d positions ratio=%.0f%%",
    ...
    def is_trading_paused(self) -> Tuple[bool, str]:
        with self._lock:
            now = time.time()
            # [P0-29修复] 暂停判断委托�?_risk_cb_half_open，不再使�?_trading_paused_until
            ...
            if cb is not None and cb.state == CircuitBreakerHalfOpen.OPEN and cb.opened_at > 0:
                ...
                if now < pause_end:
                    ...
                    return True, f"{self._pause_reason}, 剩余{remaining:.0f}�?
            if now < self._algo_paused_until:
                remaining = self._algo_paused_until - now
                return True, f"{self._algo_pause_reason}, 剩余{remaining:.0f}�?
            if self._pause_reason:
                _prev_reason = self._pause_reason
                self._pause_reason = ""
                logging.info("[P1-R9-21] 交易暂停已恢�? 原暂停原�? %s", _prev_reason)
                if self._risk_cb_half_open is not None:
    ...
    def is_algo_paused(self) -> Tuple[bool, str]:
        with self._lock:
            ...
            if self._algo_paused and time.time() >= self._algo_paused_until:
                ...
                logging.info("[SafetyMetaLayer] CMP-P1-13: 算法交易熔断已自动恢�?)
            return False, ""
    ...
    def check_algo_circuit_breaker(self) -> Optional[str]:
        try:
            ...
            if recent_st_bans > 3:
                reason = f"自成交禁止{recent_st_bans}�?小时>3, 暂停算法交易1小时"
                logging.critical("[SafetyMetaLayer] CMP-P1-13: %s", reason)
                with self._lock:
                    self._algo_paused = True
            ...
            if recent_orders_60 > 100:
                reason = f"报单频率{recent_orders_60}�?分钟>100, 暂停5分钟"
                logging.critical("[SafetyMetaLayer] CMP-P1-13: %s", reason)
                with self._lock:
                    self._algo_paused = True
            ...
            logging.debug("[SafetyMetaLayer] CMP-P1-13: 算法熔断检查异�? %s", e)
            return None
    ...
    def verify_market_safety_before_resume(self, equity_series: deque,
        ...
        if cb is not None and cb.state == CircuitBreakerHalfOpen.OPEN and cb.opened_at > 0:
            ...
            if pause_end > now:
                logging.warning(
                    "[SafetyMetaLayer] INV-RSK-01: 断路器暂停期未结�?paused_until=%.0f",
                    pause_end,
                )
                return False
        ...
        if _calm_until > now:
            logging.warning(
                "[SafetyMetaLayer] INV-RSK-01: 断路器冷静期未结�?calm_until=%.0f",
                _calm_until,
            )
            return False
            ...
            try:
                ...
                if _sse is not None:
                    if _sse.is_absolute_ev_paused():
                        logging.warning("[SafetyMetaLayer] INV-RSK-01: 影子引擎EV暂停仍生�?)
                        return False
                    if _sse.is_degradation_active():
                        logging.warning("[SafetyMetaLayer] INV-RSK-01: 影子引擎降级仍生�?)
                        return False
            except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as _sm_ev_err:
                logging.warning("[R22-EP-01b] SafetyMetaLayer影子引擎检查异�? %s", _sm_ev_err)
                return False
    ...
    def save_state(self, state_data: Dict[str, Any]) -> None:
        ...
        # [P0-29修复] �?_risk_cb_half_open 派生暂停/冷静截止时间
        cb = self._risk_cb_half_open
    ...
    def load_state(self) -> Dict[str, Any]:
        """委托CircuitBreakerStateStore进行状态恢�?""
        if not hasattr(self, "_state_store") or self._state_store is None:
            self._state_store = CircuitBreakerStateStore(self.params)
        ...
        if raw:
            ...
            # [P0-29修复] 通过 _calm_period_duration 恢复冷静�?
