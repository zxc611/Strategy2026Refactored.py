...
from risk.risk_support import safe_get_float, _get_tz_aware_now, structured_audit_log
...
class DrawdownMonitorService:
    ...
    def _notify_equity_change(self, equity: float, stats: Dict[str, Any],
        ...
        # FIX-E: dry_run模式下不触发equity<=0硬止，避免虚拟equity波动阻断所有交易
        # 根因: _notify_equity_change无条件设置_daily_hard_stop_triggered=True，
        #       dry_run模式虚拟equity可能跌至<=0→硬止→所有交易被阻断且无恢复路径
        # 修复: dry_run模式仅记录警告，不设置硬止标志
        _dry_run_mode = False
        try:
            from config.params_service import get_params_service
            _dry_run_mode = bool(get_params_service().get_bool('dry_run_mode', False))
        except Exception:
            pass
        if not _dry_run_mode:
            try:
                ...
                if _osvc is not None:
                    _dry_run_mode = bool(getattr(_osvc, '_dry_run_mode', False))
            except Exception:
                pass
        if _dry_run_mode:
            logging.warning("[SafetyMetaLayer] dry_run模式equity=%.2f<=0跳过硬止(虚拟equity不阻断)", equity)
    ...
    def _check_daily_drawdown(self, stats: Dict[str, Any], save_callback=None) -> None:
        ...
        if should_stop:
            ...
            # FIX-28 RC-36: dry_run模式下不触发日回撤硬止，避免虚拟equity波动阻断新开仓
            # 根因: dry_run模式下equity来自虚拟交易，强制减仓导致equity异常下降→日回撤>5%→禁止新开仓
            # 修复: dry_run模式仅记录警告，不设置_daily_hard_stop_triggered和_daily_new_open_blocked
            _dry_run_mode = False
            try:
                from config.params_service import get_params_service
                _ps_cfg = get_params_service()
                _dry_run_mode = bool(_ps_cfg.get_bool('dry_run_mode', False))
            except Exception:  # FIX-F: 扩大异常捕获，防止非6类异常导致dry_run检测失败→CRITICAL误触发
                pass
            if not _dry_run_mode:
                try:
                    from order.order_base import get_order_service
            ...
            if not _dry_run_mode:
                try:
                    _sml_self = getattr(self, '_dry_run_active', False)
                    if _sml_self:
                        _dry_run_mode = True
                except Exception:
                    pass
            if _dry_run_mode:
                ...
                # FIX-30 RC-38: guard同步/CRITICAL日志/EventBus/save_callback仅非dry_run时执行
                # 根因: FIX-28仅将标志位放入else分支，但guard同步/CRITICAL日志在if should_stop内无条件执行→402次CRITICAL刷屏
                # 修复: 将guard同步/CRITICAL日志/EventBus/save_callback移入else分支，dry_run模式仅输出WARNING

                try:

                    from position.position_service import get_cross_strategy_risk_guard
                ...
                try:
                    ...
                    from infra.event_bus import get_global_event_bus, DailyDrawdownHaltEvent
