...
def _compute_commission(symbol: str, lots: int, open_time=None, close_time=None,
    ...
    BF-04修复: 添加order_type和exchange参数支持maker/taker费率区分。
    当exchange提供时，从FEE_STRUCTURE_V2查找maker/taker费率。
    """
...
def _calculate_limit_prices(
    ...
    当bar数据中无is_limit_up/is_limit_down字段时，基于前一日收盘价和涨跌停比例自动计算。
    判断逻辑：当前Bar收盘价 >= 涨停价 → is_limit_up=True；当前Bar收盘价 <= 跌停价 → is_limit_down=True

    Args:
        bar: 当前行情Bar
        prev_close: 前一日收盘价（为0时使用bar中的prev_close字段或open价估算）
        instrument_type: 品种类型

    Returns:
        Dict: {"is_limit_up": bool, "is_limit_down": bool, "limit_up_price": float, "limit_down_price": float}
    """
    # 如果bar数据已提供is_limit_up/is_limit_down，直接使用
    ...
    # 判断是否涨跌停：收盘价触及涨跌停价，或最高价=涨停价且收盘接近最高价
...
def _simulate_limit_order_queue(
    ...
    基于Bar成交量计算排队位置，模拟部分成交：
    1. 限价单价格优于当前价 → 排队位置靠前，大概率成交
    2. 限价单价格等于当前价 → 排队位置中等，部分成交
    3. 限价单价格劣于当前价 → 排队位置靠后，可能不成交

    排队位置 = 基于成交量比例估算，参与率越低排队越靠前
    部分成交 = 可用成交量 * 参与率上限
    ...
    Returns:
        Dict: {"filled": bool, "fill_price": float, "fill_lots": int, "queue_time": int, "queue_position": int}
    """
    if not enable_queue:
    ...
    # 基于排队位置和成交量计算可成交量
    _max_participation_rate = 0.15  # 最大参与率15%
    if bar_volume > 0:
...
def _check_safety(
    ...
    """
    # EX-08修复: 回测保证金追保 — 保证金占用率>95%时强制平仓
    if bt.equity > 0 and len(bt.positions) > 0:
    ...
    if is_circuit_breaker_paused and not allow_close:
        ...
        if bar is not None and len(bt.positions) > 0:
            for sym in list(bt.positions.keys()):
                ...
                _limit_info = _calculate_limit_prices(bar, instrument_type=_infer_instrument_type(sym).replace('option_buyer', 'OPTION_ETF').replace('option_seller', 'OPTION_ETF').replace('future', 'FUTURE'))
                ...
                commission = _compute_commission(sym, pos.lots, open_time=pos.open_time, close_time=bar.get("datetime"), is_open=False, exchange_id=_infer_exchange_id(sym), exchange=_infer_exchange_from_id(_infer_exchange_id(sym)))  # P0-4: 按品种计算平仓手续费; N-01修复: symbol→sym; P1-R11-03: 传入exchange_id; P0-D3: 传入exchange
...
def _try_open(
    ...
    # CS-02修复: TVF语义对齐 — 回测侧TVF作为乘数调整（与生产侧对齐）
    # 生产侧: base_size *= tvf_factor (乘数调整)
    # 修复后回测侧: lots = round(lots * tvf_factor) (乘数调整，非上限约束)
    try:
        ...
        if _tvf_lots > 0 and lots > 0:
            # TVF乘数调整（与生产侧语义对齐）
            _tvf_factor = _tvf_lots / lots  # TVF调整因子
            lots = max(1, round(lots * _tvf_factor))  # 乘数调整，最小1手
    ...
    if _omq == 0:
        ...
        if _instr_type in ('option_buyer', 'option_seller'):
            ...
            return
        logger.debug("[P-39] _option_metadata_quality=0 for non-option instrument, proceeding")
    
    # BF-P1-07已修复: 品种差异化撮合策略 — 期权使用更宽滑点+更低参与率
    ...
    if _instrument_type_key in ('option_buyer', 'option_seller'):
        # 期权品种：参与率上限降低至10%，滑点已在_compute_dynamic_slippage_bps中通过INSTRUMENT_SLIPPAGE_MULTIPLIER放大
    ...
    # BF-P1-02已修复: 限价单排队模拟 — maker订单集成排队逻辑
    ...
    if _order_type == "maker" and params.get("enable_queue_simulation", ENABLE_QUEUE_SIMULATION):
        ...
        if not _queue_result["filled"]:
            logger.debug("[BF-P1-02] 限价单排队未成交: %s, 排队位置=%d", symbol, _queue_result["queue_position"])
            return
        if _queue_result["fill_lots"] < actual_lots:
            logger.debug("[BF-P1-02] 限价单排队部分成交: %s, 计划%d手, 排队成交%d手", symbol, actual_lots, _queue_result["fill_lots"])
    ...
    if actual_lots <= 0:
        ...
        logger.debug("[BF-06] 参与率限制: %s 计划%d手, 实际0手, 放弃开仓", symbol, lots)
        return
    _fill_ratio = actual_lots / lots if lots > 0 else 1.0
    if actual_lots < lots:
        logger.debug("[BF-06] 部分成交: %s 计划%d手, 实际%d手", symbol, lots, actual_lots)
        lots = actual_lots
    ...
    commission = _compute_commission(symbol, lots, is_open=True, exchange_id=_infer_exchange_id(symbol), exchange=_infer_exchange_from_id(_infer_exchange_id(symbol)))  # P0-4: 按品种计算双边手续费; P1-R11-03: 传入exchange_id; P0-D3: 传入exchange
    ...
    if params.get("execution_model", "standard") == "institutional":
        ...
        if open_delay > 0:
            if len(bt.pending_orders) >= 100:
                logger.warning("[C-4] pending_orders超过100上限, 放弃最旧订单: %s", bt.pending_orders[0].symbol)
...
def _check_positions(
    ...
    if _omq == 0:
        ...
        if _instr_type in ('option_buyer', 'option_seller'):
            ...
            logger.debug("[BACKTEST] _check_positions: _option_metadata_quality=0 for non-option instrument, proceeding")
    ...
    for sym in list(bt.positions.keys()):
        ...
        if _state_conflict:
            ...
            slip_bps = _compute_dynamic_slippage_bps(close_price, bid_ask, spread_quality=spread_q, bar=bar, params=params)
            slip = close_price * slip_bps / 10000 * pos.lots
            commission = _compute_commission(sym, pos.lots, open_time=pos.open_time, close_time=bar.get("datetime"), is_open=False, exchange_id=_infer_exchange_id(sym), exchange=_infer_exchange_from_id(_infer_exchange_id(sym)))
            net_pnl = pnl - slip - commission
            _safe_equity_add(bt, net_pnl)
            bt.total_trades += 1
...
def _compute_profit_loss_ratio_metrics(closed_trades: List[_ClosedTrade], equity_curve: np.ndarray, strategy_type: str = '', ticks_per_bar: int = 0) -> Dict[str, Any]:
    ...
    if len(equity_curve) > 1:
        ...
        annualized_ret = total_ret * (_get_annualize_factor(strategy_type, ticks_per_bar=ticks_per_bar) / len(equity_curve)) if len(equity_curve) > 0 else 0.0
        cummax = np.maximum.accumulate(equity_curve)
    ...
    # P0-R9-13修复: 存活分析修正盈亏比
    _survival_adjusted_wlr = win_loss_ratio
    try:
        _n_total = len(closed_trades)
        _NON_SURVIVAL_REASONS = frozenset({'stop_loss', 'circuit_breaker_force_close', 'daily_drawdown_force_close'})
        _n_survived = sum(1 for t in closed_trades if getattr(t, 'close_reason', '') not in _NON_SURVIVAL_REASONS)
        if _n_total > 0 and _n_survived < _n_total:
            _survival_rate = _n_survived / _n_total
            _survival_adjusted_wlr = win_loss_ratio * _survival_rate
    except Exception:
        pass

    # P1-R9-08修复: hold_minutes分层和open_reason分组统计
    _hold_buckets = {"short": 0, "medium": 0, "long": 0}
    _reason_counts = {}
    try:
        for t in closed_trades:
            _hm = getattr(t, 'hold_minutes', 0)
            if _hm < 30:
                _hold_buckets["short"] += 1
            elif _hm < 120:
                _hold_buckets["medium"] += 1
            else:
                _hold_buckets["long"] += 1
            _reason = getattr(t, 'open_reason', 'unknown')
    ...
    return {
        ...
        "calmar": calmar,
        "survival_adjusted_win_loss_ratio": _survival_adjusted_wlr,
        "hold_minutes_buckets": _hold_buckets,
        "open_reason_counts": _reason_counts,
    }
...
def _build_backtest_result(
    ...
    Args:
        ticks_per_bar: 每根Bar内tick数(仅tick_fidelity回测传入>0, 用于STAT-01-P0修复)
    """
    ...
    # P0-7修复: 换月成本从净值中扣除
    # R16-P2-014修复: 换月成本建模
    # 回测: 使用固定spread成本+滑点估算
    # 生产: 使用实际交易所价差+手续费
    # 差异原因: 回测无法获取历史价差快照，使用保守估算
    ...
    # BF-P1-10修复: 回测保真度偏差量化估算
    # BF-P1-05已修复: enable_tick_backtest=True时fidelity_score=95
