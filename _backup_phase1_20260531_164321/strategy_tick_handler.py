...
class TickHandlerMixin:
    ...
    # R10-P2-03: 背压机制 — tick队列深度超过阈值时丢弃新tick
    ...
    def on_tick(self, tick: Any) -> None:
        ...
        # R10-P2-03: 背压机制 — 队列深度超过阈值时丢弃tick并告警
        self._tick_queue_depth += 1
        if self._tick_queue_depth > self.MAX_TICK_QUEUE_DEPTH:
            if self._tick_queue_depth % 1000 == 1:  # 每1000条仅输出一次告警，避免日志风暴
                logging.warning(
                    "[R10-P2-03] tick队列深度=%d 超过阈值=%d，丢弃当前tick",
                    self._tick_queue_depth, self.MAX_TICK_QUEUE_DEPTH,
                )
            # R31-P0-04修复: tick丢弃时通知EventBus，使position_service感知行情中断
            try:
                from ali2026v3_trading.event_bus import get_global_event_bus
                _eb = get_global_event_bus()
                _dropped_instrument = self._get_tick_field(tick, 'instrument_id', '')
                _eb.publish('tick_dropped', {'instrument_id': _dropped_instrument, 'reason': 'backpressure', 'timestamp': time.time()})
            except Exception:
                pass
            self._tick_queue_depth -= 1
            return
            ...
            for _inst_id, pos_dict in positions.items():
                for _pid, rec in pos_dict.items():
                    ...
                    _result = check_hard_time_stop_for_position(
                        ...
                        bar_time=getattr(self, '_current_bar_time', None)
                    )
    ...
    def _process_tick(self, tick: Any) -> None:
        ...
        try:
            ...
            if (not isinstance(last_price, (int, float))
                or math.isnan(last_price) or math.isinf(last_price) or last_price <= 0):
                ...
                if self._iv01_drop_count <= 10 or self._iv01_drop_count % 1000 == 0:
                    logging.warning("[R24-P0-IV-01] Tick dropped: invalid last_price=%s instrument=%s (total_dropped=%d)",
                                   last_price, instrument_id, self._iv01_drop_count)
                return
    ...
    def _output_periodic_summary(self) -> None:
        ...
        """
        try:
            uptime = datetime.now(_CHINA_TZ) - self._stats['start_time'] if self._stats['start_time'] else timedelta(0)
...
# TODO: 将for循环中的.append改为列表推导式，例如:
...
# TODO: 将函数内延迟import移到模块顶部，仅在解决循环依赖时保留函数内import
