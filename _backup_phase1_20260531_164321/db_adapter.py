...
def execute_parquet_query(conn: Any, parquet_path: str, query: str,
    ...
    """
    full_query = f"SELECT * FROM read_parquet('{parquet_path}') AS {table_alias}"
    if query.strip().upper().startswith("SELECT"):
        subquery = query.strip()
        combined = f"WITH {table_alias} AS (SELECT * FROM read_parquet('{parquet_path}')) {subquery}"
...
def manage_schema(conn: Any, table_name: str, columns: Dict[str, str],
    ...
    sql = f"CREATE TABLE {clause} {table_name} ({cols_def})"
    execute(conn, sql)
