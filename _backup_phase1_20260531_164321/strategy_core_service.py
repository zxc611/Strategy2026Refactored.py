...
try:
    from ali2026v3_trading.参数池.delay_time_sharpe_3d import LiveStrategySelector
except ImportError as _e1:
    logger.warning("[R13-P1-DEAD-03] LiveStrategySelector主路径导入失败: %s, 尝试回退路径", _e1)
    try:
        from ali2026v3_trading.live_strategy_selector import LiveStrategySelector
    except ImportError as _e2:
        logger.warning("[R13-P1-DEAD-03] LiveStrategySelector回退路径也失败: %s, 功能不可用", _e2)
...
class StrategyCoreService(_LifecycleMixin, _InstrumentHelperMixin,
    ...
    def __init__(self, strategy_id: str = None, event_bus: Optional[Any] = None):
        ...
        # TODO: 实现网络分区检测器(NetworkPartitionDetector)和降级状态机
    ...
    def _auto_recovery_flow(self) -> bool:
        ...
        try:
            ...
            with open(_recovery_log_path, 'a', encoding='utf-8') as f:
                f.write(json_dumps(_recovery_record) + '\n')
        except Exception as e:
    ...
    def _validate_mixin_attributes(self) -> None:
        ...
        _REQUIRED_ATTRS = {
            '_LifecycleMixin': ['_state', '_state_lock', '_lock', '_storage', '_storage_lock'],
            '_InstrumentHelperMixin': ['_future_ids', '_option_ids'],
    ...
    def _ensure_order_service(self) -> None:
        if self._order_service is not None:
            return
        # R27-P0-RC-01修复: 加锁双重检查，防止并发初始化导致多实例
        with self._order_service_init_lock:
            ...
            try:
                ...
                logging.debug("[StrategyCoreService] OrderService initialized (singleton)")  # R13-P2-LOG-07修复
            except Exception as e:
                logging.warning("[StrategyCoreService] Failed to initialize OrderService: %s", e)  # R13-P2-LOG-01修复
                self._order_service = None
    ...
    def _ensure_position_service(self) -> None:
        ...
        try:
            ...
            logging.debug("[StrategyCoreService] PositionService initialized (scope=%s)", scope_id)  # R13-P2-LOG-07修复
        except Exception as e:
            logging.warning("[StrategyCoreService] Failed to initialize PositionService: %s", e)  # R13-P2-LOG-01修复
            self._position_service = None
    ...
    def _ensure_hft_engine(self) -> None:
        ...
        try:
            ...
            logging.debug("[StrategyCoreService] HFT增强引擎已初始化(七大竞争优势-分散架构)")  # R13-P2-LOG-07修复
        except Exception as e:
            logging.warning("[StrategyCoreService] HFT增强引擎初始化失败: %s", e)  # R13-P2-LOG-01修复
            self._hft_engine = None
    ...
    def _ensure_snapshot_collector(self, symbol: str = "") -> None:
        ...
        if self._snapshot_collector is not None:
            return
        try:
            from ali2026v3_trading.策略评判.market_snapshot_collector import MarketSnapshotCollector
    ...
    def _feed_shadow_engine(self, targets: list, open_reason: str) -> None:
        ...
        try:
            ...
            try:
                # R10-三对齐修复: 五态映射补全defensive状态
                state_to_group = {  # R25-SE-P1-02-FIX
                    STRATEGY_MODE_CORRECT_TRENDING: "s2_resonance",
                    STRATEGY_MODE_CORRECT_TRENDING_DEFENSIVE: "s2_resonance",
                    STRATEGY_MODE_INCORRECT_REVERSAL: "s2_resonance",
                    "incorrect_reversal_defensive": "s2_resonance",
                    STRATEGY_MODE_OTHER: "s2_resonance",
                    "hft": "s1_hft",
                    "box": "s3_box",
                    "spring": "s4_spring",
                    "arbitrage": "s5_arbitrage",
                    "market_making": "s6_market_making",
                }
                strategy_group = state_to_group.get(market_state, "s2_resonance")
            except Exception:
                logging.warning("[R22-EP-P1] StrategyCoreService exception swallowed")
                pass
            for t in targets:
...
class Strategy2026(BaseStrategy, UIMixin):
    ...
    def _ensure_runtime_config_loaded(self) -> None:
        ...
        logging.info("[Strategy2026] 从缓存参数表加载了 %d 个参数", len(loaded_config))  # R13-P2-LOG-01修复
    ...
    def onStop(self):
        ...
        try:
            ...
            logging.error("[Strategy2026.onStop] BaseStrategy cleanup failed: %s", e)  # R13-P2-LOG-01修复
