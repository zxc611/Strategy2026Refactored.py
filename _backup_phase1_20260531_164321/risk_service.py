    ...
    def check_before_trade(self, signal: Dict[str, Any]) -> RiskCheckResponse:
        ...
        with self._risk_check_lock:
            try:
                ...
                result = self._check_operational_risks(signal, action)
                if result is not None:
                    return self._record_result(result)
                result = self._check_market_risks(signal, action)
                if result is not None:
                    return self._record_result(result)
                result = self._check_counterparty_risks(signal, action)
                if result is not None:
                    return self._record_result(result)
                result = self._check_regulatory_risks(signal, action)
                if result is not None:
                    return self._record_result(result)
            except Exception as e:
            ...
            if result is not None:
                return result
            return self._record_and_publish(signal, _check_dedup_key, _cyclic_guard)
            ...
            try:
                ...
                if _sse is not None:
                    if _sse.is_absolute_ev_paused():
                        self._fire_alert('ev_bottom', {'signal': signal})
...
class RiskDashboardService:
    ...
    [P1-1审计] 方法数=6 | 调用方=event_bus事件拓扑/strategy_core_service注释 | 与RiskService耦合=弱(间接) | 重构判定=提取→独立模块risk_dashboard.py
    """
...
class AlertLevel(Enum):
    ...
    [P1-1审计] 方法数=0(纯枚举) | 调用方=health_check_api/risk_service._check_safety_meta_layer/SafetyMetaLayer.confirm_daily_resume | 与RiskService耦合=强 | 重构判定=保留→随alert体系提取到alert_service.py
    """
...
class AlertDeduplicator:
    ...
    [P1-1审计] 方法数=3 | 调用方=alert()函数→RiskService._fire_alert() | 与RiskService耦合=间接(通过alert()函数) | 重构判定=提取→随AlertLevel一起提取到alert_service.py
    """
...
class SimplifiedSPAN:
    ...
    [P1-1审计] 方法数=3 | 调用方=task_scheduler.py | 与RiskService耦合=无(完全独立) | 重构判定=提取→独立模块span_calculator.py
    """
