...
from ali2026v3_trading.strategy_config import (
...
from ali2026v3_trading.config_exchange_data import (
...
def invalidate_strategy_cache(strategy_id: str) -> bool:
    ...
    from ali2026v3_trading.strategy_config import invalidate_strategy_cache as _sc_fn
...
def get_param_age_summary() -> Dict[str, Any]:
    ...
    from ali2026v3_trading.strategy_config import get_param_age_summary as _sc_fn
    return _sc_fn()


def subscribe_param_changes(callback) -> None:
    """[FR-P1-10-FIX] 参数变更事件订阅 — 委托到strategy_config"""
    from ali2026v3_trading.strategy_config import subscribe_param_changes as _sc_fn
    _sc_fn(callback)


def _notify_param_change(param_key: str, old_val: Any, new_val: Any) -> None:
    """[FR-P1-10-FIX] 参数变更事件发布通知 — 委托到strategy_config"""
    from ali2026v3_trading.strategy_config import _notify_param_change as _sc_fn
...
def check_multi_level_cache_consistency() -> Dict[str, bool]:
    ...
    from ali2026v3_trading.strategy_config import check_multi_level_cache_consistency as _sc_check
    return _sc_check()


def record_pipeline_latency(stage: str, latency_sec: float) -> None:
    """[FR-P1-12-FIX] 数据管道延迟监控 — 委托到strategy_config"""
    from ali2026v3_trading.strategy_config import record_pipeline_latency as _sc_fn
    _sc_fn(stage, latency_sec)


def mark_computation_timestamp(computation_id: str) -> None:
    """[FR-P1-13-FIX] 计算结果时间戳标记 — 委托到strategy_config"""
    from ali2026v3_trading.strategy_config import mark_computation_timestamp as _sc_fn
...
def check_indicator_freshness(indicator_key: str, max_age_sec: float = 60.0) -> bool:
    ...
    from ali2026v3_trading.strategy_config import check_indicator_freshness as _sc_fn
...
def record_indicator_value(indicator_key: str, value: Any) -> None:
    ...
    from ali2026v3_trading.strategy_config import record_indicator_value as _sc_fn
...
def check_risk_data_freshness(risk_key: str, max_age_sec: float = 5.0) -> bool:
    ...
    from ali2026v3_trading.strategy_config import check_risk_data_freshness as _sc_fn
...
def update_risk_data_timestamp(risk_key: str) -> None:
    ...
    from ali2026v3_trading.strategy_config import update_risk_data_timestamp as _sc_fn
    _sc_fn(risk_key)
...
from ali2026v3_trading.security_config import _SecureCredential, reveal_credential
...
def _verify_default_param_table_integrity():
    ...
    from ali2026v3_trading.resilience_config import _verify_default_param_table_integrity as _rc_fn
    _rc_fn()
...
from ali2026v3_trading.config_version_tracker import (
...
def update_cached_params(updates: Dict[str, Any], sync_default_table: bool = True, caller_id: str = "unknown", strategy_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if caller_id not in _TRUSTED_UPDATE_CALLERS:
        ...
        try:
            from ali2026v3_trading.audit_log_utils import structured_audit_log as _sal_unauth
    ...
    for _k, _v in normalized_updates.items():
        try:
            from ali2026v3_trading.audit_log_utils import structured_audit_log as _sal
...
from ali2026v3_trading.config_json_loader import (
...
def get_params_metadata(json_path: Optional[str] = None) -> Dict[str, Any]:
    from ali2026v3_trading.config_resolver import get_params_metadata as _fn
    return _fn(json_path)


def validate_params(params: Dict[str, Any], json_path: Optional[str] = None) -> Dict[str, Any]:
    from ali2026v3_trading.config_resolver import validate_params_with_metadata as _fn
    return _fn(params, json_path)


def apply_environment(params: Dict[str, Any], env: str) -> Dict[str, Any]:
    from ali2026v3_trading.config_resolver import apply_environment as _fn
    return _fn(params, env)
    return merged


def rebuild_default_param_table(env: Optional[str] = None) -> Dict[str, Any]:
    from ali2026v3_trading.config_resolver import apply_environment
...
def _normalize_option_params_payload(payload: Any, source_path: str) -> Dict[str, Dict[str, Any]]:
    from ali2026v3_trading.config_option_loader import _normalize_option_params_payload as _fn
    return _fn(payload, source_path)

def load_option_params_from_file() -> Dict[str, Any]:
    from ali2026v3_trading.config_option_loader import load_option_params_from_file as _fn
    return _fn()

def merge_option_params_to_default():
    from ali2026v3_trading.config_option_loader import merge_option_params_to_default as _fn
    _fn()

def _sync_defaults_from_attribute_matrix() -> None:
    from ali2026v3_trading.config_sync import _sync_defaults_from_attribute_matrix as _fn
    _fn()

def verify_param_pool_sync():
    from ali2026v3_trading.config_sync import verify_param_pool_sync as _fn
    return _fn()
...
def resolve_config_with_priority(
    ...
    from ali2026v3_trading.config_resolver import resolve_config_with_priority as _rcwp
...
from ali2026v3_trading.config_resolver import _DOCUMENTED_ENV_VARS
...
from ali2026v3_trading.data_quality_config import (
...
from ali2026v3_trading.config_resolver import _data_quality_score
...
from ali2026v3_trading.config_state_defaults import (
