        ...
        for _inst_id, pos_dict in positions.items():
            for _pid, rec in pos_dict.items():
                if isinstance(rec, dict):
                    ...
                    continue
                from ali2026v3_trading.strategy.tick_hft import check_hard_time_stop_for_position
...
def process_tick_core(svc, tick: Any) -> None:
    ...
    try:
        ...
        if _hmm_state is not None:
            from ali2026v3_trading.strategy.tick_hft import check_hmm_dwell_anomaly
...
def tick_preflight_check(svc, tick) -> bool:
    ...
    if svc._tick_overload_until > _now:
        if not hasattr(svc, '_backpressure_drop_count'):
            svc._backpressure_drop_count = 0
        svc._backpressure_drop_count += 1
        if svc._backpressure_drop_count <= 10 or svc._backpressure_drop_count % 10000 == 1:
            logging.warning("[FIX-20260701] tick背压丢弃: remaining=%.1fs total=%d",
                            svc._tick_overload_until - _now, svc._backpressure_drop_count)
        try:
            from ali2026v3_trading.infra.event_bus import get_global_event_bus
            _eb = get_global_event_bus()
