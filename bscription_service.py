...
class SubscriptionCoreService:
    ...
    def audit_subscription_tick_gap(self, reason: str = 'periodic') -> Dict[str, int]:
        ...
        if subscribed_count == 0:
            try:
                # FIX-20260702: 订阅审计 fallback 不再裸连 DuckDB，统一走 DataService.query
                from ali2026v3_trading.data.data_service import get_data_service
                ds = get_data_service()
                ...
                logging.debug("[SubscribeTickGapAudit] instruments_registry 取数失败: %s", exc)
        ...
        if subscribed_count > 0:
            ...
            if gap_pct >= 90:
                ...
                if _in_warmup:
                    logging.warning(
                        "[SubscribeTickGapAudit] tick覆盖率低但处于启动宽限期(<%ds)，订阅建立中，降级为warning: local_subscription_calls=%d received=%d gap=%d%%",
