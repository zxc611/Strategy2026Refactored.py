...
class StrategyBehaviorDiagnosis:
    ...
    def _diagnose_signal(self, high_snaps, low_snaps, all_snapshots) -> DimensionDiagnosis:
        ...
        decay_consistency = SubItemScore(
            ...
            confidence=min(1.0, len(decay_scores) / 30),
            pattern="信号衰减" if decay_rate > 0.6 else "信号未衰减",
        ...
        direction_consistency = SubItemScore(
            ...
            pattern="方向稳定" if jump_rate < 0.2 else "方向跳变",
            detail=f"跳变率={jump_rate:.2f}",
        )

        should_decay = decay_rate > 0.5
        if should_decay:
            score = 0.6 * decay_rate + 0.4 * (1.0 - jump_rate)
        else:
            score = 0.4 * (1.0 - decay_rate) + 0.6 * (1.0 - jump_rate)
        score = float(np.clip(score, 0, 1))

        if jump_rate > 0.3:
            severity = DiagnosisSeverity.CRITICAL
            pattern = "信号异常跳变"
        elif decay_rate > 0.6 if should_decay else decay_rate < 0.4:
            severity = DiagnosisSeverity.HEALTHY
        ...
        return DimensionDiagnosis(
            ...
            sample_count=total, confidence=min(1.0, total / 30),
    ...
    def _diagnose_position(self, high_snaps, low_snaps) -> DimensionDiagnosis:
        ...
        smooth_score = max(0.0, 1.0 - cv / 3.0) if cv < 3.0 else 0.0
        decrease_count = 0
        for i in range(1, len(all_snaps)):
        ...
        smooth_sub = SubItemScore(
            ...
            confidence=min(1.0, len(position_deltas) / 30),
            pattern="平滑调整" if osc_rate < 0.1 else "剧烈震荡",
        ...
        decrease_sub = SubItemScore(
            ...
            confidence=min(1.0, total / 30),
            pattern="缩减" if decrease_rate > 0.5 else "未缩减",
            detail=f"缩减率={decrease_rate:.2f}",
        )

        if osc_rate < 0.1 and cv < 2.0:
            score = 0.5 * smooth_score + 0.3 * decrease_rate + 0.2
            severity = DiagnosisSeverity.HEALTHY
            pattern = "仓位平滑调整"
        elif osc_rate > 0.3:
            score = max(0.0, 0.4 - osc_rate * 0.4)
            severity = DiagnosisSeverity.CRITICAL
            pattern = "仓位剧烈震荡"
        else:
            score = 0.4 * smooth_score + 0.3 * decrease_rate + 0.2
            severity = DiagnosisSeverity.WARNING
            pattern = "仓位部分异常"
    ...
    def _diagnose_pnl(self, high_snaps, low_snaps) -> DimensionDiagnosis:
        ...
        if len(negative) == 0:
            ...
            path_score = max(0.0, 0.6 + 0.3 * (1.0 - cliff_ratio / 0.3)) if cliff_ratio < 0.3 else max(0.0, 0.5 - cliff_ratio)

        gradual_score = max(0.0, 1.0 - cliff_ratio / 0.3) if cliff_ratio < 0.3 else 0.0

        path_sub = SubItemScore(
            name="盈亏路径质量", score=path_score,
            sample_count=total, confidence=min(1.0, total / 30),
            pattern="渐进回吐" if cliff_ratio < 0.3 else "断崖回吐",
        ...
        gradual_sub = SubItemScore(
            ...
            sample_count=total, confidence=min(1.0, total / 30),
            pattern="渐进" if gradual_score > 0.5 else "断崖",
    ...
    def _diagnose_greeks(self, high_snaps, low_snaps) -> DimensionDiagnosis:
        ...
        expanding = sum(1 for i in range(1, len(delta_abs)) if delta_abs[i] > delta_abs[i-1] + 0.1)
        ...
        vega_spikes = sum(1 for v in vegas if v > 2 * mean_vega + 0.01) if mean_vega > 0 else 0
        vega_spike_rate = vega_spikes / max(1, len(vegas))
        vega_score = max(0.0, 1.0 - vega_spike_rate * 2)

        delta_sub = SubItemScore(
            name="Delta合理性", score=delta_score,
            sample_count=len(deltas), confidence=min(1.0, len(deltas) / 30),
            pattern="Delta可控" if expand_rate < 0.2 else "Delta扩展",
            detail=f"扩展率={expand_rate:.2f}",
        )

        vega_sub = SubItemScore(
            name="Vega合理性", score=vega_score,
            sample_count=len(vegas), confidence=min(1.0, len(vegas) / 30),
            pattern="Vega可控" if vega_spike_rate < 0.1 else "Vega尖峰",
            detail=f"尖峰率={vega_spike_rate:.2f}",
        )

        greeks_rat = 0.6 * delta_score + 0.4 * vega_score

        if expand_rate > 0.4:
            severity = DiagnosisSeverity.CRITICAL
            pattern = "Greeks过度暴露"
        elif expand_rate < 0.2 and vega_spike_rate < 0.1:
            severity = DiagnosisSeverity.HEALTHY
            pattern = "Greeks可控"
        else:
            severity = DiagnosisSeverity.WARNING
    ...
    def _diagnose_cross_strategy(self, high_snaps, low_snaps) -> DimensionDiagnosis:
        ...
        for s in all_snaps:
            ...
            if all_same and all(abs(sgn) > 0.3 for sgn in signals):
                correlated_count += 1
        ...
        score = max(0.0, 1.0 - corr_rate * 1.5)
