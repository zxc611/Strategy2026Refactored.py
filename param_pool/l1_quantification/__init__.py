...
def __getattr__(name: str):
    if name == "MetaAuditEngine":
        from precompute.meta_audit_engine import MetaAuditEngine
    ...
    if name == "ExternalSourceConfig":
        from precompute._data_validation import ExternalSourceConfig
        return ExternalSourceConfig
    if name in ("TierConfig", "PerformanceTierManager"):
        from precompute._quantification_core import TierConfig, PerformanceTierManager
        return locals()[name]
    if name == "get_tier_config":
        from precompute._quantification_core import PerformanceTierManager
        return PerformanceTierManager.get_tier_config
    if name == "STATES":
        from precompute._quantification_core import TripleTruthAnchor
