...
def _ensure_interpolate_ticks():
    ...
    if _interpolate_ticks_in_bar is None:
        from param_pool.ts.ts_result_writer import _interpolate_ticks_in_bar as _fn
...
def run_backtest_arbitrage(
    ...
    bar_data: pd.DataFrame,
    train: bool = True,
    strategy_type: str = "s5_arbitrage",
) -> Dict[str, Any]:
...
def run_backtest_market_making(
    ...
    bar_data: pd.DataFrame,
    train: bool = True,
    strategy_type: str = "s6_market_making",
) -> Dict[str, Any]:
    ...
    try:
        from param_pool._preprocess import validate_circuit_breaker_halts
...
def run_backtest_divergence(
    ...
    bar_data: pd.DataFrame,
    train: bool = True,
    strategy_type: str = "s7_divergence",
) -> Dict[str, Any]:
...
def __getattr__(name):
    ...
    if name in _HFT_NAMES:
        from param_pool.backtest import _backtest_runners_hft
