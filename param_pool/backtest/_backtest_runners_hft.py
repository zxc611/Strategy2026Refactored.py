...
from infra.shared_utils import ANNUALIZE_FACTOR_DAILY
from infra.resilience import should_trigger_stop_loss, should_trigger_take_profit
...
from param_pool.backtest._backtest_fidelity import (
...
from param_pool.ts.ts_backtest_strategies import BAR_INTERVAL_GRID
...
def run_backtest_hft_tick_fidelity(
    ...
    unsupported = [s for s in supported_strategies if s not in ('S1', 's1_hft')]
    if unsupported:
...
def _get_load_multiscale_data():
    from param_pool.ts.ts_result_writer import _load_multiscale_data
    return _load_multiscale_data


def _get_infer_hmm_state_from_iv():
    from param_pool.validation.validation_deep_orchestrator import _infer_hmm_state_from_iv
...
def run_backtest_multiscale(
    ...
    bt_func = {
        "s1_hft": run_backtest_hft,
        "s2_resonance": run_backtest,
        "s3_box": run_backtest_box_extreme,
        "s4_spring": run_backtest_box_spring,
        "s5_arbitrage": run_backtest_arbitrage,
        "s6_market_making": run_backtest_market_making,
    ...
    result["kline_fidelity"] = "tick_interpolated" if strategy == "s1_hft" else "bar_exact"
    return result
...
def run_backtest_with_cycle_resonance(
    ...
    跳过运行时CRM计算，大幅减少回测耗时。'
    """
    from param_pool.optimization.cycle_sharpe import CycleResonanceModule, CycleResonanceOutput, Phase  # noqa: F401
    ...
    def _get_run_fn(s: str):
        ...
        from param_pool.backtest.backtest_runner_base import run_backtest as _rb
        _fn_map = {
            "s2_resonance": _rb,
            "s3_box": run_backtest_box_extreme,
            "s4_spring": run_backtest_box_spring,
