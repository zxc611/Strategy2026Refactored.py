...
from infra.shared_utils import compute_slippage_bps, UNIFIED_EXPIRY_SLIPPAGE_MULTIPLIERS, compute_execution_delay_slippage_bps
# compute_profit_loss_ratio_metrics defined locally in this module
from param_pool.backtest.backtest_config import compute_profit_loss_ratio_metrics as _compute_plr_metrics_delegated
try:
...
from infra.shared_utils import ANNUALIZE_FACTOR_DAILY, ANNUALIZE_FACTOR_MINUTE
from infra.shared_utils import get_annualize_factor as _get_annualize_factor
...
from data.db_adapter import connect_duckdb
...
from infra.commission_utils import REASON_MULTIPLIERS, P0_IRON_RULES
...
from param_pool.backtest.backtest_config import BacktestStateEnum, _STATE_REASON_MAP
...
def set_global_random_seed(seed: int) -> int:
    ...
    """
    from infra.shared_utils import set_global_seed
...
def _select_top_k_train(results: List[Dict[str, Any]], metric: str, k: int,
    ...
    from param_pool._param_defaults import PARAM_SORT_METRIC_OVERRIDE
...
def _try_open(
    ...
    from param_pool import backtest_state as _bpm
    ...
    # [FIX-EXEC-V2-P1-1-REAL] V2 模式下通过 OrderService 下单，真正替代 try_open_execute
    if v2_order_service is not None:
        try:
            _direction_str = 'BUY' if direction > 0 else 'SELL'
            ...
            bt.total_signals += 1
            return
        except Exception as _v2_send_err:
            # 实时回调路径硬约束: except Exception
            logger.warning("[run_backtest V2] _try_open 通过 OrderService 下单失败,降级 legacy: %s", _v2_send_err)
            # fallthrough 到 legacy try_open_execute
...
def _check_positions(
    ...
    from param_pool import backtest_state as _bpm2
    from param_pool._param_grids import BACKTEST_THRESHOLDS
    ...
    for sym in list(bt.positions.keys()):
        ...
        if hasattr(bt, 'current_state') and hasattr(pos, 'open_reason'):
            if bt.current_state == "incorrect_reversal" and "correct" in str(pos.open_reason):
                ...
                _state_conflict = True
        if _state_conflict:
            # [FIX-EXEC-V2-P1-1-REAL] V2 模式下通过 OrderService 平仓
            if v2_order_service is not None:
                try:
                    _close_direction = 'SELL' if pos.volume > 0 else 'BUY'
                    _close_price = bar.get("close", 0.0)
                    v2_order_service.send_order(
                        instrument_id=sym,
                        volume=float(pos.lots),
                        price=float(_close_price),
                        direction=_close_direction,
                        action='CLOSE',
                        open_reason='state_conflict_force_close',
                    )
                    continue
                except Exception as _v2_close_err:
                    logger.warning("[run_backtest V2] state_conflict 通过 OrderService 平仓失败,降级 legacy: %s", _v2_close_err)
            from param_pool import backtest_safety_checker as _bsc2
    ...
    _used_v2_close = False
    if should_close:
        # [FIX-EXEC-V2-P1-1-REAL] V2 模式下通过 OrderService 平仓，真正替代 legacy 平仓计算
        if v2_order_service is not None:
            try:
                _close_direction = 'SELL' if pos.volume > 0 else 'BUY'
                v2_order_service.send_order(
                ...
                # 实时回调路径硬约束: except Exception
                logger.warning("[run_backtest V2] _check_positions 通过 OrderService 平仓失败,降级 legacy: %s", _v2_close_err)
                _used_v2_close = False

        if not _used_v2_close:
...
def _build_backtest_result(
    ...
    from param_pool.backtest.backtest_config import DEFAULT_RISK_FREE_RATE, INITIAL_EQUITY
    from infra.commission_utils import detect_rollover_gaps, compute_rollover_cost
    ...
    try:
        from risk.risk_service import calculate_var_historical
...
from param_pool.backtest.backtest_runner_validation import validate_shadow_param_independence  # F-09: 权威版在backtest_runner_validation，此处re-export
...
def _resolve_time_stop(params: Dict[str, float], open_reason: str, current_state: str = None) -> float:
    ...
    - 箱体: box_hard_time_stop_min → 直接使用
    """
    from param_pool.backtest.backtest_state import _resolve_time_stop_hard, _get_life_estimator
    from infra.commission_utils import REASON_MULTIPLIERS
