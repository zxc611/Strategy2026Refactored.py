...
def __getattr__(name):
    if name == 'optimize_l2_params_step1':
        from param_pool.validation_l2_hyperparams import optimize_l2_params_step1
        return optimize_l2_params_step1
    if name == 'run_backtest':
        from param_pool.backtest_runner_base import run_backtest
        return run_backtest
    if name == '_load_data_for_period':
        from param_pool.ts_result_writer import _load_data_for_period
        return _load_data_for_period
    if name in __all__:
        import importlib
        _mod = importlib.import_module('param_pool.backtest_config')
