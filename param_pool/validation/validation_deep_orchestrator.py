...
import logging
from param_pool._param_grids import DEEP_VALIDATION_TIERS
...
def _ensure_runner():
    ...
    if _runner_module is None:
        from param_pool import backtest_runner_base as _br
        _runner_module = _br
...
def _get_initial_equity() -> float:
    from param_pool.backtest.backtest_config import INITIAL_EQUITY
...
def run_deep_validation_tiered(
    ...
    if tier == "annual":
        ...
        try:
            from param_pool.optimization.sensitivity import SensitivityAnalyzer
        ...
        try:
            from param_pool.validation.statistical_validation import WalkForwardValidator
...
def run_deep_validation_suite(
    ...
    # 7. 影子参数独立性验证
    try:
        from param_pool.backtest.backtest_runner_validation import validate_shadow_param_independence
...
def run_validation_with_registry(backtest_result, validation_funcs=None):
    ...
    try:
        from governance.validation_registry import ValidationRegistry, BacktestValidator, BacktestResult, ValidationResult
...
from risk.crack_validation import validate_indicator_multiscale_consistency as validate_multiscale_indicator_consistency
...
def validate_hmm_online_vs_offline(iv_series: pd.Series,
    ...
    try:
        from data.quant_core import AdaptiveHMM
    except ImportError:
...
def validate_trend_score_bar_vs_tick_correlation(
    ...
    from param_pool.backtest.backtest_runner_validation import _infer_trend_scores_from_bar
...
def validate_rollover_impact(bar_data: pd.DataFrame,
    ...
    差异>10%则必须剔除展期数据。
    """
    from param_pool.backtest.backtest_runner_utils import exclude_rollover_signals
    from infra.commission_utils import detect_rollover_gaps
...
def validate_hft_temporal_robustness(
    ...
    from param_pool.backtest._backtest_runners_hft import run_backtest_hft_with_disturbance
...
def validate_market_friendliness_baseline(
    ...
    from param_pool.backtest.backtest_runner_utils import _sync_random_seed
...
def __getattr__(name):
    ...
    if name in _LAZY_BRB_NAMES:
        from param_pool.backtest.backtest_runner_utils import (
            detect_rollover_gaps,
            exclude_rollover_signals,
        )
        from param_pool.backtest.backtest_runner_base import (
            run_backtest,
            INITIAL_EQUITY,
            SLIPPAGE_BPS,
        )
        from param_pool._param_grids import _sync_random_seed
        from param_pool.backtest.backtest_runner_validation import _infer_trend_scores_from_bar
