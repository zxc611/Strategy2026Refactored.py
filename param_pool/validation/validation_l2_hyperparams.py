...
import itertools
import logging
from param_pool._param_grids import L2_HYPERPARAMS, L2_PARAM_GRID, L2_CONFLICT_RESOLUTION, PARAM_SOURCE_ANNOTATION
...
from param_pool._param_grids import _sync_random_seed, _BacktestState
...
def run_step2_smoke_test(
    ...
    from param_pool.backtest.backtest_runner_utils import _reset_daily
    from param_pool.backtest.backtest_runner_base import _check_state_transition
    ...
    for idx in range(len(bar_data)):
        ...
        for sym in list(bt.positions.keys()):
            if sym == bar.get("symbol", ""):
                from param_pool.backtest.backtest_runner_base import _check_positions
