...
def _ensure_validation():
    ...
    if _validation_module is None:
        from param_pool import backtest_state as _bv
...
def _ensure_runner():
    ...
    if _runner_module is None:
        from param_pool import backtest_runner as _br
        _runner_module = _br
...
def _worker_init(train_data_shared: pd.DataFrame, test_data_shared: pd.DataFrame) -> None:
    ...
    from infra.shared_utils import set_global_seed  # P2-23: 统一随机种子入口
...
def _worker_task(task: dict) -> dict:
    ...
    try:
        if l2_result and l2_result.get("qualified"):
            from param_pool.optimization.l2_optimizer import L2Optimizer
    ...
    try:
        ...
        arb_shadow_a_params.update(STRATEGY_SHADOW_DEFAULTS["s5_arbitrage"]["shadow_a"])
        arb_shadow_b_params = {**params}
        arb_shadow_b_params.update(STRATEGY_SHADOW_DEFAULTS["s5_arbitrage"]["shadow_b"])

        arb_main = _safe_backtest("S5_main", _ensure_runner().run_backtest_arbitrage, arb_params, bar_data, task["train"], "s5_arbitrage")
    ...
    try:
        ...
        mm_shadow_a_params.update(STRATEGY_SHADOW_DEFAULTS["s6_market_making"]["shadow_a"])
        mm_shadow_b_params = {**params}
        mm_shadow_b_params.update(STRATEGY_SHADOW_DEFAULTS["s6_market_making"]["shadow_b"])

        mm_main = _safe_backtest("S6_main", _ensure_runner().run_backtest_market_making, mm_params, bar_data, task["train"], "s6_market_making")
    ...
    try:
        from param_pool.quantification.external_validation_pipeline import ExternalValidationPipeline
    ...
    try:
        from param_pool.quantification.triple_truth_anchor import TripleTruthAnchor
    ...
    try:
        from param_pool.quantification.performance_tier_manager import PerformanceTierManager
...
from infra.commission_utils import (  # noqa: F401
    P0_IRON_RULES,
    REASON_MULTIPLIERS,
