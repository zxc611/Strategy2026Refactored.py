...
class CycleResonanceModule:
    ...
    def _compute_phase(
        ...
        if len(self._strength_history) >= 3:
            recent = list(self._strength_history)[-3:]
    ...
    def get_risk_surface(
        ...
        if output is None:
            # FIX-20260715-CRM-AUTOINIT: get_risk_surface被调用时若_last_output仍为None，
            # 说明PQS路径未成功更新CRM。被动等待会导致所有策略使用保守默认max_hold=1800s，
            # 与high_freq等策略的1分钟hold严重冲突。修复: 自动用NORMAL默认参数执行一次update()，
            # 使后续调用获得合理的风险曲面。
            try:
                self.update(
                    hmm_state='NORMAL',
                ...
                output = self._last_output
                logging.info(
                    '[CycleResonanceModule] get_risk_surface: _last_output=None，已自动执行默认update() '
                    'strategy=%s', strategy,
                )
            except (ValueError, KeyError, TypeError, RuntimeError, AttributeError) as _crm_auto_err:
                logging.warning(
                    '[CycleResonanceModule] get_risk_surface: _last_output=None且自动update失败: %s，'
                    'strategy=%s 使用保守默认max_hold=1800s', _crm_auto_err, strategy,
                )
                return RiskSurfaceAdjustment()
        if output is None:
            logging.warning(
                '[CycleResonanceModule] get_risk_surface: _last_output仍为None，'
                'strategy=%s 使用保守默认max_hold=1800s', strategy,
            )
            return RiskSurfaceAdjustment()

        is_co_aligned = output.directional_bias * (1 if strategy in ("s1_hft", "s2_resonance") else 0) >= 0
    ...
    def get_signal_priority(self, phase: Phase, volatility_regime: Optional[str] = None) -> List[str]:
        ...
        """
        base_priority = {
            Phase.RELEASE: ["s2_resonance", "s4_spring", "s1_hft", "s3_box"],
            Phase.CHARGE: ["s4_spring", "s1_hft", "s2_resonance", "s3_box"],
            Phase.EXHAUST: ["s1_hft", "s3_box", "s4_spring", "s2_resonance"],
            Phase.CHAOS: ["s1_hft", "s2_resonance", "s3_box", "s4_spring", "s5_arbitrage", "s6_market_making"],  # R27-P1-FIX: 补充arbitrage和market_making策略
        }.get(phase, ["s1_hft", "s2_resonance", "s3_box", "s4_spring", "s5_arbitrage", "s6_market_making"])  # R27-P1-FIX: 补充arbitrage和market_making策略

        if volatility_regime == 'extreme':
            base_priority = [p for p in base_priority if p not in ('s4_spring',)]
            base_priority.append('s4_spring')
        elif volatility_regime == 'low':
            if 's3_box' in base_priority:
                idx = base_priority.index('s3_box')
                if idx > 0:
...
# ── Sharpe 3D Mapping (merged from sharpe_3d_mapping.py) ──

from dataclasses import dataclass

try:
    # 五唯一性修复：import 路径统一为 data.data_access / data.db_adapter
...
try:
    ...
    try:
        ...
        try:
            from param_pool.backtest.backtest_config import SLIPPAGE_BPS as _bt_slippage
            SLIPPAGE_BPS = _bt_slippage
        except (ImportError, AttributeError):
            SLIPPAGE_BPS = 5.0

# P2-R11-07修复: DuckDB路径使用sharpe_3d/子目录
...
# P-10修复: DELAY_TIERS从20档位收缩为6核心档位
...
# C-18修复: STRATEGY_NAMES扩展为6策略
STRATEGY_NAMES = ["s1_hft", "s2_resonance", "s3_box", "s4_spring", "s5_arbitrage", "s6_market_making"]

def _build_time_params():
    T = TimeParams
    return {
        "s1_hft": {d: T(m, tk, 0, d, pm) for d, m, tk, pm in [
            (0,"immediate",0,"momentum"),(10,"lead",1,"momentum"),(15,"lead",1,"momentum"),(20,"lead",1,"momentum"),
            (25,"lead",1,"momentum"),(30,"lead",1,"momentum"),(35,"lead",1,"momentum"),(40,"lead",2,"momentum"),
            (45,"lead",2,"momentum"),(50,"lead",2,"momentum"),(60,"lead",2,"order_flow"),(70,"lead",3,"order_flow"),
            (80,"lead",3,"order_flow"),(90,"lead",3,"order_flow"),(100,"lead",4,"order_flow"),(120,"lead",4,"order_flow"),
            (140,"lead",5,"microstructure"),(160,"lead",5,"microstructure"),(180,"lead",6,"microstructure"),(200,"lead",6,"microstructure"),
        ]},
        "s2_resonance": {d: T(m, 0, b, d) for d, m, b in [
            (0,"immediate",0),(10,"immediate",0),(15,"immediate",0),(20,"immediate",0),(25,"immediate",0),(30,"immediate",0),
            (35,"lag",1),(40,"lag",1),(45,"lag",1),(50,"lag",1),(60,"lag",1),
            (70,"lag",2),(80,"lag",2),(90,"lag",2),(100,"lag",2),
            (120,"lag",3),(140,"lag",3),(160,"lag",4),(180,"lag",4),(200,"lag",5),
        ]},
        "s3_box": {d: T("lag", 0, b, d) for d, b in [
            (0,0),(10,1),(15,1),(20,1),(25,1),(30,1),(35,1),(40,1),(45,1),(50,1),
            (60,2),(70,2),(80,2),(90,2),(100,2),(120,3),(140,3),(160,4),(180,4),(200,5),
        ]},
        "s4_spring": {d: T(m, tk, b, d, pm) for d, m, tk, b, pm in [
            (0,"immediate",0,0,"momentum"),(10,"lead",1,0,"momentum"),(15,"lead",1,0,"momentum"),(20,"lead",1,0,"momentum"),
            (25,"lead",1,0,"momentum"),(30,"lead",1,0,"momentum"),(35,"lead",2,0,"momentum"),(40,"lead",2,0,"momentum"),
            (45,"lead",2,0,"momentum"),(50,"lead",2,0,"momentum"),(60,"lead",2,0,"order_flow"),(70,"lead",2,1,"order_flow"),
            (80,"lead",2,1,"order_flow"),(90,"lead",3,1,"order_flow"),(100,"lead",3,1,"order_flow"),
            (120,"lead",3,2,"order_flow"),(140,"lead",3,2,"order_flow"),(160,"lag",0,2,"momentum"),
            (180,"lag",0,3,"momentum"),(200,"lag",0,3,"momentum"),
        ]},
        "s5_arbitrage": {d: T("lead", tk, 0, d, "microstructure") for d, tk in [
            (0,0),(10,1),(15,1),(20,1),(25,1),(30,1),(35,2),(40,2),(45,2),(50,2),
            (60,2),(70,3),(80,3),(90,3),(100,3),(120,4),(140,4),(160,5),(180,5),(200,6),
        ]},
        "s6_market_making": {d: T("lead", tk, 0, d, "order_flow") for d, tk in [
            (0,0),(10,1),(15,1),(20,1),(25,1),(30,1),(35,2),(40,2),(45,2),(50,2),
            (60,2),(70,3),(80,3),(90,3),(100,3),(120,4),(140,4),(160,5),(180,5),(200,6),
        ]},
    }

DEFAULT_TIME_PARAMS: Dict[str, Dict[float, TimeParams]] = _build_time_params()


# 策略-参数映射
try:
    from param_pool._param_defaults import (
        PARAM_DEFAULTS, PARAM_DEFAULTS_HFT, PARAM_DEFAULTS_BOX_EXTREME,
        PARAM_DEFAULTS_BOX_SPRING, PARAM_DEFAULTS_ARBITRAGE, PARAM_DEFAULTS_MARKET_MAKING,
    )
    from param_pool.backtest.backtest_runner_base import run_backtest
    from param_pool.backtest.backtest_strategy_runners import (
        run_backtest_box_extreme, run_backtest_box_spring,
        run_backtest_arbitrage, run_backtest_market_making,
    )
    from param_pool.backtest._backtest_runners_hft import run_backtest_hft
except ImportError:
    PARAM_DEFAULTS = PARAM_DEFAULTS_HFT = PARAM_DEFAULTS_BOX_EXTREME = {}
    PARAM_DEFAULTS_BOX_SPRING = PARAM_DEFAULTS_ARBITRAGE = PARAM_DEFAULTS_MARKET_MAKING = {}
    run_backtest = run_backtest_hft = run_backtest_box_extreme = None
    run_backtest_box_spring = run_backtest_arbitrage = run_backtest_market_making = None
...
class LiveStrategySelector:
    ...
    def _measure_delay(self, tick: Dict[str, Any]) -> float:
        ...
        return max(0, int(time.time() * 1000) - exchange_ts)
    ...
    def _apply_params(self, optimal_df: pd.DataFrame):
        ...
        for _, row in optimal_df.iterrows():
            self._last_params[row["strategy"]] = TimeParams(
                mode=row["mode"], ticks=int(row["ticks"]),
                bars=int(row["bars"]), delay_ms=float(row["delay_ms"]),
            )

    def _degrade_frequency(self):
        logger.warning("[DEGRADE] 延迟%.1fms超过%.1fms阈值，高频策略降级",
        ...
        for strategy, params in self._last_params.items():
            ...
            if params.mode == "lead":
                self._last_params[strategy] = TimeParams(
                    mode="lag", ticks=params.ticks, bars=max(params.bars, 1), delay_ms=params.delay_ms)
                logger.info("[DEGRADE] %s: lead→lag, rate %.1f→%.1f", strategy, old_rate, new_rate)
            elif params.bars < 2:
                self._last_params[strategy] = TimeParams(
                    mode=params.mode, ticks=params.ticks, bars=params.bars + 1, delay_ms=params.delay_ms)
                logger.info("[DEGRADE] %s: bars %d→%d, rate %.1f→%.1f",
