...
# P1-53修复: 从backtest_config统一导入，消除重复定义
from param_pool.backtest.backtest_config import PREPROCESSED_DB, RESULTS_DB

from param_pool.optimization.phase_scan import (
...
try:
    from param_pool._param_grids import P0_IRON_RULES as _P0_IRON_RULES_LOCAL
...
# ---------- 物理约束裁剪 ----------
# P0-18修复: fallback已删除,统一使用从enhanced_phase_scan导入的版本

# ---------- P0绿灯检验 ----------
# P0-18修复: 直接使用从enhanced_phase_scan导入的p0_gate_check(原本地同名函数递归调用自身已删除)
...
def create_objective(
    ...
    def objective(trial: optuna.Trial) -> Tuple[float, float, float]:
        ...
        # ===== P0绿灯检验=====
        p0_result = p0_gate_check(train_result, test_result, params)
        p0_passed = p0_result.passed
        p0_msgs = p0_result.failures + p0_result.warnings
...
# ---------- Sigmoid综合评分 ----------
# P0-18修复: 本地定义(enhanced_phase_scan未导出sigmoid_score)
...
def main():
    ...
    logging.info("[R21-MATH-P1-03修复] 全局随机种子已设置 seed=%d", args.seed)
    ...
    except (ImportError, AttributeError) as _wfv_e:
        logger.debug("T-15: WalkForwardValidator跳过: %s", _wfv_e)

    # T-16修复: Optuna集成MultipleComparisonCorrector对帕累托前沿做BH校正
    try:
        from param_pool.validation.statistical_validation import MultipleComparisonCorrector
    ...
    except (ImportError, AttributeError) as _mcc_e:
