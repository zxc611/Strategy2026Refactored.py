...
from param_pool.backtest.backtest_config import PREPROCESSED_DB
...
def _enrich_backtest_result(result: Dict, bar_data_len: int) -> Dict:
    ...
    from param_pool.optimization.optuna_multiobjective_search import enrich_backtest_result as _public_enrich
...
def p0_gate_check(train_result, test_result, params):
    ...
    # ===== 传统P0检验(保留, 与瀑布式互补) =====
    try:
        from param_pool.backtest.backtest_config import P0_IRON_RULES as _P0
    except ImportError:
        _P0 = {
    ...
    # P0-3: 核心约束硬执行
    try:
        from param_pool.backtest.backtest_config import P0_IRON_RULES as _P0_LOCAL
    except ImportError:
        _P0_LOCAL = {
...
def phase2_scan(best_strategy: Dict, train_data, test_data, strategy_type="main", max_round2_combos=5000,
    ...
    try:
        from precompute._quantification_core import SoftConstrainedOptimizer
