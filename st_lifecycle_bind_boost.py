...
class TestBindPlatformApis:
    ...
    def test_subscribe_wrapper_deduplicates_platform_calls(self):
        provider = MagicMock()
        provider._lock = threading.Lock()
        provider._runtime_market_center = None
        provider._fallback_market_center = None
        ...
        strategy = MagicMock()
        calls = []
        def sub_market_data(exchange, instrument_id):
            calls.append((exchange, instrument_id))
        strategy.sub_market_data = sub_market_data
        strategy.unsub_market_data = MagicMock(return_value=None)
        ...
        provider.subscribe('IF2606')
        provider.subscribe('IF2606')
        provider.subscribe('IH2606')

        assert calls == [('CFFEX', 'IF2606'), ('CFFEX', 'IH2606')]
