...
class TestTierDetermination:
    ...
    def test_positive_correct_up_below_noise_ratio_still_tradeable(self):
        """positive correct_up_pct is not filtered solely because it is below noise_ratio."""
        tier = determine_tier(
            coverage=0.3, wilson=0.2, correct_up_pct=0.4, noise_ratio=0.5
        )
        assert tier == 3, \
            "positive correct_up_pct above Tier3 threshold should remain tradeable"
...
class TestRegressionGuards:
    ...
    def test_coverage_ignores_pure_other_months(self):
        """Pure-other months are inventory diagnostics, not active trading coverage."""
        weights = resolve_month_weights(3)
        data = [
            {'cr': 0, 'cf': 0, 'wr': 0, 'wf': 0, 'other': 100},
            {'cr': 1, 'cf': 0, 'wr': 0, 'wf': 0, 'other': 99},
            {'cr': 0, 'cf': 0, 'wr': 0, 'wf': 0, 'other': 0},
        ]
        result = compute_coverage(data, weights)
        assert result == pytest.approx(1 / MAX_MONTHS_FOR_SCORING, abs=1e-10)
    ...
    def test_first_option_tick_refreshes_timestamp_and_ttl_keeps_active_option(self):
        """The first live tick must make a zero-price registration active for cache cleanup."""

        class _FakeParams:
            def get_instrument_meta(self, fid):
                return {'product': 'FG', 'year_month': '2607', 'exchange': 'CZCE'} if fid == 101 else None

            def get_internal_id(self, instrument_id):
                return 9001 if instrument_id == 'FG607C2000' else None

            def get(self, key, default=None):
                return default

        cache = WidthStrengthCache(params_service=_FakeParams())
        cache.register_option('FG607C2000', 'FG', '2607', 2000, 'CALL', 0, underlying_future_id=101, internal_id=9001)
        cache._option_info_timestamps[9001] = 1.0
        cache._last_cache_cleanup = 0.0

        cache.on_option_tick('FG607C2000', 12.0, 1)

        assert 9001 in cache._option_info
        assert 9001 in cache._options_by_future_type[101]['CALL']
        assert cache._option_info_timestamps[9001] > 1.0
        assert cache._status_counts[101]['2607']['CALL']['other'] == 1

    def test_capacity_cleanup_skips_active_and_removes_inactive_from_indexes(self, monkeypatch):
        """Capacity cleanup must not silently orphan active options or stale indexes."""
        ...
        import ali2026v3_trading.data.width_cache_state_mixin as state_module

        monkeypatch.setattr(state_module, 'MAX_OPTION_CACHE_SIZE', 1)

        cache = WidthStrengthCache(params_service=_FakeParams())
        cache.register_option('FG607C2000', 'FG', '2607', 2000, 'CALL', 12, underlying_future_id=101, internal_id=9001)
        cache.register_option('FG607C2100', 'FG', '2607', 2100, 'CALL', 0, underlying_future_id=101, internal_id=9002)
        cache._option_info_timestamps[9001] = 1.0
        cache._option_info_timestamps[9002] = 2.0
        cache._last_cache_cleanup = 0.0

        cache._enforce_cache_size_limit()

        assert 9001 in cache._option_info
        assert 9002 not in cache._option_info
        assert 9002 not in cache._options_by_future_type[101]['CALL']
        assert cache._status_counts[101]['2607']['CALL']['other'] == 1
